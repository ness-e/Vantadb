...
| `FIND-109` | Alta | WAL sharded truncado deja el MCP hard-down sin vía de reparación: `verify_shard_counts` (ERR-011) aborta `server --mcp` con exit 1 (`WAL shard 1 is truncated: 39 durable records, but round-robin requires at least 40`) y no existe `wal repair/inspect/salvage` (solo `compact`/`vacuum`, ambos abren read-write y fallan igual); solo-lectura (`doctor`, `export`) sí abre. Pedir: `vanta-cli wal salvage` (replay de lo coherente + reporte explícito de registros descartados, nunca skip silencioso) y error tipado en MCP en vez de exit 1. Origen: caída MCP 2026-09-17 en `C:/Users/Eros/.vantadb` (rescate OK: backup 268MB + `export` 6 regs + DB fresca + `import` 6; originales en `.vantadb-corrupt-20260917` + `.vantadb.bak-20260917`) | `src/wal_sharded.rs:69-94,281`, `src/storage/engine/init.rs:413,486-490`, `src/cli_handlers/wal.rs`, `src/cli_handlers/server.rs:253` | 🟡 | 🔴 Alta | ⬜ Pendiente |
| `FIND-110` | Media | S4 bandeja aprobacion sin lifecycle: `CaptureApprovalQueue` in-memory por proceso (`approval.rs:9`); MCP stdio stateless = mostrador vacio salvo mismo-proceso. Pedir: disenar lifecycle (dueno del queue en proceso MCP) o persistencia; ship solo con scope-proceso + test en-proceso. Origen: FIND-107 S4 Gate diseno 2026-09-17 | `vanta-memory/src/core/record/approval.rs`, `vanta-memory/src/gateway/approval_handlers.rs` | 🟡 | 🟡 Media | ⬜ Pendiente (DEFER-ratificado) |
| `FIND-111` | Media | S5 skill_extract sin runner: `run_skill_extract_once`/`extract_skills_with_llm` exigen `LlmRunner` generico sin path degradado (`skill_extractor.rs:209`); exponer como solo-candidatos read-only o DEFER. Origen: FIND-107 S5 Gate diseno 2026-09-17 | `vanta-memory/src/core/skill/` | 🟡 | 🟡 Media | ⬜ Pendiente (DEFER-ratificado) |
| `FIND-112` | Media | S6a wiki_ingest runner real: hoy `start_ingest::<NoLlm>` fijo (`wiki.rs:274`) = sources skipped; runner real = nueva dependencia LLM + config + secrets (SPEC Ask-first). Origen: FIND-107 S6a pre-mortem #2 2026-09-17 | `vantadb-mcp/src/wiki.rs`, `vanta-memory/src/ingest/` | 🟡 | 🟡 Media | ⬜ Pendiente (DEFER-ratificado) |
| `FIND-113` | Media | S6b programador: `PipelineWorker::run_once` + `LocalStateBackend` + locks TTL; exponer exige dueno del backend en proceso MCP (dir estado, quien lo reclama). Origen: FIND-107 S6b 2026-09-17 | `vanta-memory/src/services/pipeline_worker.rs` | 🟡 | 🟡 Media | ⬜ Pendiente (DEFER-ratificado) |
| `FIND-114` | Baja | `examples/python/agent_memory.py` usa import legacy `vantadb_py.VantaDB` -> migrar a `Client` (patron SHOW-04 `test_sdk.py` + `agent_memory_demo.py`). Origen: SHOW-04 Gate C 2026-09-17 | `examples/python/agent_memory.py` | 🟢 | 🟢 Baja | ⬜ Pendiente |
| `FIND-115` | Baja | `README_ES.md:250,256` + `web/.../docs-view.tsx:263,270` duplican one-liner viejo de instalacion -> sync con one-liner FIND-105 o referenciar. Origen: FIND-105 Gate C 2026-09-17 | `README_ES.md`, `web/` docs-view | 🟢 | 🟢 Baja | ⬜ Pendiente |
