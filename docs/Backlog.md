...
| `EMB-11` | Alta | Asistente de instalación con defaults (Q1/Q2: 3 verificados + resto avisado; secrets nunca a disco) | `setup-embeddings.ps1` (nuevo), `embeddings/`, `docs/tasks/EMB-11.md` | 🟡 | 🟠 Media-Alta | ⬜ Pendiente |
| `EMB-12` | Alta | Script lanzador (env + arranque MCP, `--db` explícito) | `vanta-mcp-local.ps1` (nuevo), `opencode.jsonc`, `docs/tasks/EMB-12.md` | 🟢 | 🟠 Media-Alta | ⬜ Pendiente |
| `EMB-13` | Alta | `embed_texts` al proveedor real + fallback avisado `"fallback": true` (núcleo FIND-99, Q5) | `vantadb-mcp/src/handlers/tools.rs:2607`, `docs/tasks/EMB-13.md` | 🟡 | 🔴 Alta | ⬜ Pendiente |
| `EMB-14` | Alta | Auto-embed en `memory_put`/`put_batch` (respeta vector provisto; fallo → guarda + aviso) (cierra FIND-99 con EMB-13) | `vantadb-mcp/src/handlers/tools.rs`, `docs/tasks/EMB-14.md` | 🟡 | 🔴 Alta | ⬜ Pendiente |
| `EMB-15` | Media | Embed de query con el mismo proveedor + prueba sinónimos (FIND-99) | `vantadb-mcp/src/handlers/tools.rs`, `docs/tasks/EMB-15.md` | 🟡 | 🟠 Media-Alta | ⬜ Pendiente |
| `EMB-16` | Media | Prefijos e5 (`query:`/`passage:`) por familia + margen medido | `src/llm.rs`, `docs/tasks/EMB-16.md` | 🟢 | 🟡 Media | ⬜ Pendiente |
| `EMB-17` | Media | Parámetro `model` honrado (caché por modelo + error claro; Q2) | `vantadb-mcp/src/handlers/tools.rs`, `src/llm.rs`, `docs/tasks/EMB-17.md` | 🟡 | 🟡 Media | ⬜ Pendiente |
| `EMB-18` | Media | Regla una-dim-por-base: error claro + comando reindex (Q4, nunca auto silencioso) | `vantadb-mcp/src/server.rs`, `docs/tasks/EMB-18.md` | 🟢 | 🟡 Media | ⬜ Pendiente |
| `EMB-19` | Alta | Verificación e2e + reinstalación coordinada (matriz local/ollama/openai condicional) | `~/.cargo/bin/vanta-cli.exe`, `docs/tasks/EMB-19.md` | 🟡 | 🔴 Alta | ⬜ Pendiente |
| `EMB-20` | Baja | Docs: README tabla + MCP doc + SKILL (Regla 11) | `embeddings/README.md`, `docs/api/MCP.md`, `docs/tasks/EMB-20.md` | 🟢 | 🟢 Baja | ⬜ Pendiente |
| `FIND-99` | Media | `embed_texts` del MCP devuelve siempre vectores falsos deterministas (hash, sin significado): el handler llama `embed_texts_fallback` hardcodeado y nunca al proveedor real — probado: sim("el gato duerme","un felino descansa")=-0.03 vs sim con "mecánica cuántica")=+0.13. Peor que fallar: parecen reales y ordenan al azar. Además `memory_put` no auto-embed por vía MCP (guarda sin vector; recall degrada a keyword D38) y el path del modelo local es relativo al CWD. Arreglar = cablear `embed_texts` a `LocalOnnxProvider` (modelo e5-small 448MB + tokenizer ya en `embeddings/models/`) + auto-embed en `memory_put` o documentar límite. Origen: verificación MVP 2026-09-16 (código `tools.rs:2607,3148-3204` + prueba semántica en vivo) | `vantadb-mcp/src/handlers/tools.rs:2607`, `src/llm.rs:139-156`, `embeddings/models/` | 🟡 | 🟡 Media | ⬜ Pendiente |
