...
| `FIND-123` | Baja | `vs-table.tsx` compara latencias/costos sin fuente visible (Regla 11 + web AGENTS.md: claims numéricos DEBEN salir de `docs/operations/BENCHMARKS.md`): `vantadb:"1.2ms"` OK (BENCHMARKS.md:34 p50 HNSW 10K), pero `pinecone:"~50-150ms"`, `weaviate:"~20-80ms"`, `chroma:"~5-30ms"`, `"$1,800/mo"`, `"$600/mo"` sin cita. Fix: agregar fuente por celda o calificar como aproximado/fechado; verificar resto de filas del archivo. Origen: DIST-10/14 Gate C 2026-09-19 (verificado por lead en `web/src/components/vanta/vs-table.tsx:23-26,78-79`) | `web/src/components/vanta/vs-table.tsx` | 🟢 | 🟢 Baja | ⬜ Pendiente |
| `FIND-124` | Alta | Build & Test rojo en Linux/Windows/macOS (PR #182): `desktop/src-tauri` usa API `vantadb::Vanta*` eliminada hace meses (`VantaEmbedded`, `VantaConfig`, `VantaError`, `VantaMemory*`, `VantaNodeRecord`, `VantaQueryResult`, `VantaBm25TermContribution` — E0432 en `src-tauri tests`, verificado en log CI run 35413944127). Pre-existente (no lo rompió ninguna campaña; AST-010 renombró y desktop no migró). Fix: migrar `desktop/src-tauri` a la API actual + compila en los 3 OS. Origen: PR #182 CI 2026-09-19 (Gate P owner: crear tarea fix) | `desktop/src-tauri/` | 🟡 | 🔴 Alta | ⬜ Pendiente |
| `FIND-125` | Media | Tests npm/TS rojo (PR #182): ~15 errores de tipos en `vantadb-ts/src/vantadb.ts` (drift vs core/wasm: `Capabilities.runtime_profile` 395:30, `MemoryListPage` 546:5, `SearchRequestInput` 603:37/657:55/802:40, `SearchExplanation` 801:5, `ExportReport` 829:5/870:5, `ImportReport` 938:5, `OperationalMetrics` 1090:5, `IqlResult` vs `QueryResult` 1108:5, `NodeRecord` 1181:20, `GraphBfs/Dfs/TopoResult` 1322:5/1346:5/1366:5/1413:5). Pre-existente. Fix: resync tipos TS vs core hasta `tsc --noEmit` 0. Origen: PR #182 CI 2026-09-19 run 35413944241 (Gate P owner: crear tarea resync) | `vantadb-ts/src/vantadb.ts` | 🟡 | 🟠 Media-Alta | ⬜ Pendiente |
| `FIND-126` | Baja | Checks docs rojos (PR #182): Check Frontmatter exige `title:` en los 1473 `docs/**/*.md` (falla estructural, pre-existente) + Lint Markdown con 70 issues viejos en 12 archivos (solo 1 era nuestro `MCP.md:212`, ya fixed `4f2d7d1c`). Fix: tarea de limpieza (frontmatter masivo o scope reducido a archivos del PR + 70 lints viejos). Origen: PR #182 CI 2026-09-19 (Gate P owner: tarea limpieza) | `docs/` (12 archivos con issues viejos) | 🟢 | 🟡 Media | ⬜ Pendiente |
| `FIND-127` | Media | Triage infra CI + Dependabot (PR #182): providers openai/ollama/litellm fallan por imagen sin rustc (`'rustc' is not installed for toolchain '1.95.0'`), wheel linux-aarch64 sin docker (exit 127), CodeQL falla en 14s (config?), Vercel deploy failed, scheduled HEAVY con `startup_failure` + 20 vulns Dependabot en default (2 critical, 10 high — aviso del push). Todo pre-existente/infra, nada de código nuestro. Fix: triage + fix workflows (imagen rustc, docker aarch64, CodeQL, Vercel) + triage SEC/AUD de las 20 vulns. Origen: PR #182 CI 2026-09-19 + push 2026-09-19 (Gate P owner: tarea triage) | `.github/workflows/` (providers, wheels, codeql, vercel) | 🟡 | 🟠 Media-Alta | ⬜ Pendiente |
| `FIND-128` | Media | Auditoría integral de los 28 workflows (organización, distribución, nombres, docs, necesidad): solo 7/28 tienen header, `CI_POLICY.md` menciona 25 yml, triggers mezclados (`heavy-certification-50` y `ci-gate` solo schedule, `perf-bench-40` sin schedule ni PR, 5 release-* sin PR). Evaluar por workflow qué hace/por qué/cómo, cuáles sobran o se fusionan, normalizar headers + registrar inventario en `CI_POLICY.md`. Origen: pedido owner 2026-09-19 (verificación: 28 yml listados + conteo headers + matriz triggers) | `.github/workflows/`, `docs/operations/CI_POLICY.md` | 🟡 | 🟡 Media | ⬜ Pendiente |
| `FIND-129` | Media | Triage de 20 PRs abiertos (19 dependabot + release-plz v0.6.0 #161): no cerrarlos a ciegas — mergear por lotes con CI verde (seguridad primero: #180 sharp+next cubre alertas sharp/Next.js; luego patch/minor; majors `toml 0.9→1.1` #174 y `rocksdb 0.24→0.25` #175 con cuidado). #161 `chore(vantadb): release v0.6.0` (+1818/-9, 2026-09-03) se revisa en el flujo release, no se descarta. Documentar orden de merge + cerrar solo superseded con motivo. Origen: pedido owner 2026-09-19 (21 PRs abiertos listados) | PRs #161, #162-180, #183 | 🟢 | 🟡 Media | ⬜ Pendiente |
| `FIND-130` | Alta | `memory_recall` no ve L1 del pipeline automático (Codex P1 en PR #182): usa sesión sintética `mcp` (`scope:"session"` solo lee `l1/mcp`) y el reader exige `agent_id`/`team_id` iguales a los defaults mientras el pipeline escribe `None` → los recuerdos L1 son invisibles para la tool publicitada. Fix: rutear recall a sesiones L1 persistidas o alinear IDs de aislamiento. Archivos: `vantadb-mcp/src/handlers/tools.rs:1723-1731` (verificar en DISCOVERY, línea del review). Origen: Codex review PR #182 2026-09-19 | `vantadb-mcp/src/handlers/tools.rs` | 🟡 | 🔴 Alta | ⬜ Pendiente |
| `FIND-131` | Media | `docker-compose.yml` sin auth por defecto (Codex P1 en PR #182): bind no-loopback sin API key ni override → `validate_auth_config` rechaza y el contenedor muere antes del healthcheck (también perfil unprivileged). Fix: key documentada/env o perfil loopback por defecto. Archivos: `vantadb-server/docker-compose.yml:18-24` (verificar en DISCOVERY). Origen: Codex review PR #182 2026-09-19 | `vantadb-server/docker-compose.yml` | 🟢 | 🟡 Media | ⬜ Pendiente |
| `FIND-132` | Media | Pérdida de mensajes L0 en el mismo milisegundo (Codex P2 en PR #182): `/conversation/add` concurrentes derivan la misma key `t{timestamp}_0` y `L0Recorder` rechaza `timestamp_ms <= cursor` → un mensaje se pierde en silencio. Fix: ID estable o desempate (cursor + secuencia). Archivos: `vanta-memory/src/services/conversation_hook.rs:64-68` (verificar en DISCOVERY). Origen: Codex review PR #182 2026-09-19 | `vanta-memory/src/services/conversation_hook.rs` | 🟡 | 🟡 Media | ⬜ Pendiente |
