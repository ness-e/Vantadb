...
| `FIND-89` | Media | Consolidar 12 ficheros con lectura directa `env::var`/env fuera de `Config` en fuente unica (Q2=B: no consolidado en F3C-impl; dos fuentes temporalmente) — incluye legacy `VANTA_DB` (CLI), `VANTA_EMBEDDING_PROVIDER`/`VANTA_OPENAI_*` (`src/llm.rs`), `VANTA_BACKUP_DIR`, mirrors `VANTA_PREFETCH`/`VANTA_LOCAL_MODEL` fuera de `config.rs`. Origen: ref: plan-2026-09-13-cleanCA-fase3#F3C H2. **COMPLETADO 2026-09-14** — 4 slices S1-S4: commits `1ca57649` (S1 llm), `3a0e42d7` (S2 vector/prefetch), `4cdc1970` (S3 telemetry/maintenance/crypto/metadata), S4 help texts + CONFIGURATION.md. Solo `env::var` restantes son excepciones documentadas: test en `llm.rs` (VANTADB_OPENAI_API_KEY), `metadata.rs` (ENV_REPORTED_VERSION interno), `telemetry.rs` (OTEL_* estándar OpenTelemetry). | `src/cli.rs`, `src/crypto.rs`, `src/cli_handlers/server.rs`, `src/llm.rs`, `src/metadata.rs`, `src/index/graph/prefetch.rs`, `src/server/bootstrap.rs`, `src/physical_plan/vector.rs`, `src/server/telemetry.rs`, `src/error.rs`, `src/server/errors.rs`, `src/storage/engine/maintenance.rs` | 🟡 | 🟡 Media | ✅ **COMPLETED** (residual 0) |
...
> **Cierre MGR (obligatorio en las 21 filas):** (1) escribir `docs/research/mgr-XX-<tema>.md` con TODA la información conseguida; (2) hacer las preguntas necesarias con question-opencode al owner; (3) crear el plan de implementación o grupo de tareas de implementación con el mismo nivel de detalle.
...
| `EXE-03` | **Ejecutar el gate Fase A** — 🎯 OP: correr el gate (5 installs limpias documentadas, stranger-test 2-3 personas, 0 críticos). OS: registrar resultados; veredicto GO/NO-GO de anuncio. Tipo: ejecución + validación. Cobertura: Roadmap Fase A; transversal. Refs Notion: Roadmap. Dep: MGR-19, EXE-01. DoD: reporte de gate firmado. | `docs/` (reporte) | 🟡 1sem | 🔴 | 🆕 Pendiente (P50 2026-09-14) |
...
| `EXE-08` | **Utopia a futuro (ontología, multimodal, memoria ejecutable)** — 🎯 OP: registrar direcciones como research futuro (scope, triggers de activación, no-acción hoy). OS: mover a P24 (FUT-15) cuando se abra. Tipo: research-registro. Cobertura: Problema § direcciones futuras; fuera de alcance v1.0. Refs Notion: Problema. Dep: ninguna. DoD: fila FUT-15 creada o registro equivalente. | `docs/research/` | 🟢 1d | 🔵 | 🆕 Pendiente (P50 2026-09-14) |
