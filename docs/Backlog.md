...
| `FIND-64` | Alta | CI no dispara en cambios solo-`vanta-memory/`: paths `ci-rust-10.yml` (`vantadb-*/**`) no matchean el crate — agregar `'vanta-memory/**'` | `.github/workflows/ci-rust-10.yml` | 🟢 | 🔴 Alta | ✅ Completada 2026-09-15 (`0f16cd39`; gemelo → `FIND-92`) |
| `FIND-65` | Alta | Test `cache::tests::ttl_expiry_predicate` paniquea (`Instant - 10_000s` overflow en hosts con uptime <2.7h) — usar `checked_sub` o duraciones pequeñas | `vanta-proxy/src/cache.rs:760-763` | 🟢 | 🟠 Media-Alta | ✅ Completada 2026-09-15 (fix solo-test: `checked_sub` + sleep 10ms/TTL 1ms; prod `elapsed()` intacto; suite cache 18/18; fmt OK; clippy vanta-proxy 0 warnings — `-D warnings` global bloqueado por FIND-93 pre-existente) |
...
| `FIND-69` | Media | Fallback dspy roto sin framework: `super().__init__(k=k)` → TypeError siempre — tolerar o no llamar super en fallback | `integrations/dspy/vantadb_dspy/vectorstore.py:20-25,62` | 🟢 | 🟡 Media | ✅ Completada 2026-09-15 (try/except TypeError + `self.k = k` explícito; fallback probado aislado con DB stub: INIT OK, forward `_Prediction.passages`, dump/load OK; `py_compile` 0; suite 8/8 bloqueada por FIND-94 drift `vanta.VantaDB` pre-existente, no regresión) |
...
| `FIND-77` | Media | Comentarios stale conteo tools MCP (76 vs 79 reales) en `tools.rs:1090` + `config.rs` | `vantadb-mcp/src/handlers/tools.rs:1090` | 🟢 | 🟢 Baja | ✅ Completada 2026-09-15 (recount mecánico 79=49+30 via parser JSON; bloque :25-35 reescrito ro 46/33 de 11/68 id 61/18 ow 2/77 hits 85/115; check 0 warnings; profiles+annotations tests ok; review P2-01 2 hallazgos aplicados; commit c32940db) |
...
| `FIND-86` | Media | Memory diferidos: MEM-69 wiring al worker + MCP tool 77 `scene_consolidate` (diseñada ADR-040) + MEM-70 números reales; consolidación automática la espec MGR-08 (esta fila = wiring, MGR-08 = diseño) | `vanta-memory/` | 🟡 | 🟡 Media | ⬜ Pendiente |
...
| `FIND-90` | Alta | Fallout F3X: `vantadb-mcp` no compila — E0609 `.config`/`.nodes` sobre `Guard<Arc<Box<dyn IndexPort>>>` en `resources.rs:134` (config clone para `schema://`), `tools.rs:1821` (`.config.distance_metric`), `tools.rs:2850` (`.nodes.iter()` para dim). Migrar a getters del trait (`distance_metric()`, serialización vía trait, iteración vía `all_node_ids`/`stored_vector`). Bloquea crate mcp + `vantadb-server` + `mcp_tests` + Fast Gate workspace. Origen: validación proyecto 2026-09-15, causa `13f0f729` | `vantadb-mcp/src/handlers/resources.rs:134`, `vantadb-mcp/src/handlers/tools.rs:1821,2850` | 🟡 | 🔴 Alta | ⬜ Pendiente |
| `FIND-91` | Alta | Fallout F3X: `tests/durability_recovery.rs:433` usa `hnsw.nodes.get` sobre `dyn IndexPort` (E0609 con `--all-features`) — migrar a `contains_node` (el `node_count()` de la línea siguiente ya usa el trait). Bloquea clippy `--all-features`, perfil chaos y job heavy `storage-persistence`. Origen: validación proyecto 2026-09-15, causa `13f0f729` | `tests/durability_recovery.rs:433` | 🟢 | 🔴 Alta | ✅ Completada 2026-09-15 (fix 1 línea; desbloquea clippy --all-features + chaos) |
| `FIND-92` | Media | Gap gemelo de FIND-64: paths `ci-rustdoc.yml` (`vantadb-*/**`) tampoco matchean `vanta-memory/` — agregar `'vanta-memory/**'` en push+PR. Origen: ref plan 2026-09-15-find-correcciones#FIND-64 (discovery 2026-09-15; `rg vanta-memory .github/workflows/` = 0 hits) | `.github/workflows/ci-rustdoc.yml:24,33` | 🟢 | 🟡 Media | ⬜ Pendiente |
| `FIND-93` | Media | `clippy -p vanta-proxy --all-targets -- -D warnings` rojo por `dead_code` pre-existente en la dependencia `vantadb` lib (`cloned_sole_buffer` nunca usado) — `#[allow]`/`#[expect]` o eliminar el método. Bloquea el gate clippy con `-D warnings` de cualquier crate que dependa de `vantadb`. Origen: verify FIND-65 2026-09-15 (repro sin el fix vía `git stash`: mismo error; `vanta-proxy` solo = 0 warnings) | `src/storage/engine/txn.rs:158` | 🟢 | 🟡 Media | ⬜ Pendiente |
| `FIND-94` | Media | Drift SDK 0.5.0 vs adapters: `vantadb_py` (editable desde `vantadb-python/`, v0.5.0) ya NO expone `VantaDB` (solo `Client`/`connect` + `MemoryClient`/`GraphClient`/...) pero los 9 adapters de `integrations/` construyen `vanta.VantaDB(...)` (dspy:75, letta:43, haystack:135, crewai:68+104, langchain checkpointer:80/store:88/vectorstore:44, llamaindex:53, mem0:122, shared:47) — todas sus suites fallan con `AttributeError`. Migrar los 9 al API `Client`/`connect` o re-exponer shim `VantaDB`. Origen: verify FIND-69 2026-09-15 (`pytest integrations/dspy/tests/` 5 failed + 3 errors todos en `vanta.VantaDB`; `hasattr(vantadb,'VantaDB') is False`; `rg VantaDB\( integrations/` = 12 hits en 9 adapters) | `integrations/*/vantadb_*/vectorstore.py`, `vantadb-python/vantadb_py/` | 🟡 | 🟡 Media | ⬜ Pendiente |
