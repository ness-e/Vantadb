...
|---|---|---|---|---|---|
| `STABLE-06` | **Validar `vantadb-ts` (npm)** — no entra en `default-members`, pero gate npm debe ser Fast Gate. Gates 5+2: `release-npm-61.yml:tests` ya mide 26s (<5 min) — re-medir en CI limpio + `npm ci && npm run build && npx vitest run` 280 tests (medido 2026-09-08: 275 estáticos src/__tests__ + 5 tests/graph.test.ts; vitest 278/278 en 2026-09-05 commit 7ff70b01, +2 MOD-24) + `npx eslint .` + `npm pack` incluye `engines`. | `vantadb-ts/package.json`, `vantadb-ts/src/`, `.github/workflows/release-npm-61.yml` | 🟢 4h | 🟠 Media | ⬜ Pendiente |
...
### Revisión SyntropyOS Org (pendientes del usuario — RE-PREGUNTAR otro día)

Contexto: org `SyntropyOS` configurada (admin `ness-e`, email/website/description/topics, repo `.github` público con 15 archivos ES/EN, commit `cc521d1`+). `ness-e/Vantadb` NO migrado ni modificado (decisión explícita). Respuestas del usuario 2026-09-08: dominio → quedarse en GitHub por ahora; migración → pospuesta; Discord → usar VantaDB Community tal cual; invite nuevo → otro día.

- `ORG-01` — Invite permanente nuevo de Discord + Server ID (widget). Estado: invite actual `g8nqB3NtXt` (VantaDB Community, permanente) en 5 archivos; badge estático sin conteo. RE-PREGUNTAR: pedir código del invite nuevo + Server ID → actualizar READMEs/MANIFESTO/CONTRIBUTING en 1 commit.
- `ORG-02` — Migración `ness-e/Vantadb` → `SyntropyOS/vantadb`. Estado: pospuesta/sin decidir. RE-PREGUNTAR: decisión migrar/dejar → si migra: transferir repo, actualizar homepage PyPI/npm, enlaces `.github`, pins. [2026-09-08] Creados 9 repos `SyntropyOS/syntropy-{iris,reverb,cardinal,orchestra,sage,execute,plan,reason,meta}` (públicos, README inicial ES/EN + LICENSE Apache 2.0, 2 commits c/u). VantaDB NO incluido (sin migrar, por decisión).
- `ORG-03` — Dominio `syntropyai.com`: registrado pero EN VENTA vía GoDaddy (brokerage, precio a convenir — verificado 2026-09-08). `syntropy.com` = Syntropy Technologies LLC (biomédica). `syntropyos.org` = portal contable ajeno. RE-PREGUNTAR: comprar `syntropyai.com` o alternativo (`syntropy-os.org`, `syntropyos.dev` — verificar antes).
- `ORG-04` — Homepage npm `vantadb.dev` MUERTO (sin DNS); sitio real vivo: `vantadb.vercel.app` (verificado 2026-09-08, con docs/benchmarks/FAQ). Requiere editar `ness-e/Vantadb` (package.json) — PROHIBIDO tocarlo sin orden del owner. RE-PREGUNTAR: autorización para fix o reportarlo como issue al owner.
- `ORG-05` — Discord API bloqueada desde este entorno: bot `VantaDB` válido y dentro del guild, pero todo `/guilds/{id}/*` → `40333 internal network error` (6 intentos). Widget SÍ activo. RE-PREGUNTAR: (1) confirmar en el Developer Portal que App ID `1524473671345897512` (creada 2026-07-08, coincide con el bot) y Public Key `59355c7f…14546a` (formato válido, verificado 2026-09-08) coinciden; (2) `/applications/{id}/commands` da `403 Forbidden` — posible app limitada: regenerar token y reintentar, o runbook manual. [2026-09-08+] App ID+secret+key VERIFICADOS por API: grant `client_credentials` aceptado (secret válido para esa app), `verify_key` de `/applications/{id}/rpc` == public key entregada ✅. Secreto usado solo en memoria, NO guardado en ningún archivo. Bloqueo 403/40333 en rutas guild/commands persiste con el bot token actual.
- `ORG-06` — Servidor `SYNTROPY` vacío (1 miembro, solo `#general`, invite expirado 2026-10-07). RE-PREGUNTAR: borrar tras consolidación (requiere cliente Discord, owner).
- `ORG-07` — Render visual final de `github.com/SyntropyOS` (badges derecha/idiomas izquierda, taglines centrados, espejos ES/EN). Fetch no muestra layout real. RE-PREGUNTAR: que el usuario confirme visualmente o envíe screenshot.
- `ORG-08` — Esquema de naming `syntropy-[nombre]` + subdominios: APROBADO con condiciones (evaluación 2026-09-08). Scope canónico **`@syntropy-ai/*`** — org npm `syntropy-ai` creada y verificada (0 paquetes). `@syntropy` DESCARTADO: ocupado desde ~2017 por terceros (`mongoose-document-versioner`, `repo-tools`, `token-manager`). Nombres con colisión rechazados: `echo` (Amazon Echo), `compass` (MongoDB Compass); con ruido: `prism` (Prism.js), `nexus` (Sonatype Nexus), `lumen`, `synapse`. **Nombres elegidos por el usuario (2026-09-08): Iris (Visión), Reverb (Audio), Cardinal (Orientación), Orchestra (Coordinación)** — publicados en tabla de holones del profile README ES/EN como "Próximamente". **Segunda vuelta (2026-09-08): Sage (Aprendizaje), Execute (Ejecución), Plan (Planificación), Reason (Razonamiento), Meta (Metacognición)** — agregados a la misma tabla. Comunicación DESCARTADO (lo cubren los LLMs). Orchestra → candidato a core (no holón cognitivo). Flag: `Meta` colisiona con Meta/Facebook como marca. NO renombrar `vantadb`/`vantadb-py` (tracción existente).
- `ORG-09` — Primer publish bajo `@syntropy-ai/*` (scope vacío, 0 paquetes). Solo con holón real (prohibido squatting/placeholders en npm). Depende de: nombre finalista de ORG-08 + ORG-10 (auth).
- `ORG-10` — Auth de publicación npm antes del primer publish (verificado 2026-09-08 en changelog oficial `2026-07-08-npm-install-time-security-and-gat-bypass2fa-deprecation` + docs `configuring-two-factor-authentication`): (a) tokens granulares con bypass-2FA YA no pueden hacer account/org/package management (vigente desde ago-2026); (b) perderán el publish DIRECTO ~ene-2027 → solo leer privados + *staged publishing* con aprobación humana 2FA; vía futura = *trusted publishing* (OIDC) — ver también changelog `Sep.03 multiple trusted publishing configurations`. Acción: cuenta npm con 2FA (`auth-and-writes`, recovery codes guardados, GitHub linkeado como recovery) + trusted publishing en CI al publicar `@syntropy-ai/*`. Estado local: CLI npm 11.6.0 SIN login (`ENEEDAUTH`) → `npm login` pendiente al publicar. Owner: `vanta-lead` (CI/CD).
- `ORG-11` — PyPI `syntropy-*`: los 5 candidatos verificados libres el 2026-09-08 (`vantadb`, `prism`, `echo`, `compass`, `nexus` → todos 404). PyPI no reserva sin publicar: asegurar SOLO finalistas al publicar (no squatting masivo).