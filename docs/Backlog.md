...
| `FIND-98` | Media | Binario `vanta-cli` instalado (2026-08-23) stale: sirve 56 tools vs 79 del fuente — faltan `memory_search/recall/versions/supersede`, `search_with_method/multi`, `thread_*` (6, memoria episódica ausente vía agente), `scene_*` (3), `context_assemble`, `snapshot_create/restore`, `vacuum`, `embed_texts`, `remove_edge`, axiomas write/delete. Fix: reinstalar desde fuente (`cargo install --path` o copiar `target/debug/vanta-cli.exe` 2026-09-16) cuando el proceso vivo PID 3864 (otra sesión, server MCP con ese binario desde 01:07) libere el archivo — hoy está bloqueado con lock de Windows. Origen: verificación MVP 2026-09-16 (diff tools/list 79 vs 56 + Copy-Item bloqueado) | `C:/Users/Eros/.cargo/bin/vanta-cli.exe`, `target/debug/vanta-cli.exe` | 🟢 | 🟡 Media | ⬜ Pendiente |
...
| `FIND-100` | Alta | `ort::init().commit()` hace `expect` (BadVersion/Dlopen) + mutex poisoned → abort del proceso (CLI) / pánico (HTTP 500) con onnxruntime <1.27 — la "degradación graceful" es falsa sin dylib compatible. Fix: `catch_unwind` + pre-chequeo de versión, o empaquetar dylib. Origen: EMB-10 2026-09-16 (crash `ort/lib.rs:234` BadVersion 1.17.1 System32; solución env-only ORT 1.30 en Temp + `ORT_DYLIB_PATH`; task `docs/tasks/EMB-10.md:148-171`) | `src/llm.rs` (init ort), `docs/tasks/EMB-10.md` | 🟡 | 🔴 Alta | ⬜ Pendiente |
| `FIND-101` | Baja | `vanta-cli query` abre read-only (`data.rs:212`) → todo IQL INSERT/UPDATE/DELETE por CLI falla. Doc vs fix a decidir. Origen: EMB-10 2026-09-16 (`docs/tasks/EMB-10.md:173`) | `src/bin/vanta-cli.rs`, `data.rs:212` | 🟢 | 🟢 Baja | ⬜ Pendiente |
| `FIND-102` | Baja | `tests/sdk_serialization.rs` no compila (39 errores, `QueryResult` no declarado) — pre-existente, ajeno a embeddings. Origen: EMB-10 2026-09-16 (`docs/tasks/EMB-10.md:174`) | `tests/sdk_serialization.rs` | 🟢 | 🟢 Baja | ⬜ Pendiente |
| `FIND-103` | Alta | Skills MCP: investigación en código + revisión profunda y corrección total del contenido (counts 45→79 en SKILLMANIFEST/AGENTS, paths `~` que no expanden, env vars LLM/MCP sin documentar en `configuration.md`, `initialize` sin `instructions`, 4 prompts genéricos sin workflow recall-first, `test-mcp.py` solo-handshake) + diseño de recall mid-conversación (hooks SessionStart/message/PreCompact/Stop, recall automático por mensaje con filtros temporales tipo "ayer a las 2pm" (incluye traductor determinista de expresiones temporales a rangos ms con tests, no improvisación del agente), política de recall con umbrales top-k, inyección vía `additionalContext`, auto-capture al cerrar y diseño de curaduría proxy→hilos vía bandeja de aprobación; las plantillas por cliente que ejecutan esta política viven en FIND-106) para memoria real continua sin depender de AGENTS.md. Origen: verificación MVP + research skills/hooks 2026-09-16 | `skills/vantadb-mcp/`, `vantadb-mcp/src/handlers/`, `docs/api/MCP.md` | 🟡 | 🔴 Alta | ⬜ Pendiente |
| `FIND-106` | Alta | Ganchos de memoria por cliente: investigación + análisis + mejores prácticas + optimización (plantillas OpenCode/Claude/Codex: SessionStart recall, recall por mensaje con umbral top-k, PreCompact save-state, Stop auto-capture; presupuesto de tokens con umbrales, qué se inyecta y cuándo nada; tests por cliente; implementa la política motor de FIND-103 por cliente). Origen: pedido owner 2026-09-16 (memoria "siempre, sin excepción") | `.opencode/`, `skills/vantadb-mcp/assets/`, `vanta-mcp-local.ps1` | 🟡 | 🔴 Alta | ⬜ Pendiente |
| `FIND-107` | Alta | Exponer en el MCP lo que falta de `vanta-memory`: sueños (consolidate/promote/discard), bandeja de aprobación, extracción de habilidades, ingesta con runner real, programador de fondo/orquestación, escenas en escritura; cada uno con tool, schema, tests y docs (dividir por sub-módulo si excede appetite). Origen: gaps R2 verificación MVP 2026-09-16 | `vanta-memory/src/`, `vantadb-mcp/src/` | 🟠 | 🟠 Media-Alta | ⬜ Pendiente |
| `FIND-104` | Media | Instalador interactivo completo: extender el wizard a flujo end-to-end con todo explicado (modelo con tabla tamaño/idioma/tiempo, carpeta de base, proveedor local/ollama/openai con detección de servidor/key, runtime ORT ≥1.27, escritura del bloque MCP por cliente, línea de regla en AGENTS.md/CLAUDE.md, ofrecer proxy default prendido (con aviso de proceso extra + apagado fácil, config TOML mínima, guía base_url, namespace `proxy-turns` + flujo de curaduría a hilos), resumen + prueba viva final put→recall→search; Enter = defaults, `-NonInteractive` intacto, re-ejecutable sin romper, secrets nunca a disco). Origen: pedido owner 2026-09-16 (cero fricción) | `setup-embeddings.ps1`, `vanta-mcp-local.ps1`, `skills/vantadb-mcp/assets/` | 🟡 | 🟠 Media-Alta | ⬜ Pendiente |
| `FIND-105` | Media | Comando único de instalación en conjunto con el instalador interactivo: publicar el instalador como asset descargable sin clonar (raw URL versionada + checksum), one-liner por OS (`iwr|iex` / `curl|sh`) que encadena instalador→wizard interactivo, documentado en README/QUICKSTART. Sin rustup ni clone para el usuario final. Origen: pedido owner 2026-09-16 (cero fricción) | `scripts/install.sh`, `scripts/install.ps1`, `README.md`, `docs/QUICKSTART.md` | 🟢 | 🟠 Media-Alta | ⬜ Pendiente |
| `FIND-108` | Media | Integración completa open-code-review (hoy solo delegation sin key): review-rules por path mapeadas a reglas VantaDB (`.opencode/rules/`), job CI nocturno `ocr review` (requiere key, documentado), session viewer como evidencia de auditoría; delegation sigue default sin key. Origen: research integración OCR 2026-09-17 (solo `dev-tools/ocr-review.ps1` + gates; sin rules/CI/plugins/MCP) | `.github/workflows/`, `.opencodereview/` (nuevo), `dev-tools/ocr-review.ps1` | 🟡 | 🟡 Media | ⬜ Pendiente |
...
| `SHOW-04` | **Prototipo estrella agente-con-memoria** (asistente CLI que recuerda entre sesiones vía `vanta-memory` scenes + MCP `inject_context`): reusa `langgraph_checkpoint.py` + `agent_memory.py`; consolidación auto la espec MGR-08 (esta fila = demo, MGR-08 = diseño) | `examples/` (nuevo `agent_memory_cli/`) | 🟠 3-5d | 🔴 Alta | ⬜ Pendiente |
