...
### The Memory Problem
Prior to integrating VantaDB, CodexAgent maintained history using a raw in-memory HNSW index (via FAISS). See [[architecture/ARCHITECTURE#HNSW Graph|HNSW architecture]] for details on the production index. This caused several problems:
...
2. **Inconsistent Search Quality:** Semantic search failed on precise technical terms (like matching specific Rust crate versions, e.g., `PyO3 v0.24.1`). Conversely, keyword search ([[architecture/TEXT_INDEX_DESIGN#BM25 Ranking|BM25]]) failed to match conceptual intents.
...
* **Durable Writes:** CodexAgent writes interaction logs, code snippets, and error payloads to VantaDB. Every write writes to the [[architecture/adr/ADR-002-WAL-CRC32C-AUTOHEALING|Write-Ahead Log (WAL) with CRC32C validation]].
* **Topological Re-layout:** Every hour or upon index rebuild request, VantaDB runs a [[architecture/ARCHITECTURE#Topological Re-layout|BFS re-layout]] of the memory-mapped HNSW graph to align nodes topologically on disk.
...
| **Recall@10 Accuracy** | 81.2% (Semantic only) | **100.00%** ([[architecture/adr/ADR-006-RRF-CONSTANT|RRF]]) | 🟢 Perfect precision on mixed queries |
...
1. **[[architecture/adr/ADR-006-RRF-CONSTANT|RRF Fuses Rankings Correctly]]:** During testing, the query `"PyO3 GIL release"` was run. Semantic search returned general Python multithreading articles. BM25 keyword search returned exact PyO3 binding reference docs. RRF combined the ranks and correctly put the exact PyO3 bindings documentation at Rank 1.
2. **Page Fault Mitigation via [[architecture/ARCHITECTURE#Topological Re-layout|BFS Layout]]:** Without the BFS compaction reorder, running the HNSW search on disk-mapped files triggered high levels of OS page faults, dropping query QPS to 8. Compacting the graph using the BFS layout restored performance to **26.7 QPS** (37ms latency) on local consumer storage.
