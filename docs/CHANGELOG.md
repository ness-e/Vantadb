...
## [0.6.0](https://github.com/ness-e/Vantadb/compare/v0.5.0...v0.6.0) - 2026-09-20

### Added

- SHOW-03 — prototipo RAG-sobre-PDFs 100% local con citas verificables
- SHOW-02 — 6 recetas clicables del playground + nota secuencial MCP
- IMPL-112-S2 — runners ollama/openai de ingesta (tests 7-8 G2, degrada-P4 + mock-escribe, 0 codigo prod)
- IMPL-112-S1 — runner local de ingesta (cfg TOML+env, enum ConcreteRunner, wiring wiki_ingest, tests 1-6,9-10)
- FIND-111 — skill_extract solo-candidatos read-only con degrada honesta (86→87 tools)
- FIND-109 — wal salvage opt-in + MCP WAL error tipado
- FIND-105 — comando único de instalación (one-liner por OS + wizard chain + docs)
- FIND-106 — ganchos de memoria por cliente (OpenCode+Claude+Cursor+Codex, tests 50/50, token budget)
- FIND-104 — instalador interactivo completo end-to-end
- FIND-107 S3 - dream_consolidate/promote MCP tools (86 total)
- FIND-107 S2 - dream_list/load/discard MCP tools (84 total)
- FIND-107 S1 - scene_write/scene_edit MCP tools (81 total)
- SHOW-04 — demo agente-con-memoria (2 sesiones, 1 comando, local)
- EMB-17 parametro model honrado (switch por nombre, Q2)
- EMB-15 embed de query con el MISMO proveedor + prueba sinonimos
- EMB-14 auto-embed en memory_put/put_batch + fallback avisado (FIND-99 con EMB-13)
- EMB-18 regla una-dim-por-base con guía de regeneración
- EMB-12 launcher MCP con env explicito + smoke dim==384
- EMB-11 asistente instalacion embeddings con defaults + ORT nativo >=1.27
- EMB-10 build embed-local + token_type_ids fix + cat test con senal real
...
- PRX-09-wiring cablea with_embedder en AppState con gate semantic_enabled
- *(desktop)* i18n ES/EN lentes + paneles via tt()/tp() (DESKTOP-40-slice3)
- PRX-11-slice2 thinking fidelity fina + TranslateConfig contract-first (hook DEFER)
- PRX-09-embeddings — trait EmbedProvider + embed-path en cache
- PRX-11 slice 1 — translate.rs Anthropic↔OpenAI (lib pura + prx11 9/9)
- PRX-10 — per-key model allowlists (gate 1c) + 403 guardrail_blocked
- MCP-41 — auto-consolidación conversacional local-first (slice vertical)
- PRX-13 — Optimización contexto en tránsito
- PRX-09-slice2 - semantic caching + TTL + LRU en vanta-proxy
- INTG-02 - Backend Memory unificada CrewAI (StorageBackend + tests)
- PRX-07 - PII/secret redaction en egress
- INTG-01 — Adapter LangGraph (checkpointer + BaseStore)
...
- PRX-02 — fallback multi-upstream + retries con backoff exponencial
- MEM-69 batch extraccion costo-reducida (split+dedup en 1 llamada)
- DESKTOP-40-slice2 — i18n shell chrome
- DESKTOP-40 — i18n ES/EN slice 1 (catalogo + tt/tp + Settings)
- FIND-21 menu contextual propio + atajos Alt+T y Ctrl+, desktop
- FIND-20 persistencia estado ventana desktop (manual, sin plugin)
- PRX-12 compat suite contra releases de coding agents (6 fixtures + 6 tests regresion)
- PRX-09 exact response cache first slice (semantic DEFER)
- PRX-05 model discovery + endpoints auxiliares (GET /v1/models desde config, count_tokens local, beta passthrough)
- MEM-68 gate opcional de aprobacion de capturas (default off)
- STABLE-09 subset default-members <5min (Owner A)
- BLOG-CTA CTAs + metadata serie blogs + posts 6-7 drafts
- PRX-04 cache-preserving injection (prefijo estable byte-a-byte)
- MEM-66 claimStaleTasks (recuperación multi-worker)
- PRX-08 higiene proxy (auth O(1), self-loop fail-fast, caps sesion/buckets, writeback append, mixed-tools filter)
- public benchmarks page with sourced tables (WEB-02)
- *(proxy)* wiring advance+classifier+mem-commands+degraded (PRX-01)
...
- FIND-123 — fuentes/calificación Regla 11 en vs-table
- FIND-124 — migrar desktop/src-tauri a API post-AST-010 (Vanta* -> sdk/error/config)
- FIND-98-retry — rebuild par con embed-local verificado 87 + fallback:false, reinstall STOP por lock
- FIND-117 — TUI REPL documenta limite de writes read-only + flush tras Write
- P2-01 review follow-ups (seguimiento-mvp: assert salvage, nota flush, Trust honesto, SPEC URLs)
- FIND-101 — vanta-cli query abre read-write para IQL mutante (parse-then-open)
- P2-01 review follow-ups (FIND-100/103/104/105/106/108)
- FIND-103 - skills correctas + motor de recall continuo
- FIND-100 — graceful ante onnxruntime incompatible (fallback dummy sin abort)
- EMB-13 embed_texts al proveedor real + fallback avisado (Q5)
- FIND-97 — espejar triage rsa AST-007 a audit.toml (cargo audit exit 0)
- FIND-96 — bump rustls 0.23.43→0.23.45 (RUSTSEC-2026-0285) + docs deps
- FIND-94 — migrar 9 adapters de vanta.VantaDB a Client/memory.* + score-similitud
- FIND-93 — gate cloned_sole_buffer bajo feature rayon
- FIND-72 — CLI argparse + pins + WinError32 en benches py
- FIND-84 — pins + fixtures + dist/PyPI integrations (9 adapters)
- FIND-69 — dspy sin framework tolera fallback object + self.k explicito
- FIND-73 — key en 3 .pyi (store) + verify_pyi con firmas inspect/ast
- FIND-71 — embeddings patterns recortados + verify smoke + sizes re-medidos
- FIND-65 — checked_sub en test TTL de vanta-proxy (solo test, prod intacto)
- FIND-90 — handlers MCP a getters IndexPort (fallout F3X E0609)
- FIND-91 — durability_recovery:433 nodes.get → contains_node (fallout F3X E0609 --all-features)
- *(llm)* use Config-backed constructors in Default impls and test (FIND-89)
- DTO id number|bigint simetrico en grafo TS (D5d)
- distance_metric/method desconocido lanza error (D5b)
- clamp top_k/MAX_K en server handlers (D5c)
- PROV-openai verify-first provider openai compila sin cambios (contrato verde, bug pre-fixado)
- node typings u64 as bigint (FIND-BND12-01)
- desktop a11y keyboard focus labels slice (UX-A11Y-01)
- migrate llamaindex put_batch to kwargs signature (FIND-64)
...
- bump next 16.1.1->16.3.5 + sharp 0.35.4 en web (cubre RCE criticos #35/#34 + libheif; supersede #180)\
- archive planes cierre-total + publicacion\
- progreso cierre-total (Backlog -6, avance 4 dominios) + cierre planes\
- P2-01 Wave A transcripciones (CODEX + FIND-128, approve)\
- FIND-129 — triage 20 PRs (Lote1a admin a develop, #180 staging, resto veredicto)
- FIND-133 — Step 6 cierre (commit 3c4f146c + recitation)
- FIND-128 — fix ADR-Gate multiline GITHUB_OUTPUT + matriz triggers 28 workflows
- *(deps)* bump PyO3/maturin-action ([#163](https://github.com/ness-e/Vantadb/pull/163))
- *(deps)* bump pypa/gh-action-pypi-publish from 1.14.1 to 1.14.2 ([#164](https://github.com/ness-e/Vantadb/pull/164))
- *(deps)* bump github/codeql-action/analyze from 4.37.1 to 4.38.0 ([#165](https://github.com/ness-e/Vantadb/pull/165))
- *(deps)* bump github/codeql-action/init from 4.37.1 to 4.38.0 ([#168](https://github.com/ness-e/Vantadb/pull/168))
- archive plan ci-green 5/5 + nota meta
- progreso plan ci-green 5/5 (Backlog -5, avance 4 dominios)
- EXE-03-prep — kit gate Fase A (checklist + usuario-01 + README honesto)
- FIND-127 — providers a API post-AST-010 + wheels aarch64 portable + triage Dependabot
- FIND-126 — limpieza docs formato-only (frontmatter + 70 lints, 0 issues)
- FIND-128..132 + plan CI-green PR #182 (5 DO)
- FIND-124/125/126/127 (fallos CI PR #182: desktop, TS, docs, infra+deps)
- fix MD028 blank-in-blockquote MCP.md:212 (CI Lint Markdown)
- PROV-12 recitation + FIND-98 re-DEFER + FIND-123 (Gate C DIST)
- PROV-12 — release wheels listo y verificado en seco (TestPyPI primero, sin publish local)
- DIST-10/14 — plan distribucion + adopcion npm (canales, comparativa Orama, nicho, checklist anuncio)
- publicacion 6/6 lineas Dependencias (wave + NextTask)
- archive plan 2026-09-10-correcciones (superado por 2026-09-15, sin iniciar)
- publicacion 6/6 Dependencias + archive smoke-e2e 7/7 + nota meta
- plan Publicacion MVP (6 DO: parity+showcase+distribucion+PyPI+Fase-A-kit)
- smoke-e2e 7 fases verdes + triage (0 bugs nuevos, GO a Publicacion)
- plan smoke-e2e 7 fases (verificacion punta a punta pre-publicacion)
- FIND-120/121/122 + P51 IMPL-MGR + backlog-notion.md (N-01..N-11)
- archive plan cierre-mvp 9/9 + nota meta
- progreso plan cierre-mvp (Backlog -4, avance 3 dominios, plan 9/9, SPEC alcance)
- SHOW-05 — referenciar vantadb-ts/examples desde README y QUICKSTART (no mover)
- FIND-119 — sync counts mirrors 86 to 87 (solo strings, sin commit submodule)
- FIND-116 — migrate remaining examples from legacy VantaDB/search_memory to Client
- FIND-113-spec — diseño dueño scheduler S6b (spec-first, cero código)
- FIND-110-spec — diseño lifecycle S4 bandeja aprobación (spec-first, cero código)
- FIND-98 — stop por lock Windows, parity 79/87 pendiente (task file + evidencia)
- archive plan seguimiento-mvp 9/9 + nota meta
- progreso plan seguimiento-mvp (Backlog -9/+1 FIND-116, avance 3 dominios, plan 9/9)
- FIND-115 — sync one-liner con FIND-105 (nota fuente unica ES + Trust/wizard docs-view)
- FIND-114 — migrate agent_memory.py to Client
- FIND-102 - sdk_serialization ya verde (drift QueryResult autocorregido, 16/16, asserts intactos)
- FIND-113 — S6b programador re-DEFER fundado (sin dueno defendible en 1d, espejo FIND-110)
- FIND-112 — spec runner real de ingesta (trait+matriz, TOML+env, lifecycle-dueno, gates+tests, degradado P4)
- FIND-111 — avance bindings (skill_extract SHIP 234f0627)
- FIND-110 — S4 bandeja aprobación re-DEFER fundado (sin dueño defendible en 1d)
- archive budget plan MVP memoria-agentes
- archive plan MVP memoria-agentes 8/8 + nota meta
- progreso plan MVP memoria-agentes (Backlog -8/+6, avance 3 dominios, plan 8/8)
- FIND-108 — review-rules por path + job CI nocturno + viewer como evidencia
- FIND-107 - task file sync cierre (S1-S3 ship, S4-S6 DEFER-ratificado)
- SHOW-04 — registra avance en bindings.md
- tauri lockfile bumps + archive orphan campaign budgets 2026-09-02
- OCR delegation (wrapper sin key + workflow informativo + recetas just)
- *(cli)* regenerate shell completions (tui subcommand)
- SPEC.md + plan MVP memoria-agentes + Backlog FIND-103..108
- archive plan embeddings-auto 11/11 + nota meta
- progreso Wave5 embeddings-auto (EMB-19/20 ✅ + avance + cierre)
- EMB-20 embeddings docs (README + MCP + SKILL) — tabla 9 modelos + 3 proveedores + dim-gate + embed_texts real
- EMB-19 verificacion punta a punta + reinstalacion coordinada
- progreso Wave4 embeddings-auto (EMB-15/17 ✅ + avance)
- progreso Wave3 embeddings-auto (EMB-14 ✅ + FIND-99 cerrada)
- progreso Wave2 embeddings-auto (EMB-13/16/18 ✅ + avance)
- EMB-16 prefijos e5 por familia + margen medido
- progreso Wave1 embeddings-auto (EMB-11/12 ✅ + avance operaciones)
- plan embeddings-auto + Backlog EMB-10..20 (+FIND-99/100/101/102) + progreso EMB-10
- audit-quick INDEX + state (post FIND-96/97, gate verde)
- cierre campaña FIND 2026-09-15 — progreso masivo 31/31 + archive
- FIND-95 — add vanta-memory/** to rustdoc-70 paths (push+PR)
- FIND-92 — add vanta-memory/** to ci-rustdoc paths (push+PR)
- FIND-86 — discovery + spec + verify (wiring pendiente Gate D)
- FIND-74 — requirements + enlaces + decision TS
- FIND-80 — seed corpus + crash upload + fuzz-pr
- FIND-85 — recorte classifiers a 3.11/3.13 + firma put_batch_raw verificada 3 vías
- FIND-75 — WASM README re-medido + d.ts sync doc
...
- FIND-68 — docs/api/PROXY.md (endpoints + 8 opt-in + defaults + env) + enlace master-index
- FIND-83 — bump opencode submodule (skills sync 3-files 8b8b9a4) + review P2-01 approve
- FIND-82 — asserts de conteo por perfil en test-mcp.py (gate anti-drift)
- FIND-77 — sync Backlog + avance (completada c32940db)
- FIND-77 — conteo 76→79 en comentarios MCP (49 base + 30 extend, hits 115)
- FIND-83 — unificar skills (copias + MCP-27/29 + api-ref)
- FIND-87 — ./native en exports + seccion native TS_SDK
- FIND-71 — registro avance operaciones (commit 94c94f25)
- FIND-79 — portabilidad TS con importRecords estricto (fix put-loop) + pins FS
- FIND-70 — run ingestion_concurrent in nightly with async-ingestion
- FIND-78 — link review a archive + nota engines node18 vs ts22.19
- FIND-63 — suite desktop verde por archivo (localStorage stub) + tsc Client alias\
- FIND-65 — registro avance operaciones (commit 432eee45)
- FIND-88 — DEFER-ratificado record_response_usage SSE drain (sin punto de buffer general, suite proxy 163/163)
- FIND-64 — add vanta-memory/** to ci-rust-10 paths (push+PR)
- *(templates)* 4 plantillas schema M0 (api/research/review/operations)
- *(metadata)* frontmatter canonico vault Obsidian M0-M11\n\n- M0: schema title/type/status/tags/last_reviewed/aliases+related\n- M1: FM 38 nucleo vivo (api/ops/tutorial/strategy/web...)\n- M2: FM 40 research vigente (tdam/validacion/evals active, corpus stable)\n- M3/M4: FM minimo archived 55 reviews + 38 campanas\n- M5/M6: FM 27 web + 15 workflow (related al yml)\n- M8: 7 stubs glosario (qdrant/lancedb/crdt/bincode/serde/otel/wasm)\n- M9: related en 11 hubs (indices, SDKs, roadmap, vision, planes)\n- M10: book SUMMARY + stub progreso -> avance/\n- M11: last_refined -> last_reviewed (64 glosario); template ya OK\n- Excluidos: tasks/ plans/ book-output (M7)
- *(exhaustivo)* A1-A7 auditoria docs por area\n\n- A1: links progreso/ rotos -> avance/ (master-index, README x2)\n- A2: VANTA_BACKEND -> VANTADB_BACKEND (deploy x3, tuning x3)\n- A3: BENCHMARKS ref heavy-certification-50.yml + banner\n  canonico IVF post-b4ff157d + _run_stdout.md a archive/\n- A4: conteos CI 14/15 -> 26 workflows verificados\n- A5: BINDINGS TS 38->43 metodos, GRAPH_RAG dedup + ejemplo\n  canonico, TS_SDK fecha\n- A6: tutorials a SDK real (Client/search/memory.get)\n- A7: 3 templates muertos + ejemplos fnd05 a archive/
- *(find-89)* cierre FIND-89 — review approve + avance + archivar plan
- *(backlog)* header 100 abiertas / 103 filas
- *(backlog)* analisis tarea por tarea + 6 correcciones
- *(research,backlog)* auditoria research 5 frentes + fusiones + archive
- *(strategy)* mover manual estrategico a docs/strategy + refs
- *(negocio,research)* validacion manual estrategico + BIZ-04..08
- *(backlog,strategy)* auditoria fila por fila + limpieza + vigencia
...
- diseno trait-split storage-index + ADR-datos (F3X-diseno)
- cierre gate Fase 2 a 4/4 con re-medicion M1 + excepcion firmada + firma A1 (F3G)
- cierre campana cleanCA Fase 2 (18/18) + avance + archivar plan
- romper ciclos sdk-planner-query con hoja neutral (C2M3)
- convencion prefijo--modificador utilities web (C2A2)
- API publica bindings TS+Python+WASM baseline (C2C2)
- extraer CacheLayer sin stutter N2 (C2S3b)
- AAA Act bulk en load/certification (C2T3)
- determinismo seeds + clock inyectable sin sleep TTL (C2T2)
- aislar lentos intra-binario con ignore+doc suite completa (C2T1)
- Task 13b S3b-CacheLayer tras Wave 3 en plan Fase 2
- extraer TxnManager de StorageEngine slice-txn (C2S3)
- partir Executor por variante de Statement (C2S5)
- registry/factory de backends OCP (C2S2)
- decision D0 B+B + Fase 2 (plan)
- puerto EntityRepository en entity/ (C2M2)
- partir AccessTracker en AccessStats+Pinnable (C2S7)
- segregar StorageBackend en roles + tipar checkpoint/compact (C2S1)
- boundaries + doctrina hibrida + regla anti-ciclo (C2A1)
- ADR-datos decision Config monolitico (C2D0)
- limpieza comentarios redundantes (C2C1)
- baseline metricas Ca/Ce/I/A/D + gate ADP informativo (C2M1)
- archivo planes stutter + config + api-reference MCP
- avance AST-001
- backlog + plan correcciones 26 tasks + informe validacion
- call-sites D2 restantes (AccessMode, TelemetrySink) (D2-resto)
- [**breaking**] renames Error en call-sites restantes (D4b sweep)
- renames Error en server/errors.rs (D4b resto)
- archivar plan cleanCA-remediation (campana 15/15)
- plan remediacion cleanCA con adaptador y decision benches (H2)
- diseno renames enum Error + ADR-datos (D4b-diseno)
- validar frontera TS con DbError (D5a)
- structs de comando para firmas 5-7 params (D3)
- flags bool a enums dedicados (D2)
- ListRecordsUseCase extraido de records_list (D1c)
- partir vector_search y run_pipeline en helpers SLAP (D1b)
- partir get y batch_insert en helpers SLAP (D1a)
- eliminar unwrap prod con Result/? + invariantes (B2b)
- triaje unwrap prod vs test + plan B2b (B2a)
- StartConversationUseCase extraido de conversation_add (B1)
- ThreadStore sin stutter (D4a)
- STU-001/002/003/004 task files + plan stutter-interno
- STU-003 entity/scene sin stutter + serde alias
- STU-001 renames baratos sin stutter
- STU-002 AsyncVantaDB→AsyncClient
- STU-004 FOOTER_GROUPS→GROUPS
- [**breaking**] AST-012 subclientes Python sin apellido + plano limpio (rename directo, 0 aliases)
- AST-011 sync estado COMPLETED + review veredicto
- AST-011 verify final + cierre campana anti-stutter (4/4, archiva plan)
- [**breaking**] AST-010 quitar aliases deprecated + compat-notes (rename directo, 0 usuarios)
- [**breaking**] AST-008 Python directo pyclass Client + Record/SearchHit/ListResult + search namespaced
- [**breaking**] AST-009 wasm Client + TS isValidValue/isMetadata + tests canonicos
- registra AST-007 en avance/ci-cd + dependencias + meta (archiva plan 7/7)
- archiva plan anti-stutter 7/7 COMPLETED (AST-001...007)
- AST-007 release gate major + cero-remanentes (semver 15/0 + deny + verify 3143)
- registra AST-006 en avance/bindings
- AST-006 docs anti-stutter + compat notes (scoped contract + coverage 0 gaps)
- registra AST-005 en avance/bindings
- [**breaking**] AST-005 metodos sin stutter + aliases deprecated
- registra AST-004 en avance/bindings
- [**breaking**] AST-002 Rust tipos sin stutter + aliases deprecated
- freeze anti-stutter map + ADR (AST-001)
- *(showcase)* P49 tareas SHOW-01..05 + examples/README.md indice
- *(review)* apendice detalle 19 modulos + FIND-82..88
- *(review)* informe 19 módulos + FIND-63..81 (review-full-20260910)
- *(campaign)* sync PRX-09-wiring verificada + P39 completado (/pipeline task)
- *(campaign)* sync PRX-11-slice3 completada lead-inline (plan 2026-09-10-code)
- *(campaign)* sync slices D40-slice3 + PRX-11-slice2/3 + PRX-09-wiring (plan 2026-09-10-code)
- registra PRX-09-embeddings slice 3 en avance/operaciones
- PRX-09 cierra slice 3 embeddings (steps S10-S13)
- *(campaign)* cierra plan 2026-09-10-code 19/19 (proxy+memoria+desktop+server+providers+integrations)
- GOV-TK8 - Benchmarks section 18 run 2026-08-12 + evidencia R11
- *(campaign)* cierra plan 2026-09-10-fixes 6/6 (recitation sync)
- PROV-openai registra avance bindings + archiva plan 2026-09-10-fixes 6/6
- DESKTOP-40 sync recitation campaign state en plan
- DESKTOP-40 registra avance desktop + slice 2 DEFER en Backlog
- FIND-21 registra menu contextual + atajos en avance/desktop + recitation
- FIND-21 sync cierre completed en plan + task file
- FIND-20 registra persistencia ventana en avance/desktop
- FIND-20 sync cierre completed en plan
- FIND-20 sync recitation completed en plan
- ISSUE-TS-001 resuelto-stale verify-first 280/280 (plan 2026-09-10-fixes)
- FIND-MCP-001 — sync recitation completed en plan
- FIND-MCP-001 — registra re-verificacion 2026-09-10 en avance/bindings
- FIND-MCP-001 — sync hash 8e98823f en plan
- FIND-MCP-001 — verify-first contrato verde (check --tests jobs 2 exit 0, 14/14 nextest, E0786 no reproducido, cero cambios)
- *(campaign)* cierra plan 2026-09-09-backlog 6/6 + P47 cerrada (STABLE-09 subset)
- PRX-12 registra compat suite en avance/vanta-proxy
...
- *(node)* BND-08 reconcile — contrato verificado mecánicamente
- bump opencode submodule (campaign_verify_cmd fix 4fdc916)
- FIND-50 split parser mod.rs into grammar/lexer
- FIND-48 split graph.rs 2031L por concern (types/prefetch/core/tests)
- FIND-49 sync estado task file a COMPLETED (re-verificado)
- FIND-49 split sdk types por dominio (record/search/graph)
- *(campaign)* GOV-TK2 gap-muerto a historial (plan sync)
- *(campaign)* archiva 2026-09-07-cleanup-gates (3/3 completo)
- *(campaign)* cleanup Wave0 3/3 STABLE-05 + GOV-TK2 + DOC-SYNC-01 (plan sync + backlog + avance)
- validate wasm gates (STABLE-05)
- sync stale backlog counts (DOC-SYNC-01) + re-verify MCP tools gap as dead (GOV-TK2)
- *(campaign)* archiva 2026-09-07-followup (4/4 completo)
- *(campaign)* followup Wave1 STABLE-07 (plan sync + backlog + avance)
- validate node matrix and pack (STABLE-07)
- *(campaign)* followup Wave0 3/3 FIND-BND12-01 + PERF-BENCH-01 + UX-19 (plan sync + backlog + avance)
- fix multi-perfil locator, smoke guard green (UX-19)
- node native vs wasm A/B numbers (PERF-BENCH-01)
- *(campaign)* archiva 2026-09-07-backlog-triage (8/8 completo)
- *(campaign)* Wave2 2/2 BND-13 + WEB-02 + stale SKIP (plan sync + backlog + avance)
- complete NODE_SDK with matrix and runtime examples (BND-13)
- *(campaign)* Wave1 3/3 BND-12 + TS-09 + UX-A11Y-01 (plan sync + backlog + avance)
- reproducible JS/WASM insert search bench (TS-09)
- expand node coverage search batch capabilities close (BND-12)
- *(campaign)* Wave0 3/3 FIND-64 + MOD-24 + FIND-60 (plan sync + backlog + avance)
- fix rustdoc warnings to zero (FIND-60)
- dedup TS guards and map helpers (MOD-24)
- *(campaign)* archiva durability-release-readiness (12/12 completo)
- *(campaign)* cierra durability-release-readiness 12/12 (2026-09-07)
- *(campaign)* Wave 3 3/3 - BND-09 + MCP-34b + PRX-01 (plan sync + backlog + avance)
- bump opencode submodule (prompt sub-agentes enriquecido)
- bump opencode submodule (learnings STABLE-04)
...
## [0.6.1](https://github.com/ness-e/Vantadb/compare/v0.6.0...v0.6.1) - 2026-09-22
...
- *(deps-dev)* update maturin requirement in /vantadb-python ([#194](https://github.com/ness-e/Vantadb/pull/194))
