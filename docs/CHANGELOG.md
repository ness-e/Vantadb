...
- **PERF-17: ef_construction 200→400** — Higher recall with moderate index-time cost (`src/index/core.rs`)
- **PERF-18: M/max0 16→32/64** — Better HNSW graph connectivity. m_max0=64 ensures high-layer nodes have sufficient connections (`src/index/core.rs`)
- **PERF-19: WAL batch append** — `WalWriter::append_batch()` and `ShardedWal::append_batch()` serialize multiple records in a single write + single fsync (`src/wal.rs`, `src/wal_sharded.rs`)
- **PERF-20: Storage batch insert** — `insert_batch()` and `delete_batch()` acquire insert lock once, batch WAL append, batch KV writes, batch HNSW add/remove (`src/storage/engine/ops.rs`, `src/sdk/api.rs`)
