...
Every figure below is copied from [`docs/user/operations/BENCHMARKS.md`](operations/BENCHMARKS.md). **They are measurements of specific runs on specific machines — mostly local Windows hardware — not universal guarantees.** Reproduce them with the cited commands before drawing conclusions for your workload.
