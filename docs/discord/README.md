# VantaDB Discord Server — Documentation

**Server:** VantaDB Community
**ID:** 1522793011094749204
**Invite:** https://discord.gg/g8nqB3NtXt
**Icon:** [`d0e08d1dcb8002a809165b60d8ddc03e`](https://cdn.discordapp.com/icons/1522793011094749204/d0e08d1dcb8002a809165b60d8ddc03e.webp)

## Overview

This document covers the complete configuration of the VantaDB Community Discord server — an open-source embedded vector database for AI agents, built in Rust.

### Quick Stats

- **Members:** 3 (owner + VantaDB bot + Carl-bot)
- **Categories:** 8
- **Channels:** 24
- **Roles:** 14
- **Features:** COMMUNITY, NEWS, WELCOME_SCREEN_ENABLED, MEMBER_VERIFICATION_GATE_ENABLED
- **Verification Level:** LOW (email required)
- **Premium Tier:** 0 (no boosts yet)

## Table of Contents

1. [Server Configuration](./server-config.md) — Full channel/category/role/permission layout
2. [Bilingual Strategy](./bilingual-strategy.md) — English/Spanish language architecture
3. [Pending Tasks](./todo.md) — What needs manual setup
