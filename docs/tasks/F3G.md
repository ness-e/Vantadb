...
## Metadata
- **Plan file:** docs/plans/2026-09-13-cleanCA-fase3.md (Wave 0, Task 1)
- **Creado:** 2026-09-13
...
- **Contrato exacto:**
  1. Re-medicion same-tool M1 post-Fase-2 archivada en docs/reviews/ con tabla D antes/despues de storage/engine y sdk (D debe bajar; proxy estructural 26-24-20 campos de StorageEngine).
  2. Parrafo de excepcion GraphAccumulator-new firmado en docs/architecture/BOUNDARIES.md BND-03.
...
- **Acceptance criteria (3 parciales, evidencia verificable por archivo):**
  - [ ] AC1: docs/reviews/dsm-post-fase2-slim.json existe y parsea (ConvertFrom-Json OK) + tabla D antes/despues presente en este task file con D a la baja o HALLAZGO Gate V reportado.
  - [ ] AC2: docs/architecture/BOUNDARIES.md BND-03 contiene el parrafo de excepcion firmada (firma + fecha + justificacion artefacto-ctor + alcance).
...
- **Clave (lectura + 1 edicion docs):**
  - docs/reviews/dsm-baseline-slim.json (baseline, solo lectura)
  - docs/architecture/BOUNDARIES.md (BND-03, edicion: parrafo excepcion + firma A1)
  - docs/architecture/ARCHITECTURE.md (contexto §7, solo lectura)
  - src/accumulator.rs (SOLO LECTURA, verificado: struct :31 + new() :41 -> Self)
- **Relacionados (solo lectura):**
  - docs/tasks/C2M1.md (tabla baseline + sesgos same-tool: subconteo Ca, ceguera macros/globs)
  - docs/tasks/C2S3.md (post-numeros 26->24 campos, slice txn)
  - docs/tasks/C2S3b.md (post-numeros 24->20 campos, slice cache)
  - docs/tasks/C2A1.md (lo pendiente de firma: revision vanta-arch al cierre)
  - docs/reviews/acyclic-baseline.txt (baseline: exit 1 solo self-edge accumulator)
- **Prohibidos:**
...
- Impacto: docs + docs/reviews/; cero src/ (salvo lectura). nextTask F3X.

## 4. REFERENCIAS
- Guia .opencode/references/clean-code-clean-architecture.md §4.3: Ca/Ce/I/A/D con D=|A+I-1| (I=Ce/(Ca+Ce)).
- docs/architecture/BOUNDARIES.md BND-01..08 (BND-03 manda para este gate; BND-05..08 doctrina hibrida como contexto).
...
- **Referencias hacia dentro (nuevas):** docs/reviews/dsm-post-fase2-slim.json + docs/reviews/acyclic-post-fase2.txt (artefactos nuevos, sin entrantes).
...
- **Archivos (lectura):** src/ (via herramientas) · **Archivos (escritura):** docs/reviews/dsm-post-fase2-slim.json, docs/reviews/acyclic-post-fase2.txt, este task file (tabla D).
...
- **Estado:** ✅ COMPLETE (2026-09-13: post archivado docs/reviews/dsm-post-fase2-slim.json 107KB/364 mods + acyclic-post-fase2.txt identico + tabla D abajo; Gate V no-disparado)

### Step G2: excepcion GraphAccumulator-new firmada en BND-03
- **Archivos:** docs/architecture/BOUNDARIES.md (parrafo en BND-03).
...
- **Archivos:** docs/architecture/BOUNDARIES.md (linea de firma de revision).
...
- `cargo modules dependencies --lib -p vantadb --acyclic` (0.27.0): exit 1 SOLO por `vantadb::accumulator::GraphAccumulator <-> GraphAccumulator::new`, IDENTICO a docs/reviews/acyclic-baseline.txt (cero ciclos nuevos).
...
- rust-dsm post archivado: docs/reviews/dsm-post-fase2-slim.json (107KB, 364 mods, misma forma que el baseline slim; CLI escribe el JSON a archivo, no a stdout — leccion registrada).
