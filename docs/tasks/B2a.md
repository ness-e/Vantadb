...
- Cambia: SOLO este archivo `docs/tasks/B2a.md` (tabla archivo:línea + categoría + variante Error para B2b).
- NO cambia: NINGÚN archivo bajo `src/` (prohibido código prod en B2a). No commitear (lo ejecuta el lead).
- Archivos exactos: lectura `src/**/*.rs` (6 focos obligatorios); escritura `docs/tasks/B2a.md`.
...
- Recitation: objetivo B2a completo; estado done; última acción tabla escrita + clippy verde; contrato `cargo clippy -p vantadb -- --deny warnings` ✅ (Finished dev profile, 0 warnings); invariantes: `src/` intacto (`git status` solo muestra `docs/tasks/B2a.md` nuevo); deuda: ninguna para B2a; próxima tarea B2b (fix 44×(c), orden §3).
