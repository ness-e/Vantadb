...
> Campaign: `6ab26f3f-cf16-4416-9255-c18cca0bcaf0` · Plan: `docs/plans/2026-09-15-find-correcciones.md` (Task 25, Wave8)
...
- 1 línea QUICKSTART→examples en `docs/QUICKSTART.md` (hoy 0 hits `examples|demo|colab`, verificado)
...
- `Select-String -Path docs/QUICKSTART.md -Pattern "examples/README"` ≥1 hit (1 línea añadida, 6 hunks FIND-67 intactos)
...
- `docs/QUICKSTART.md:1-214` — leído entero (214 líneas, post-FIND-67 `b5d9b3f8` con 6 hunks: `:6` fecha, `:12` v0.5.0, `:90` wheel, `:142-144` search, `:179-183` audit nota, `:195` VANTADB_*); añadir 1 línea SIN revertir hunks (append al final, no tocar hunks)
...
- Callers de `examples/README.md`: `examples/demo/README.md:33,44` (rutas demo propias), plan file `:370,:424` (cita FIND-74), `docs/QUICKSTART.md:5` (link inverso ya existe desde examples → QUICKSTART, falta QUICKSTART → examples)
- Callers de QUICKSTART: `README.md:60` (`[5-Minute Quickstart](docs/QUICKSTART.md)`), `examples/README.md:5` (`[QUICKSTART](../docs/QUICKSTART.md)`), `SUPPORT.md:11`, plan FIND-67/FIND-74
...
- Untracked ajenos: `.github/workflows/ocr-delegate.yml`, `dev-tools/ocr-review.ps1`, `reparacion.bat`, `docs/plans/2026-09-15-find-correcciones.md` (plan file, solo recitation del orquestador), `docs/plans/archive/*.budget.json`
...
- Orden plan: post-Task 16 FIND-67 por QUICKSTART compartido — YA cumplido (`b5d9b3f8` en `git log`, 6 hunks verificados con `git show b5d9b3f8 -- docs/QUICKSTART.md`); no pisarlo (append-only en QUICKSTART)
...
- FIND-67: commit `b5d9b3f8 docs: FIND-67 — QUICKSTART a 0.5.0` (verificado `git show --stat` + `git show -- docs/QUICKSTART.md` 6 hunks); QUICKSTART actual `last_reviewed: 2026-09-15`, `:12` v0.5.0, `:90` wheel 0.5.0, `:195` VANTADB_* — no revertir
...
Comando: `campaign_discover_skills_v2 archivosClave="examples/demo/requirements.txt, examples/README.md, docs/QUICKSTART.md, README.md" phase="BUILD" contractKeywords=["examples-requirements","version-drift","ts-decision","quickstart-link"] maxSkills=8` → 8 candidatas + 2 keyword-mapped (total 13, filtradas a 8 por score).
...
- `Select-String -Path README.md -Pattern "QUICKSTART|examples/demo|colab"` + `Select-String -Path docs/QUICKSTART.md -Pattern "examples|demo|colab"` — gaps de enlace (= 0 antes del fix)
- `git diff --check` — whitespace errors (= 0 exigido)
- `git status --short` — solo archivos propios al commitear
- `git show b5d9b3f8 -- docs/QUICKSTART.md` — 6 hunks FIND-67 que no se tocan
...
- Grafo: `requirements.txt:1` → `examples/demo/README.md:20,27` (ya 0.5.0, outlier confirmado) → `examples/demo/demo.py:17` (colateral NOTICED, no tocado) → `examples/README.md:16-32` (tabla Python, sin versión, no tocada) → `docs/QUICKSTART.md` (0 links a examples, gap) → `README.md` Quick Links (0 fila examples, gap)
...
- Implicaciones: no romper links (append-only, rutas relativas existentes intactas); riesgo único: pisar 6 hunks FIND-67 en QUICKSTART (mitigación: 1 línea al final, `git diff docs/QUICKSTART.md` debe mostrar +1/-0 en ese archivo)
...
- **Commit conventional `docs:` con task ID (solo archivos propios):** `git add examples/demo/requirements.txt docs/QUICKSTART.md README.md docs/tasks/FIND-74.md` + `git commit -m "docs: FIND-74 — ..."` (NO `.opencode/`, `Justfile`, `completions/`, `Cargo.lock`, `pipeline-state.json`, plan file, `demo.py`, stash)
- **Backlog→avance NO tocar** (race paralelo, orquestador) + **push vía vanta-lead**

## SDP

`campaign_discover_skills_v2 archivosClave="examples/demo/requirements.txt, examples/README.md, docs/QUICKSTART.md, README.md" phase="BUILD" contractKeywords=["examples-requirements","version-drift","ts-decision","quickstart-link"] maxSkills=8` → 8 skills (ver tabla §5).
`SDP: documentation-and-adrs + writing-guidelines + writing-plans + spec-driven-development + source-driven-development + incremental-implementation (+ base campaign-executor/progreso/ponytail; doubt/frontend-ui/api descartadas por docs-only)`
...
- **Archivos leídos (completos):** `examples/demo/requirements.txt` (4 líneas); `examples/README.md` (50 líneas); `docs/QUICKSTART.md` (214 líneas + `git show b5d9b3f8 -- docs/QUICKSTART.md` 6 hunks); `README.md` (349 líneas, foco `:55-70` Quick Links + `:104-138` Quickstart); `examples/demo/README.md` (79 líneas); `examples/demo/demo.py:1-20` (cabecera versión); `Cargo.toml` (workspace.package) + `vantadb-python/pyproject.toml` (versión); `git log --oneline -15` + `git tag --list` + `git status --short` + `git stash list` (WIP ajeno mapeado).
- **Archivos referenciados hacia dentro (lo que los editados citan):** `vantadb-py>=0.5.0` (PyPI/TestPyPI/source `vantadb-python/`); `examples/README.md` (demo + colab + TS tabla); `docs/QUICKSTART.md` (CLI + binding 0.5.0); `vantadb-ts/examples/*.mjs`.
...
Archivos tocados: `examples/demo/requirements.txt`, `docs/QUICKSTART.md`, `README.md` (+ este task file). `examples/README.md` verificado sin edición.
...
- **Claim:** QUICKSTART sin enlace a examples. **Evidencia:** `Select-String -Path docs/QUICKSTART.md -Pattern "examples|demo|colab"` = 0 hits. **Confianza:** alta.
...
- **Archivos:** `examples/demo/requirements.txt:1`, `docs/QUICKSTART.md` (append 1 línea final), `README.md` (1 fila Quick Links) + este task file (sync estados)
- **Acción (mínimo diff, sin reescritura, sin mover ejemplos):**
  ...
  2. `docs/QUICKSTART.md` append final (post `EXPERIMENTAL_FEATURES.md` línea, sin tocar 6 hunks): `> For runnable examples beyond this quickstart, see [examples/](../examples/README.md) (demo + Colab).`
  3. `README.md` Quick Links +1 fila tras `| Follow a tutorial | [Tutorials](docs/tutorials/) |`: `| Run runnable examples | [Demo + Colab](examples/README.md) |`
...
- `docs/plans/2026-09-15-find-correcciones.md` Task 25 — contrato y contexto verificado
