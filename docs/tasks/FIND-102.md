...
- **Objetivo:** test roto pre-existente ensucia `cargo check --tests`. Evidencia EMB-10 `docs/tasks/EMB-10.md:174`: "`tests/sdk_serialization.rs` no compila (39 errores, `QueryResult` no declarado) — pre-existente, ajeno a este task". Fix probablemente mecánico (imports/tipos).
...
- **Prohibidos (intocables):** reescritura de suite; cambios fuera del archivo salvo imports necesarios; resto del repo (`src/`, otros tests); `reparacion.bat`; `.opencode`; `Justfile`; `ocr-*`; `completions/*`; `desktop/src-tauri/Cargo.lock`; stash@{0} GOV-C4; `docs/Backlog.md`; plan file (solo recitation); `C:/Users/Eros/.vantadb*`. WIP ajeno verificado intocable (`git status`: `M .opencode`, `M completions/*` ×4, `M docs/Backlog.md`, `M docs/avance/activo/core-engine.md`, `?? reparacion.bat`, `?? docs/plans/2026-09-17-seguimiento-mvp.md`).
...
- **Evidencia previa:** `docs/tasks/EMB-10.md:174` (FIND-D: 39 errores pre-existentes).
...
- **Archivos leídos completos:** `tests/sdk_serialization.rs` (371L), `src/sdk/mod.rs` (33L), `src/sdk/types.rs` (head + re-exports), `src/sdk/types/graph.rs:1-32` (def-site `QueryResult`, vía codegraph verbatim), `docs/tasks/EMB-10.md:160-187` (evidencia FIND-D + spec gates), `SPEC.md` (head 60L), `docs/tasks/FIND-101.md` (patrón task file), plan file `docs/plans/2026-09-17-seguimiento-mvp.md` §Task 7 + recitations.
...
- **OCR delegation:** ✅ ejecutado `pwsh dev-tools/ocr-review.ps1 -Format json` — `docs/tasks/FIND-102.md` sale como `excluded_files/unsupported_ext` (.md advisory, sin regla aplicable); único `rule group default` cubre WIP ajeno `completions/*` (no del slice, no commitear). 0 Critical/High del slice → no bloquea commit.
...
- NOTICED BUT NOT TOUCHING: WIP ajeno en `git status` (`.opencode`, `completions/*`, `docs/Backlog.md`, `docs/avance/activo/core-engine.md`, `reparacion.bat`, plan file untracked) → no incluir en staging (staging selectivo solo `docs/tasks/FIND-102.md` + recitation del plan).
