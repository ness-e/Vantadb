...
**Plan:** `docs/plans/2026-09-08-backlog.md` → Task 10 · **Estado:** ⏳ IN PROGRESS
...
| 4 | Posts 6-7 = solo drafts `docs/blog/` | NO añadir a `BLOG_POSTS` web (eso = publicar) | Plan: "solo publicación final es humana — los drafts quedan en repo" |
...
- **Leídos completos:** `web/src/app/blog/page.tsx` (101L), `[slug]/page.tsx` (174L), `[slug]/layout.tsx` (40L), `blog/layout.tsx` (22L), `vanta-data.ts:878-1035` (BLOG_POSTS 4 posts), `sitemap.ts` (blogSlugs 4), `dictionaries.ts` (90 keys blogPost.*), `language-provider` + `i18n-utils.ts` (tt=key→dict??fallback), `docs/blog/how_hybrid_search_works.md` (156L), `sqlite_for_ai_agents.md` (117L), `why_i_built.md` + `introducing_vantadb.md` (frontmatter), `BLOG_SERIES_PLAN.md` (211L), `BENCHMARKS.md` §6/§14, `docs/api/MCP.md` (server cmd), `docs/tasks/MKT-18i.md` (stack Ollama).
...
- **Veredicto:** edición aditiva + frontmatter; sin cambio de rutas/slugs live; sin tocar `.opencode`, `opencode.jsonc`, `docs/Backlog.md` (WIP ajeno). Riesgo bajo, reversible por archivo.
...
- Posts 6-7: `docs/blog/ollama_vantadb_local_memory.md` (3851B) + `claude_code_mcp_memory.md` (3403B), ambos `draft:true` ✅
...
- `docs/blog/campaign-ai-agent-memory.md` repite "QPS 750→1,195" (doc interna de campaña, no publicada) → notar, no tocar.
...
- WIP ajeno: `M .opencode`, `M docs/Backlog.md`, `M opencode.jsonc`, `?? Investigacion-plan.md`, `?? docs/plans/2026-09-08-backlog.md` → NO tocar ni stagear.
