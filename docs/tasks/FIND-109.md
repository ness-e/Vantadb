...
## Metadata
- **Plan file:** `docs/plans/2026-09-17-seguimiento-mvp.md`
...
- **Archivos:** `src/error.rs` (hint `Wal` → salvage + test `recovery_hint_for_wal_points_at_salvage`), `docs/operations/CONFIGURATION.md` (fila `wal salvage`, 0 gaps), este task file (evidencia MCP); commit selectivo (NO PUSH)
...
- **Problemas conocidos:** ninguno pendiente. `campaign_verify_cmd` bug exit -1 → se usó bash directa (mencionado). `campaign_get_next_task` sin task registrada → ejecución file-based. Staging selectivo: fuera quedan `.opencode`, `completions/*` (regenerado por build + WIP previo), `docs/Backlog.md` (1 línea ajena), `reparacion.bat`, plan file.
