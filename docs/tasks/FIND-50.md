...
> **Plan:** docs/plans/2026-09-08-backlog.md (Task 3, Wave0)
...
- Estado git al inicio: `M .opencode`, `M opencode.jsonc`, `?? docs/plans/2026-09-08-backlog.md` — FUERA de scope, no commitear.
...
- Commit pathspec solo: `src/parser/mod.rs src/parser/lexer.rs src/parser/grammar.rs docs/tasks/FIND-50.md` (plan file queda sin commitear — ownership lead).
...
- **Resume:** cuando Wave0 estabilice lib → `cargo test -p vantadb --test parser` + `cargo insta test --check` (0 `.snap.new`) + `cargo clippy -p vantadb -- -D warnings` + commit pathspec solo `src/parser/* docs/tasks/FIND-50.md`.
