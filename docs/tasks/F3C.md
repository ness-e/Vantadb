...
> **Plan:** `docs/plans/2026-09-13-cleanCA-fase3.md` · **Wave:** 2 (tras F3X ✅) · **Ruta:** vanta-arch (diseño + ADR-datos) → vanta-worker (F3C-impl por slices)
...
> **Research base:** `docs/tasks/C2D0.md` (E1–E3 + Duda) — este archivo actualiza SOLO lo que cambió desde D0.
...
**C7 — unificación breaking (B+B, mismo cambio):** 12 `VANTA_*` → `VANTADB_*` (lista H2), **sin shims/aliases** (dolor único, decisión Q4). Documentación del breaking **sin tocar `docs/CHANGELOG.md` a mano** (Regla 7: lo genera release-plz): commit `feat!:` + footer `BREAKING CHANGE:` + sección en `docs/operations/CONFIGURATION.md` (tabla legacy→nuevo + nota de migración) — el changelog nace del commit, no de edición manual. Semver: major (`feat!:`).
...
- **Objetivo:** F3C diseño C0 ✅ en `docs/tasks/F3C.md` — cero código, cero commits.
