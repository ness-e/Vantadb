...
> **Plan:** `docs/plans/2026-09-21-workflows-repair.md` Wave 1 (paralelo ×3 disjunto: 137/138/139; este task solo toca sus 3 archivos).
...
- **Plan file:** `docs/plans/2026-09-21-workflows-repair.md`
...
- **Invariantes a preservar:** resto workflows intactos (FIND-138 diagnostica el flake "Generate API reference" sobre el survivor vivo; FIND-139 `ci-gate`/`gate-docs-21` paralelos); `src/`, `web/`, `desktop/`, `reparacion.bat`, `.opencode/`, `completions/*`, `*.lock`, stash GOV-C4, `docs/Backlog.md`, plan file (solo recitation server) intactos; `C:/Users/Eros/.vantadb*` y secretos ni leídos; NO PUSH (pushea solo vanta-lead).
- **Comandos de verificación:** `actionlint .github/workflows/ci-rustdoc.yml` (exit 0) · `git diff --check` (limpio) · `rg -n "rustdoc-70" .github/ README.md README_ES.md docs/workflow/ docs/operations/` (0 hits) · `git status --short` (solo 3 paths: survivor M + eliminado D + task file)
...
- **Archivos:** `docs/tasks/FIND-137.md` (NUEVO)
...
rg -n "rustdoc-70" .github/ README.md README_ES.md docs/workflow/ docs/operations/  # 0 hits (contrato b)
