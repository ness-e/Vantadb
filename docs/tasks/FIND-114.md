...
## Metadata
- **Plan file:** docs/plans/2026-09-17-seguimiento-mvp.md (Wave2, Task 8)
...
- **PROHIBIDOS (no tocar):** cambiar comportamiento del ejemplo; resto de `examples/` (otros ejemplos legacy como `demo.py`, `autogen_memory.py`, `smoke_test_extended.py` quedan intactos — scope discipline); `vantadb-python/src/` (bindings — solo lectura); `reparacion.bat`; `.opencode`; `Justfile`; `ocr-*`; `completions/*`; `desktop/src-tauri/Cargo.lock`; `stash@{0}` GOV-C4; `docs/Backlog.md`; plan file (solo recitation); `C:/Users/Eros/.vantadb*`.
...
- **Archivos:** `docs/tasks/FIND-114.md` (este file)
...
- **Archivos:** `docs/tasks/FIND-114.md` (sync), plan file (recitation FIND-114)
