...
Blast radius = 1 README (edición) + 1 package.json (solo lectura) + `docs/reviews/` (solo búsqueda).
...
- **Archivos leídos completos:** `vantadb-node/README.md` (79 líneas, entero),
  ...
  `docs/reviews/README.md:40-60` (índice, solo lectura); `docs/tasks/TS-12.md:1-15` (existe);
  `docs/api/NODE_SDK.md:1-10` (existe); `docs/reviews/archive/research-vantadb-node-20260825.md:1-15` (existe, contenido correcto).
- **Referencias hacia dentro (lo que el README cita):** `https://napi.rs` (externo),
  ...
  ``docs/tasks/TS-12.md`` (bare, resuelve a raíz `docs/tasks/TS-12.md` True; relativo a `vantadb-node/` False — code span, no link MD),
  ``docs/reviews/research-vantadb-node-20260825.md`` (bare, raíz False → **roto**; reubicado a `docs/reviews/archive/` True).
...
- Review **SÍ existe en otra ruta**: `docs/reviews/archive/research-vantadb-node-20260825.md` (True;
  ...
  `docs/reviews/README.md:46` también lo indexa (sin prefijo `archive/` — colateral fuera de scope, no se toca).
...
- Muestreo total links README: `napi.rs` (externo, no verificado red — fuera de contrato mecánico),
  `../vantadb-ts` True, `../docs/api/NODE_SDK.md` True, `docs/tasks/TS-12.md` True (raíz),
  review roto → archive True. Tras fix: 0 rotos locales.

## Steps

- [x] **Step 1 (único, ~6 líneas):** PLAN→ACT→VERIFY — `:16` ruta → `docs/reviews/archive/research-vantadb-node-20260825.md`;
  ...
  - `Test-Path docs/reviews/archive/research-vantadb-node-20260825.md` → True ✅
    (vieja ruta `docs/reviews/research-vantadb-node-20260825.md` → False, ya sin referencias en el README ✅)
  - `Test-Path docs/tasks/TS-12.md` → True ✅; `docs/api/NODE_SDK.md` → True ✅; `vantadb-ts` → True ✅
