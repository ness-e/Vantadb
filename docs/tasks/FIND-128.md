...
- **Plan file:** docs/plans/2026-09-19-cierre-total.md
...
| Callees | `git diff --name-only BASE HEAD` (two-tree diff, depth-1 fetch); `$GITHUB_OUTPUT` file-command parser del runner; regex `^docs/architecture/adr/ADR-[0-9]{3}` |
...
- **Conteo CI_POLICY:** `docs/operations/CI_POLICY.md:17` dice 26; real 28 (incluye `ci-gate.yml` + `opencode.yml` probablemente). NO se toca (cero refactors) — propuesta escrita abajo.
...
- **Archivos:** `docs/tasks/FIND-128.md`
...
- **Archivos:** `.github/workflows/ci-rust-10.yml`, `docs/tasks/FIND-128.md`
