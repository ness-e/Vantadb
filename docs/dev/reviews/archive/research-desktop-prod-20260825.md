...
| H-02 | APLICAR | 🟢 Baja | 🟢 <1h | CommandPalette desincronizada con surfaces del shell (faltaba `memoria`, anotada en DESKTOP-38; verificar union completa tras AJUSTES/PROXY) | `components/palette/CommandPalette.tsx` · `docs/dev/avance/activo/desktop.md` DESKTOP-38 |
...
| H-08 | ESTRATEGIA | 🟠 | 🔴 | Distribución: instaladores unsigned (SmartScreen avisa) + smoke-test en VM limpia nunca ejecutado (Step 3 DESKTOP-24). Smoke = APLICAR 🟢; firma de código requiere certificado (decisión negocio) | `docs/dev/avance/activo/desktop.md` DESKTOP-24 |
...
| H-13 | MEJORAR | 🟢 | 🟢 | Filas DAUD-01..09 en Backlog siguen ⬜ Pendiente pese a commits aplicados (stale — Trigger 4/1 de progreso); limpiar o actualizar | `docs/dev/Backlog.md:522-530` vs git log `3c53d8b2`,`480935a7`,`b865c625` |
...
| H-15 | OPTIMIZAR | 🟢 | 🟡 | Sin baseline medido del app (startup time, RAM idle, tamaño heap WebView) — cualquier claim de performance actual es estimación de plataforma, no medición (Regla 11) | Ausencia en `docs/user/operations/BENCHMARKS.md` |
...
- **Interno:** codegraph_explore (shell/e2e/bridge), glob inventario `desktop/src/**`, `docs/dev/avance/activo/desktop.md`, `docs/dev/Backlog.md` (P37/DAUD), `package.json`, `tauri.conf.json`, `DESIGN_DECISIONS.md`, campaign_memory lessons, git log desktop.
