...
| `FIND-148` | Media | Harness: migrar frontmatter de 11 agents a vocabulario OpenCode v2 (`bash`→`shell`, `task`→`subagent`) + descriptions trigger primera persona ("Use me when…"). Piloto `vanta-review` primero (rollback si el OpenCode instalado no mapea v2). Actualizar tabla AGENTS.md. Origen: auditoría harness 2026-09-24 (commits `dda76a4`+`ceca4d7` configOpencode) | `.opencode/agents/*.md`, `.opencode/AGENTS.md` | 🟡 | 🟡 Media | ⬜ Pendiente (plan `docs/dev/plans/2026-09-24-harness-gaps.md`) |
| `FIND-149` | Media | Harness L7: Semgrep OSS + MCP (`semgrep mcp`) + ast-grep como checks versionados (reglas: `unwrap` en prod, `unsafe` sin `SAFETY`, `expect` genérico). Empezar warn, endurecer a error tras 1 semana verde. Origen: auditoría harness 2026-09-24 | `.opencode/configs/semgrep-vanta.yml` (nuevo), `.opencode/agents/vanta-audit.md`, `opencode.jsonc` | 🟡 | 🟡 Media | ✅ Completado 2026-09-24: gate exit 0 + negativa 3+2 (falla→fix→pasa) + dictamen con output real (ver docs/dev/tasks/FIND-149.md). Endurecimiento warn→error: dueño vanta-lead, 1 semana verde |
...
| `FIND-150` | Media | Higiene dependencias + DoD v2: `cargo-llvm-cov` (70% módulos nuevos, piloto crate `vantadb`), `cargo machete`/`udeps` (0 sin justificar, Regla 6), OSV-Scanner en CI junto a `cargo audit`. Origen: auditoría harness 2026-09-24 | `deny.toml`, `.github/workflows/`, `.opencode/references/definition-of-done.md` | 🟢 | 🟡 Media | ⬜ Pendiente (plan `docs/dev/plans/2026-09-24-harness-gaps.md`) |
| `FIND-151` | Baja | Eval de agentes (Giskard OSS, 10 casos dorados, solo en `/audit full` L9) + Lurkr a obligatorio en CI host (2 semanas informativo → falla build). Origen: auditoría harness 2026-09-24 | `evals/agent/` (nuevo), `.github/workflows/`, `.opencode/commands/harness.md` | 🟡 | 🟢 Baja | ⬜ Completado 2026-09-24: eval 10/10 + lurkr-informational.yml (ver docs/dev/tasks/FIND-151.md) |
