...
- **Historia intacta (no reescribir):** `.opencode/.../SRV-07.md`, `docs/dev/avance/activo/operaciones.md:178` (deuda SRV-07), `docs/dev/plans/archive/*`, `docs/dev/reviews/archive/*` conservan sus menciones fechadas — son registro, no punteros vivos. El `rg` de cierre las reportará como hits históricos exentos.

## Impacto mapeado (Regla 0)

- **Archivos leídos completos:** `vantadb-server/Dockerfile` (112L), `vantadb-server/docker-compose.yml` (95L), `vantadb-server/docker-compose.prod.yml` (45L), `Dockerfile` raíz (89L, lectura), `docs/user/operations/hardening.md` §5 (L195-252), `docs/user/operations/DEPLOYMENT_GUIDE.md` L206-232, `vantadb-server/src/main.rs` (vía lectura parcial 1-80 + validate_args), `src/cli.rs:314-326` (lectura puntual), `src/bin/vanta-cli.rs` (rg puntual), `.github/workflows/release-binaries-63.yml` L103-184 (rg puntual), `.dockerignore`, `Cargo.toml` (workspace/members/bins), `vantadb-server/Cargo.toml`, `docs/dev/Backlog.md` L219, `docs/dev/avance/activo/operaciones.md` L162-178.
...
| hardening honesto | `rg "vantadb-server/Dockerfile" docs/user/operations/hardening.md docs/user/operations/DEPLOYMENT_GUIDE.md` | 0 |
...
- **Archivos:** `docs/user/operations/hardening.md` (§5 rewrite), `docs/user/operations/DEPLOYMENT_GUIDE.md` (L231-232)
...
### Step 3: Cierre
- **Archivos:** `docs/dev/Backlog.md` (fila FIND-56 eliminada), `docs/dev/avance/activo/operaciones.md` (entrada; dominio ops por tabla progreso — docker cae en "Ops"; alternativa ci-cd: se anota operaciones por ser hardening/deploy), memoria `decisions`, commit scoped
