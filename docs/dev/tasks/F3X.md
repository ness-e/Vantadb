...
## Metadata
- **Plan file:** docs/dev/plans/2026-09-13-cleanCA-fase3.md (Wave 1, Task 2 — diseño-primero; tras F3G)
- **Creado:** 2026-09-14
...
- **Relacionados (solo lectura):**
  - `docs/dev/architecture/BOUNDARIES.md` BND-02/BND-03 + §3 (tabla del ciclo + plan opción 1/2) + BND-04 (re-exports solo compat)
  - `docs/dev/tasks/C2M3.md` (precedente: hoja `src/search_profile.rs` std+serde-only + inversión fns→`sdk/search/fusion.rs`; criterio Box/re-export)
  - `docs/dev/tasks/F3G.md` (base: commit `dd892c4c`, gate 4/4, firma A1 árbol `5a1dae99`, tabla D)
...
## 4. REFERENCIAS
- M3 (`docs/dev/tasks/C2M3.md`): inversión fns→sdk + hoja `search_profile.rs` (tipos+consts, `pub use` BND-04 solo compat, participantes a ruta neutral).
...
- `Write` del task file: este file (`docs/dev/tasks/F3X.md`) — único artefacto de escritura de la tarea.
...
- **Decisión:** PENDIENTE HUMANO (question abajo; el humano la articula con sus palabras en el ADR final `docs/dev/architecture/adr/NNN_*.md`).
...
- **Referencias hacia dentro (nuevas):** `docs/dev/tasks/F3X.md` (este file; sin entrantes aún — F3X-impl lo consume).
...
- **Archivos (lectura):** §2 clave+relacionados. **Archivos (escritura):** `docs/dev/tasks/F3X.md` (este file).
...
ARCHIVOS: docs/dev/tasks/F3X.md
