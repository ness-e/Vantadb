...
## Metadata
- **Plan file:** `docs/dev/plans/2026-09-03-quality-gtm-wave.md` (Wave0 — Task 3)
- **Creado:** 2026-09-03
...
- **Referencias entrantes (quién cita `vantadb-examples`):**
  - `rg -n "vantadb-examples" docs/` → solo 3 hits no-clone: Backlog fila 436 (tracking), plan Task 3 (contrato/meta), archivo `docs/dev/plans/archive/2026-08-28-backlog-triage.md:633` (histórico). Ningún otro doc usuario (`docs/README*`, `docs/api/*`, `docs/user/operations/*`) referencia el repo → el único paso roto de cara al piloto es checklist:51.
...
- Tras fijar: `rg -n "vantadb/vantadb-examples" docs/user/operations/ docs/api/ docs/user/QUICKSTART.md docs/README*` == 0 (cero URLs clone muertas en docs de cara al usuario; plan/backlog/archive son tracking histórico, no pasos del piloto).
...
- **Archivos:** `docs/dev/plans/2026-09-03-quality-gtm-wave.md` (Task 3 → ✅), `docs/dev/Backlog.md` (fila 436 se ELIMINA vía skill progreso Trigger 1 — con eso muere también la cita a la ruta vieja)
- **Acción:** commit `docs(gov): verificar URL vantadb-examples (GOV-TK9)` solo con archivos de esta tarea; skill progreso (registro en `docs/dev/avance/activo/operaciones.md`)
- **Verify:** `git log --oneline -1` muestra el commit; `rg -n "GOV-TK9" docs/dev/Backlog.md` == 0
