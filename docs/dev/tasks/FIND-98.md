...
> **Plan:** `docs/dev/plans/2026-09-18-cierre-mvp.md` (Wave0) → retry por `docs/dev/plans/2026-09-19-publicacion.md` Task 1 (alcance corregido por smoke: rebuild con `embed-local`, no basta copiar)
...
**Prohibidos (respetados — `git status` verificado §6, staging selectivo solo este file):** forzar unlock matando procesos ajenos · `reparacion.bat` · `.opencode` · `Justfile` (inexistente, no tocado) · `ocr-*` · `completions/*` · `desktop/src-tauri/Cargo.lock` · `stash@{0}` GOV-C4 · `docs/dev/Backlog.md` (fila 98 NO tocada — re-DEFER vía orquestador) · plan file (sin recitation propia — el handoff es el RESULTADO) · `C:/Users/Eros/.vantadb*` (smokes solo en DB temporal del script) · files de FIND-110-spec/113-spec (paralelas de wave, disjuntas).
...
- **Wave0 primera en secuencia** (plan §Grafo): FIND-110-spec + FIND-113-spec después por rate-limit; archivos disjuntos (fuera-del-repo · docs/dev/tasks/110 · docs/dev/tasks/113) — esta STOP no las bloquea.
...
| `git diff --check` + `git status --short` (bash) | OK; WIP ajeno intacto (`.opencode`, `SPEC.md`, `completions/*`, `docs/dev/Backlog.md`, `reparacion.bat`, plan untracked) |
...
ARCHIVOS: docs/dev/tasks/FIND-98.md
...
- **Flags exactos EMB-10:** `cargo build --bin vanta-cli --features embed-local,remote-inference -j 2` (`docs/dev/tasks/EMB-10.md:16`, comando reverificado en su DISCOVERY). **Hallazgo:** el comando literal del plan (`--bin vanta-cli --bin vantadb-server` sin `-p`) NO compila en este workspace — `vantadb-server` es crate aparte (`vantadb-server/src/main.rs`) y no reenvía `embed-local` (su `[features]` no lo tiene; `vantadb-mcp` sí: `embed-local = ["vantadb/embed-local"]` en `vantadb-mcp/Cargo.toml`). Comando corregido equivalente (misma intención, unificación de features por invocación):
