...
- README raíz — NO se modifica. La sección "Quick Links" ya apunta a docs/user/QUICKSTART.md y docs/api/MCP.md. La integración del registry se documenta en `docs/user/operations/MCP_REGISTRY.md` (nuevo) y se linkea desde `docs/api/MCP.md` (doc canónico del MCP).
- Docs `docs/api/MCP.md` — sí, se referencia brevemente el manifest (nueva sub-sección).
- `docs/user/operations/INDEX.md` (si existe) — se añade entrada para el nuevo doc.

### Veredicto de impacto

- **blast radius:** NINGUNO en código. Dos archivos nuevos (`server.json`, `docs/user/operations/MCP_REGISTRY.md`) y un par de referencias en docs/API.
...
- **pre-mortem (riesgo explícito en task description):**
  - **Fallo 1:** registry submission requiere approval manual → documentar PR/submission state en `docs/user/operations/MCP_REGISTRY.md` (sección "Submission state").
...
- **Archivos:** `docs/user/operations/MCP_REGISTRY.md` (nuevo)
...
- **Acción:** `git add server.json docs/user/operations/MCP_REGISTRY.md docs/api/MCP.md`. NO ejecutar commit — vanta-worker deja staged y reporta. vanta-lead ejecuta `git commit -m "docs: MCP-40 — Registry manifest + ecosystem listings"`.
...
| `docs/user/operations/MCP_REGISTRY.md` secciones `##` | 8 (≥ 5) | ✅ |
| `docs/api/MCP.md` match `server.json` | 3 hits (≥ 1) | ✅ |
| `docs/user/operations/master-index.md` match `MCP_REGISTRY` | 1 hit (GOV-C5) | ✅ |
