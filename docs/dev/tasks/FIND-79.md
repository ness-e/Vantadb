...
- Incidente `git stash pop` (mid-task): el worktree es compartido con agentes Wave2 paralelos + 15 stashes históricos (incl. GOV-C4 prohibido). Mi `stash push` falló por pathspec y el `pop` incondicional intentó popear `stash@{0}` ajeno → conflicto en `docs/dev/plans/archive/2026-09-03-quality-gtm-wave.md` (estaba limpio). Recuperado con `git restore --source=HEAD` (solo ese archivo); stash ajeno intacto (15 entradas verificadas). **Lección:** jamás `git stash` en repo compartido con agentes paralelos — RED-proof por Edit round-trip en su lugar (lesson registrada).
