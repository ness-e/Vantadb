...
Crear `docs/workflow/RULES.md` (nuevo) + refresh `docs/user/operations/CI_POLICY.md` (conteo/triggers) + 1 fila puntero en `AGENTS.md` raíz. Contrato: lint verdes; una regla verificable por cada hallazgo Alta; 0 links rotos.

## Contrato (ley)

- `npx markdownlint-cli2 "docs/workflow/**/*.md" "docs/user/operations/CI_POLICY.md"` → 0 issues.
...
- `docs/user/operations/CI_POLICY.md` (479L entero) — §1 Fast Gate `ci-rust.yml`, §§3-10 triggers, línea 17 conteo `26` stale (2026-09-15).
...
- [x] **S5 VERIFY** — `npx markdownlint-cli2 "docs/workflow/**/*.md" "docs/user/operations/CI_POLICY.md"` + `git diff --check` + grep old-names + grep links. ✅ lint 0/22, diff-check limpio, old-names 0, links resuelven. Retry ladder; 2 fallas mismo-error → Gate V.
