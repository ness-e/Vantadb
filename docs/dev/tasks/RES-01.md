...
## Metadata
- **Plan file:** `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` (Wave0 SOLO — 20260902-alta-prioridad-paralelo)
- **Plan histórico:** `docs/dev/plans/2026-08-29-full-backlog-parallel.md` (W3-SOLO, ✅ COMPLETED 2026-08-29 — WAL v2 Phase 4a landed)
- **Fuente:** `docs/dev/Backlog.md` P38 RES-01..15 (RES-01 prerequisite durabilidad 🔴) + `docs/dev/research/archive/res02-backup-restore.md` §2-3 S1-S2 + `src/wal.rs` + `src/storage/engine/mod.rs:507,540` + FIND-26 (wal_archiver eliminado 2026-08-25)
...
- `Read` — `src/wal.rs`, `src/storage/engine/mod.rs`, `docs/dev/research/archive/res02-backup-restore.md` §2-3
...
N/A — durabilidad infra, sin símbolos públicos nuevos en este wave. Decisión técnica ya tomada en `docs/dev/research/ACID_ROLLBACK_DESIGN.md` (recuperado b85b52b3): Prepare {txn_id, op_count} + WAL_FORMAT_VERSION=2 + range-based header compat + two-phase commit (Begin→Prepare→fsync→apply→Abort-on-fail→Commit). Si S1 requiere ajuste wal/ subdir, añadir doc en `docs/user/operations/DISASTER_RECOVERY_RUNBOOK.md` mismo PR (Regla 3).
...
- **Archivos:** `src/wal.rs` (1328L), `src/storage/engine/mod.rs` (829L), `src/storage/engine/maintenance.rs:flush()`, `docs/dev/research/archive/res02-backup-restore.md` §2-3, `src/storage/vfile*.rs`, `src/wal_sharded.rs`
...
- **Archivos:** `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` (RES-01 §137-145), este task file
...
- **Verify:** `campaign_verify_cmd` del contrato + `Select-String -Path "docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md" -Pattern "RES-01.*⏳|RES-01.*✅" | Measure-Object Count` >=1
- **Estado:** ⬜ PENDING
...
- **No tocar MCP-35/GOV-T01:** Wave0 disjoint — respetar DAG. Solo `src/wal.rs`, `src/storage/engine/**`, `src/storage/vfile*`, `docs/dev/research/res02-*`.
...
- `docs/dev/research/archive/res02-backup-restore.md` (57L): gaps flat copy + no quiesce (S1), plan atomic S1-S5, recomendación (a) directory swap RECOMMENDED, (b) PITR DEFER, (c) logical EXISTS.
