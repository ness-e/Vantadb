...
- **Fuente:** docs/dev/Backlog.md línea 119 — "Benchmarks internos existen, sin script público standalone" / audit backlog-validation-2026-07-28 línea 105: "⚠️ Scripts existen pero requieren build local; no standalone"
- **Esfuerzo:** 🟡 1d
- **Prioridad:** 🟠
...
- `docs/user/operations/BENCHMARKS.md` sección 3 ("Reproducing the Benchmark Locally") solo documenta `maturin develop` (líneas 68-74). CI `perf-bench-40.yml` ya corre `vantadb_local_bench.py` y actualiza la tabla vía `update_markdown.py`.
...
| Dirección | Módulos |
|-----------|---------|
| Callers | `docs/user/operations/BENCHMARKS.md` (sección reproduce), CI `perf-bench-40.yml`, README badge (workflow perf-bench-40) |
...
## Contrato
"`pip install -r benchmarks/requirements.txt` en un venv limpio instala `vantadb-py` desde PyPI; `python benchmarks/vantadb_local_bench.py --size 1000 --queries 50 --output <tmp>.json` corre y emite JSON con insert/rebuild/query_text/query_vector/query_hybrid; `docs/user/operations/BENCHMARKS.md` documenta el path standalone público (pip install, sin maturin); los 3 scripts muestran hint `pip install vantadb-py` en lugar de 'maturin develop'."
...
- `update_markdown.py` ya auto-actualiza `docs/user/operations/BENCHMARKS.md` (markers `BENCHMARK_METRICS_START/END`) — no necesita cambios.
...
- **Branch:** develop
- **CI pendiente:** no
- **Decisiones:** path público = `pip install -r benchmarks/requirements.txt` + hint en scripts; dev local (maturin) se mantiene como alternativa. No crear BENCHMARKS.md en raíz — `docs/user/operations/BENCHMARKS.md` es la fuente canónica.
...
- ✅ `docs/user/operations/BENCHMARKS.md` sección 3 documenta el path pip install (3a standalone) antes del path maturin (3b).
