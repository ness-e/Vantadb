...
## Metadata
- **Plan file:** docs/dev/plans/2026-08-18-vanta-memory.md (NO editar — lead)
...
- **Reportes:** `docs/dev/research/tdam/01-core-pipeline.md` (§62-63 contratos L1), `02-scene-persona.md` (§33 SceneSegment, §34-37 strategy/heat, §41 triggers persona, §52-53 META/índice), `SYNTHESIS.md` (§2.4 qué NO copiar)
...
- **Archivos leídos completos:** `vanta-memory/Cargo.toml`, `vanta-memory/src/lib.rs`, `vanta-memory/src/{core,adapters,offload}/mod.rs`, `vanta-memory/tests/smoke.rs`, `docs/dev/plans/2026-08-18-vanta-memory.md` (secciones plan/principios/tasks), `Cargo.toml` raíz (workspace.lints, members, workspace.package), `src/llm.rs` (grep: reqwest blocking pattern), TDAM refs listadas arriba.
...
- **Referencias hacia fuera (de lo que dependo):** `serde`/`serde_json`/`thiserror`/`tracing` ya en Cargo.toml (MEM-08a); `reqwest 0.12 blocking` patrón del workspace (Cargo.toml:50, src/llm.rs:15) — dep opcional SOLO bajo feature `llm-driver`. No toco `docs/dev/plans/`.
