...
## Contrato
`Select-String -Path "docs/user/operations/DISASTER_RECOVERY_RUNBOOK.md" -Pattern "restore --dry-run|doctor --fix" | Measure-Object Count` ==0
...
- **Comandos de verificación:** `Select-String -Path "docs/user/operations/DISASTER_RECOVERY_RUNBOOK.md" -Pattern "restore --dry-run|doctor --fix" | Measure-Object Count` ==0 ; `Select-String -Path "docs/user/operations/DISASTER_RECOVERY_RUNBOOK.md" -Pattern "restore --input" | Measure-Object Count` >=1 ; `cargo check -p vantadb`
...
## Investigation Notes
- **Plan file GOV-B2:** docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md:418-425 — descripción reescribir runbook a CLI real (restore --input/--force/--rebuild, sin --dry-run/doctor --fix) + §3.1 Daily Backup Verification con restore temp+doctor+conteo insumo GOV-A3, contrato Select-String ghost 0, estado ✅ COMPLETED pero sin task file físico (gap que este pipeline cierra)
...
- **Disjoint Wave2:** GOV-B2 toca docs/user/operations/DISASTER_RECOVERY_RUNBOOK.md; MEM-01 toca src/planner.rs + sdk/serialization + sdk/search; RES-02 toca src/wal.rs + storage/engine — 0 archivos en común → parallel 3 seguro; instrucción Disjoint con MEM-01 respetada.
...
- **Archivos:** `docs/user/operations/DISASTER_RECOVERY_RUNBOOK.md:146,242`, `.opencode/skills/campaign-executor/tasks/GOV-B2.md` (este file)
...
- **Archivos:** `.opencode/skills/campaign-executor/tasks/GOV-B2.md` (este file), `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` §GOV-B2
