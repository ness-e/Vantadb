...
| Dirección | Módulos |
|-----------|---------|
| Callers | `src/bin/vanta-cli.rs` (thin entry → `cli_handlers::cmd_doctor/cmd_backup/cmd_restore`), `docs/user/operations/DISASTER_RECOVERY_RUNBOOK.md` (consume CLI real, alimenta GOV-B2 §3.1), `dev-tools/verify.ps1` (gate), `tests/cli_tests.rs` (`test_backup_and_restore`, `test_doctor_*`) |
...
- **Archivos que referencian a los editados:** grep `vanta-cli|cmd_doctor|cmd_backup|cmd_restore|VantaEmbedded::restore_from` → src/bin/vanta-cli.rs, src/cli_handlers/*.rs, tests/cli_tests.rs, docs/user/operations/DISASTER_RECOVERY_RUNBOOK.md (GOV-B2), docs/user/operations/CONFIGURATION.md (VANTA_DB env), dev-tools/verify.ps1 (será probe)
...
- `Select-String -Path docs/dev/plans/... -Pattern "GOV-A3.*COMPLETED"`
...
- **Disjoint garantía:** Wave1 1b MAX 3 — GOV-A3 (src/cli.rs, dev-tools/verify.ps1 probe) vs GOV-A4 (dev-tools/validate_doc_snippets.py, docs/user/tutorials/) vs RES-02 (src/storage/engine/mod.rs, src/wal.rs, tests/fjall_cold_copy_restore.rs) — 0 archivos en común → parallel 3 sin contención.
...
- **Archivos:** `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` §GOV-A3, `.opencode/skills/campaign-executor/tasks/GOV-A3.md` (este file), `git`
...
- **Verify:** `Test-Path .opencode/skills/campaign-executor/tasks/GOV-A3.md` + `Select-String -Path "docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md" -Pattern "GOV-A3.*COMPLETED"` >=1 + `git log --oneline -1 | Select-String "GOV-A3"` >=1 + `git status --short` clean
...
- `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` — Wave1 GOV-A3 fila
