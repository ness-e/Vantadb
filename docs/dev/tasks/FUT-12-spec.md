...
> **Plan:** `docs/dev/plans/2026-09-04-durability-release-readiness.md` Task 9, Wave 2 · **Ruta:** vanta-arch · **Branch:** develop
...
- **Leídos:** `src/wal.rs:305-394` (`append`, `batch_append`, `maybe_sync` Always|Never|Periodic, `sync` = flush+sync_data) · `src/wal_sharded.rs:198-235` (`batch_append` agrupa por shard, 1 lock+1 write_all+1 maybe_sync por shard, move sin clones) · `src/config.rs:85-103` (`SyncMode` Always/Periodic-default/Never) · `benches/wal_throughput.rs` (rig 3 SyncMode × 4 batch sizes, base de medición) · `ADR-022` (consolida DRV-014/015; Phase 1 group-commit **no implementado**) · `DRV-014` (batch-append 3-5× tradeoff) · `DRV-015` (roadmap Phase 1 group-commit + Phase 2 io_uring, riesgos R1/R2/R5) · `docs/dev/_templates/adr.md`.
...
- [x] **S2 — EJECUCIÓN:** redactar `docs/dev/architecture/adr/ADR-038-wal-fsync-batching-opt-in.md` (objetivo/diseño/aceptación/límites).
