...
- **Invariantes:** cero cambios en `desktop/src/**` (lógica de la app intacta); vitest 68/68; `npm run build` OK; NO commit (lead commitea solo los archivos de esta tarea: `desktop/package.json`, `desktop/package-lock.json`, `desktop/vitest.config.ts`, `desktop/.gitignore`, `desktop/e2e/*`, `desktop/playwright.config.ts`, `.opencode/skills/campaign-executor/tasks/E2E-VISUAL.md`, `docs/dev/Backlog.md` [solo la fila FIND-23]).
