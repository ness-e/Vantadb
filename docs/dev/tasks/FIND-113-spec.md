...
- **Plan file:** `docs/dev/plans/2026-09-18-cierre-mvp.md` (Task 3, Wave0 tercera en secuencia)
- **Fuente:** `docs/dev/tasks/FIND-113.md` (re-DEFER fundado 2026-09-18: 0 productor, daemon prohibido, MEM-65 intacto) + Gate Justificación del plan (espejo FIND-110-spec; cierra el par S4/S6b en diseño)
...
**Contrato (plan Task 3):** `docs/dev/tasks/FIND-113-spec.md` (este archivo) con (a) dueño del backend (dir estado, quién reclama locks TTL, scope-proceso), (b) superficie mínima (`scheduler_status`/`run_once` o equivalente), (c) gates+tests futuros, (d) coherencia con FIND-112 §(c) escrita + P2-01-spec; **cero código**.
...
- `docs/dev/tasks/FIND-113-spec.md` (ESTE archivo — ES el entregable)

**Lectura (leídos completos en DISCOVERY, verbo vía `codegraph_explore` + `Read`):**

- `docs/dev/tasks/FIND-113.md` (665L) — re-DEFER fundado con 6 evidencias + Spec Phase 1b + motivo Backlog; base de todo el diseño (no re-derivado, heredado)
...
- `docs/dev/tasks/FIND-112.md` §(c) (líneas 209-240, lectura SIN editar) — dueño runner por-llamada + 5 puntos lifecycle; referencia de coherencia, no de edición
- `docs/dev/tasks/FIND-110-spec.md` (339L, commit `89b118be`) — hermana S4, coherencia de estilo (slices A+B, tests nombrados, gates G0–G3, condición de ship, restart documentado)
...
- `docs/dev/plans/2026-09-18-cierre-mvp.md` Task 3 (solo recitation/contrato — el plan file no se edita desde la tarea)
...
- **Wave0 tercera en secuencia.** FIND-98 ✅ (re-DEFER con evidencia, commit `4187eabf`), FIND-110-spec ✅ (spec 339L, commit `89b118be`) — archivos disjuntos (fuera-del-repo · `docs/dev/tasks/110` · `docs/dev/tasks/113`), sin interferencia.
...
- `lastAction`: DISCOVERY (FIND-113 + worker/backend/locks/MEM-65/dispatch/tipos + timer_scanner pull-based + FIND-112 §c + FIND-110-spec hermana + rules core-engine + SPEC.md + codegraph 91 símbolos + diff-check) + spec escrita en `docs/dev/tasks/FIND-113-spec.md` (dueño writer-side `Arc`, pull-based sin daemon, restart efímero documentado, superficie 2 tools, slices A+B+C, S1–S7, G0–G3, coherencia-112 escrita) + commit `docs:` solo-task-file
...
- **Archivos:** `docs/dev/tasks/FIND-113.md`, `pipeline_worker.rs` (§§1-130, 198-335, 540-565, 804-820), `local_backend.rs` (§§1-70, 228-392), `types.rs` (`:38-70`), `timer_scanner.rs`, `FIND-112.md` §(c), `FIND-110-spec.md`, rules `core-engine.md`, `definition-of-done.md`, `SPEC.md` §cierre-mvp
...
- **Archivos:** `docs/dev/tasks/FIND-113-spec.md` (nuevo, único permitido)
...
- **Archivos:** `docs/dev/tasks/FIND-113-spec.md` (único en staging)
- **Acción:** `git diff --check` + `git status` (solo-task-file) + `git add docs/dev/tasks/FIND-113-spec.md` + `git commit -m "docs: FIND-113-spec — ..."` (NO PUSH) + `campaign_update_task_state` completed + `campaign_memory_write` + bloque RESULTADO §7
...
- NOTICED BUT NOT TOUCHING: WIP ajeno en `git status` (`.opencode`, `SPEC.md`, `completions/*`, `docs/dev/Backlog.md` modificados; `reparacion.bat` y plan file untracked) — fuera de scope, no se tocan ni se stagean. `docs/dev/Backlog.md` fila FIND-113 y plan recitation los actualiza el orquestador/lead.
...
- `git status --short` — ✅ solo `docs/dev/tasks/FIND-113-spec.md` en staging al commitear; WIP ajeno intacto
...
- **Scope discipline:** 1 archivo nuevo (`docs/dev/tasks/FIND-113-spec.md`), 0 ediciones. WIP ajeno intacto.
