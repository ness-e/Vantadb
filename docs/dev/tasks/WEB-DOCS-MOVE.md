...
- **Set a mover (13 archivos, existencia verificada):**
  ...
  3. `docs/dev/plans/archive/2026-08-19-web-design-audit.md` + `.budget.json` (mismo nivel ✅)
  4. `docs/dev/plans/archive/2026-08-04-launch-web-campaign.md` + `.budget.json` (mismo nivel ✅)
  5. `docs/dev/plans/archive/2026-08-25-research-web-quickwins.md` (CORRECCIÓN: el .md vive en
     `archive/`, NO en `docs/dev/plans/` como decía el brief) + `docs/dev/plans/2026-08-25-research-web-quickwins.budget.json` (al mismo nivel NO — está en `docs/dev/plans/`, el .md en `archive/`)
  6. `docs/dev/research/archive/INV-005-error-boundary-web.md` (0 refs en repo ✅)
  7. `docs/user/glosario/web-to-vantadb-mapping.md` (0 refs en repo ✅)
  8. `docs/dev/avance/historial/campanas/detalle-inv-web-seo.md` + `web-docs-mcp-tooling.md` + `web-launch-campana.md`
- **Referencias entrantes:** 0 markdown links `](...)` vivos hacia el set (grep exhaustivo
  ...
  `docs/dev/avance/historial/fuentes/README.md:127,135,139` — archivo congelado, eximido por contrato (b).
...
- [x] **STEP 3 — GIT RM + refs:** `git rm` 12 trackeados + 4 filas en 3 índices
  ...
  Desviación: `docs/dev/plans/2026-08-25-research-web-quickwins.budget.json` es gitignored+untracked
