...
Cada celda comparativa de `web/src/components/vanta/vs-table.tsx` con fuente o calificada honestamente (Regla 11: claims numéricos DEBEN salir de `docs/user/operations/BENCHMARKS.md`; web AGENTS.md). `vantadb:"1.2ms"` OK (fuente `docs/user/operations/BENCHMARKS.md:34` p50 HNSW 10K); 5 celdas competidoras sin cita (`pinecone:"~50-150ms"`, `weaviate:"~20-80ms"`, `chroma:"~5-30ms"`, `"$1,800/mo"`, `"$600/mo"`).
...
- **Relacionados:** `docs/user/operations/BENCHMARKS.md` (fuente de verdad, §1 línea 34 p50 10K = 1.2ms); `web/src/lib/dictionaries.ts:1465-1482,2954-2971` (solo claves `vsTable.*` de labels — valores de celdas hardcodeados en el tsx, no requieren cambio de dict).
- **Prohibidos (NO tocar):** rediseñar la tabla o la página; `reparacion.bat`; `.opencode`; `Justfile`; `ocr-*`; `completions/*`; `desktop/src-tauri/Cargo.lock`; stash@{0} GOV-C4; `docs/dev/Backlog.md`; plan file (solo recitation al cierre); `C:/Users/Eros/.vantadb*` (datos vivos); `src/`; `desktop/` (FIND-124 ✅); resto de `web/` fuera del archivo.
...
- **Archivos leídos completos:** `web/src/components/vanta/vs-table.tsx` (199 líneas); `docs/user/operations/BENCHMARKS.md` (§1-§2, fuente línea 34); `docs/dev/plans/2026-09-19-ci-green.md` (Task 2 §67-82); `.opencode/rules/frontend-web.md` (completa); `.opencode/references/definition-of-done.md` (completa); `web/src/lib/dictionaries.ts` (grep `vsTable`, 36 matches — solo labels).
