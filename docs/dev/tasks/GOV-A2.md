...
- **Acción:** Leer dora.md (402L) + dora.mjs (360L recoveryPairs) + Cargo.toml (workspace 0.5.0) + validate-docs-coverage.ps1 (197L/6 checks) + nextest.toml (default-filter BND-06) + TEST_MAP.md (155L Coverage+CI Gates) + grep SKILLS-MANIFEST.md por keywords "tests/coverage/validate-docs/dora/nextest" → discovery skills. Mapear blast radius: callers docs/dev/Backlog, callees TEST_MAP/validate-scripts/nextest.toml/.codegraph. Confirmar no overlap RES-02..05 ni GOV-A3.
...
- **Archivos:** `.opencode/skills/campaign-executor/tasks/GOV-A2.md` (este file), `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` §GOV-A2
...
- **Verify:** `Test-Path .opencode/skills/campaign-executor/tasks/GOV-A2.md` == true AND `Select-String -Path "docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md" -Pattern "GOV-A2.*COMPLETED" | Measure-Object Count` >=1
...
- `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` — Wave1 GOV-A2 fila
