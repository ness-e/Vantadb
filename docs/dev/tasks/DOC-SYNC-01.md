...
- **Scope:** SOLO `docs/dev/Backlog.md` (2 textos). NO tocar plan file, ni archivos de STABLE-05/GOV-TK2.

## Contrato (ley)

- Header con conteo real de filas activas (contado por comando, con fecha de medición)
- STABLE-06 dice 280 tests
- `git diff --stat` muestra SOLO `docs/dev/Backlog.md` (en lo tocado por esta tarea)
...
- **Archivos leídos completos:** `docs/dev/Backlog.md:1-80` (header) + `docs/dev/Backlog.md:600-629` (fila STABLE-06) + `docs/dev/tasks/STABLE-06.md` (medición 278/278) + `docs/dev/avance/activo/ci-cd.md:292` (278/278) + `docs/dev/plans/2026-09-07-cleanup-gates.md` Task 3.
...
- **Filas activas:** `Select-String -Path docs/dev/Backlog.md -Pattern '^\| `[^`]+` \|' | Measure` = **67** (no-struck). Cross-check: pattern `Pendiente` = 67; total `^\| (`|~~`)` = 68 (1 struck `TBH-01` ✅). Header decía 99 → stale.
...
- **Comando header a pegar:** `Select-String -Path docs/dev/Backlog.md -Pattern '^\| `[^`]+` \|'`

## Steps

### Step 1: Editar header (99→67 con fecha+comando) ✅ COMPLETED 2026-09-08
- ACT: edit `docs/dev/Backlog.md` old `**Total open items:** 99 activas` → new con 67 + fecha/comando.
...
- VERIFY: grep STABLE-06 → 280 ✅; `git diff --stat -- docs/dev/Backlog.md` = 1 file (mis 2 líneas) ✅. Global incluye dirty ajeno pre-existente (ORG-01..11, M .opencode, M opencode.jsonc) — no tocado.
