...
## Contrato
`Test-Path docs/dev/archive/case-studies-unverified/rag_edge_device.md` == true AND `Select-String -Path "docs/dev/archive/case-studies-unverified/README.md" -Pattern "no-público|ilustrativos" | Measure-Object Count` >=1
- **Verificación atómica extendida (pipeline-full docs-only no Rust):**
  - `Test-Path docs/dev/archive/case-studies-unverified/rag_edge_device.md` == True (5942L)
  - `Test-Path docs/dev/archive/case-studies-unverified/agent_local_memory_ollama.md` == True (6398L)
  - `Select-String -Path "docs/dev/archive/case-studies-unverified/README.md" -Pattern "no-público|ilustrativos" | Measure-Object Count` >=1 (Count 1, line 15 "ilustrativos NO verificados")
...
N/A — docs-only archiving, sin símbolos públicos nuevos. Decisión ya tomada D6 (owner): retirar ficticios antes de Show HN para honest results (PERF-03 / Regla 11). Implementación: `git mv docs/case_studies/*.md → docs/dev/archive/case-studies-unverified/` + README disclaimer (17L) + stubs `docs/user/book/src/case_studies/index.md` (no borrar carpeta book/case_studies, solo stub) + sweep refs públicas (docs/README.md, master-index.md, graphrag/README.md, skills/vantadb/SKILL.md) — 2 commits ya landed. Esta task Wave2 solo re-verifica guard anti-regresión.
...
| `nextTask` | GOV-B2 — Runbook DR sin comandos fantasma (docs/user/operations/DISASTER_RECOVERY_RUNBOOK.md) |
...
- **Archivos:** `docs/dev/archive/case-studies-unverified/*`, `docs/user/book/src/case_studies/index.md`, `docs/case_studies` (negativo)
...
- **Archivos:** `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` §GOV-B1 (Estado PENDING → COMPLETED si aplica, ya ✅ en este plan pero se sincroniza last-synced + recitation)
...
- GOV-B2 paralelo (runbook) — disjoint (docs/archive vs docs/user/operations/DISASTER_RECOVERY_RUNBOOK.md), MAX 3 ok
