...
## Metadata
- **Plan file:** `docs/dev/plans/2026-08-17-skills-vantadb.md` (wave SKL)
...
| Callees | API Python real (`vantadb-python/vantadb_py/vantadb_py.pyi` — fuente de verdad de firmas), API Rust (`src/sdk/api.rs`), integraciones (`integrations/langchain/`, `integrations/llamaindex/`), docs vivos (`docs/api/MCP.md`, `docs/user/operations/BENCHMARKS.md`) |
...
6. **🟡 Paths muertos** (verificados con Test-Path):
   - `docs/BENCHMARKS.md` → real `docs/user/operations/BENCHMARKS.md`
   - `docs/adr/` → real `docs/dev/architecture/adr/`
...
- **Invariantes a preservar:**
  ...
  2. NO tocar `docs/dev/Backlog.md`, task files, `docs/api/MCP.md`, `docs/dev/plans/*` — solo `skills/vantadb/`.
  3. Regla 11 (claims perf): número de latencia SOLO si cita benchmark reproducible (`docs/user/operations/BENCHMARKS.md`); si no hay fuente, reformular sin número.
...
1. **📝 DISCOVERY** — leer `skills/vantadb/SKILL.md` completo (666L) + `vantadb-python/vantadb_py/vantadb_py.pyi` (firmas reales) + `src/sdk/api.rs` (Rust SDK) + `docs/user/operations/BENCHMARKS.md` (números con fuente). Confirmar hallazgos del lead con `rg` de cada claim falso. Verify: lista de correcciones con `archivo:línea` objetivo.
2. **📝 EJECUCIÓN** — reescribir `skills/vantadb/SKILL.md`:
   - Versiones: 0.5.0 / Rust 1.94.1 / Py ≥3.11; dep `vantadb = "0.5.0"`.
   - Fix paths: `docs/user/operations/BENCHMARKS.md`, `docs/dev/architecture/adr/`, `integrations/langchain/` (o eliminar referencia si no aplica), reemplazar `langchain_rag.py` por ejemplo existente real.
   - Mover IQL/LISP: IQL soportado (sección propia), LISP no.
   - Tokenization: stemming/stopwords SÍ soportado (tantivy default).
   - Benchmarks: actualizar con fuente de `docs/user/operations/BENCHMARKS.md` o reformular sin números no verificables.
...
- [ ] **PERFORMANCE** — NO aplica: no toca hot path; los números que se citen vienen de `docs/user/operations/BENCHMARKS.md` (Regla 11).
...
- Claims perf: "sub-millisecond" eliminado (BENCHMARKS certifica p50 1.2ms @10K); tabla de benchmarks conservada con cita a `docs/user/operations/BENCHMARKS.md` §1/§2/§5/§6 (todos los números verificados contra la fuente, incluidos 68.4s y 2.43ms/4.01x).
