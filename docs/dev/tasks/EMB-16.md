...
- `docs/dev/tasks/EMB-10.md` (cat test referencia pares ≥0.91 vs impares ≤0.78; observación doble `encode` `llm.rs:~388` anotada).
...
- [ ] Commit `perf:` SOLO `src/llm.rs` + `src/physical_plan/vector.rs` + `docs/dev/tasks/EMB-16.md` (WIP ajeno excluido) + `campaign_memory_write` lessons (1-2). SIN push. Backlog→avance NO tocar.
...
- Commit: `perf: EMB-16 prefijos e5 por familia + margen medido` — SOLO `src/llm.rs` + `src/physical_plan/vector.rs` + `docs/dev/tasks/EMB-16.md`. SIN push (vía vanta-lead).
