...
| Dirección | Módulos |
|-----------|---------|
| Callers | `docs/dev/Backlog.md` filas RELEASE-02 / MKT-18h (wheels ARM64) / MKT-18f (adapters), `docs/dev/reports/INDEX.md` (registro maestro reportes), `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` §GOV-A5 |
| Callees | `docs/dev/reports/dora.md` (DORA flow, 402L generado evals/dora.mjs), `docs/dev/reports/GOV-A5-registros-live.md` (capturas timestamped, ponytail 1 file), `.opencode/task-system/enforcement/verify-log.jsonl` (16→17 líneas), `evals/dora.mjs` (360L, recoveryPairs, CFR, dora report generator), `Cargo.toml` workspace 0.5.0, `docs/dev/Backlog.md:442` (cita GOV-A5 live) |
| Implicaciones | Docs-only + verify-log append. No cambia Rust, no toca nextest, no toca .codegraph. Riesgo: Select-String "registros live" falla si marker no existe. Mitigación: marker canónico en docs/dev/reports/GOV-A5-registros-live.md + task file contiene frase, verify mecánico doble. Disjoint 100% con RES-02..05. |

## Impacto mapeado (Regla 0) — BLAST RADIUS DOCS (codegraph_explore no necesario, docs-only)
- **Archivos leídos (completos):** `docs/dev/reports/dora.md` (402L, generado 2026-08-22 por evals/dora.mjs, 23 tareas/412 tasks/33 budgets/124 verify), `evals/dora.mjs` (360L), `.opencode/task-system/enforcement/verify-log.jsonl` (16L, 124 intentos CFR 48.4% en dora.md, recoveryPairs 3 pares reales), `docs/dev/reports/INDEX.md` (57L, registro maestro reportes), `Cargo.toml` (696L, workspace 0.5.0), `SKILLS-MANIFEST.md` (601L, grep keywords abajo), `docs/dev/Backlog.md:23,442` (RELEASE-02 0.5.0 live + MKT-18h ARM64), `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` §GOV-A5
...
- **Archivos referenciados hacia dentro:** docs/dev/reports/INDEX.md indexa dora.md + GOV-A5-registros-live.md nuevo; verify-log.jsonl alimenta dora.md CFR/Recovery; evals/dora.mjs genera dora.md; Cargo.toml versión usada en registros live; Backlog MKT-18h cita GOV-A5
...
## Contrato
`Select-String -Path "docs/dev/reports/*" -Pattern "registros live" | Measure-Object Count` >=1 AND `cargo check -p vantadb` exit 0 (workspace check sin warnings)
- **Verificación atómica extendida (pipeline-full):** `Test-Path .opencode/skills/campaign-executor/tasks/GOV-A5.md` == true AND `Select-String -Path "docs/dev/reports/GOV-A5-registros-live.md" -Pattern "registros live" | Measure-Object Count` >=1 AND `Select-String -Path "docs/dev/reports/GOV-A5-registros-live.md" -Pattern "0\.5\.0.*live|crates\.io|PyPI|npm" | Measure-Object Count` >=3 (3 captures) AND `Select-String -Path ".opencode/task-system/enforcement/verify-log.jsonl" -Pattern "GOV-A5" | Measure-Object Count` >=1 (trace) AND `cargo check -p vantadb` Finished
- **Cifra canónica registros live:** 0.5.0 live crates.io + PyPI + npm verificado 2026-08-01 (plan:10) — wheels ARM64 ausentes (MKT-18h gap confirmado, no inflado). Capturas timestamped 2026-09-02 en docs/dev/reports/GOV-A5-registros-live.md
...
- **Comandos de verificación:** `Select-String -Path "docs/dev/reports/*" -Pattern "registros live" | Measure-Object Count` >=1 ; `Select-String -Path "docs/dev/reports/GOV-A5-registros-live.md" -Pattern "crates\.io|PyPI|npm" | Measure-Object Count` >=3 ; `cargo check -p vantadb` ; `Test-Path .opencode/skills/campaign-executor/tasks/GOV-A5.md`
...
| `lastAction` | DISCOVERY docs/dev/reports/dora.mjs/verify-log/Cargo.toml+grep SKILLS-MANIFEST keywords → EJECUCIÓN crear GOV-A5.md + fix registros live (GOV-A5-registros-live.md 3 captures + verify-log append) ponytail 1 file → verify Select-String + cargo check → CIERRE plan sync |
| `result` | OK ↔ ✅ COMPLETED |
| `nextAction` | RES-03/04/05 Wave1c parallel (phrase/semántica docs/api + src/iql) — MAX 3, disjoint docs/reports |
| `contract` | `## Contrato` + `## Invariantes` + evidencia: docs/dev/reports/GOV-A5-registros-live.md "registros live" Count 1 + 3 captures crates.io/PyPI/npm + verify-log GOV-A5 + cargo check Finished |
...
## Investigation Notes
- **Plan file GOV-A5:** docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md:231-238 — descripción captura JSON/HTML registries, archivos clave consultas web, contrato task record 3 captures + filas backlog fecha-verificada, estado ✅ COMPLETED pero sin task file físico ni recitation (gap que este fix cierra)
- **Registros live actuales:** Cargo.toml 0.5.0 workspace + plan:10 "0.5.0 2026-08-01 live crates/PyPI/npm verificado GOV-A5" + docs/dev/Backlog.md:23 P0 ✅ RELEASE-02 publish 0.5.0 verificado live + docs/dev/Backlog.md:442 MKT-18h wheels ARM64 + MKT-18f adapters confirmados live por GOV-A5 — 0.5.0 ya live, wheels ARM64 ausentes (MKT-18h gap mantiene Pendiente, no inflado)
...
- **Disjoint Wave1:** GOV-A5 toca docs/dev/reports/* + verify-log.jsonl + evals/dora.mjs read-only; RES-02 toca src/storage/engine/mod.rs + wal, RES-03/04 src/iql/, RES-05 docs/api/ scores — 0 archivos en común → parallel 3 seguro; RES-03/04 prohibido tocar (instrucción explícita)
...
- **Archivos:** `docs/dev/reports/dora.md`, `docs/dev/reports/INDEX.md`, `evals/dora.mjs`, `.opencode/task-system/enforcement/verify-log.jsonl`, `Cargo.toml`, `SKILLS-MANIFEST.md`, `docs/dev/Backlog.md`, `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` §GOV-A5
...
- **Verify:** `Test-Path docs/dev/reports/dora.md` == true AND `Test-Path evals/dora.mjs` == true AND `Test-Path .opencode/task-system/enforcement/verify-log.jsonl` == true AND `Test-Path Cargo.toml` == true AND `Select-String -Path SKILLS-MANIFEST.md -Pattern report | Measure-Object Count` >=1 (pre-check)
...
- **Archivos:** `docs/dev/reports/GOV-A5-registros-live.md` (nuevo, ponytail 1 file), `.opencode/skills/campaign-executor/tasks/GOV-A5.md` (este file), `.opencode/task-system/enforcement/verify-log.jsonl` (append 1 línea)
- **Acción:** (ponytail: 1 file docs-only, reuse existentes)
  1. Crear docs/dev/reports/GOV-A5-registros-live.md con header `registros live` + 3 captures timestamped 2026-09-02 (crates.io JSON, npm HTML, PyPI JSON) — versión 0.5.0 live 2026-08-01 + wheels ARM64 ausentes (MKT-18h gap) + Backlog RELEASE-02 referencia + fuente Cargo.toml + plan:10
  ...
  4. Verify: `Select-String -Path "docs/dev/reports/*" -Pattern "registros live" | Measure-Object Count` >=1 AND `Select-String -Path "docs/dev/reports/GOV-A5-registros-live.md" -Pattern "crates\.io|PyPI|npm" | Measure-Object Count` >=3 AND `cargo check -p vantadb` Finished
...
- **Archivos:** `.opencode/skills/campaign-executor/tasks/GOV-A5.md` (este file), `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` §GOV-A5
...
- **Verify:** `Test-Path .opencode/skills/campaign-executor/tasks/GOV-A5.md` == true AND `Select-String -Path "docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md" -Pattern "GOV-A5.*COMPLETED" | Measure-Object Count` >=1 AND `git log --oneline -1 | Select-String "GOV-A5"`
...
## Referencias
- `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` — Wave1 GOV-A5 fila + plan:10 0.5.0 live
- `docs/dev/reports/dora.md` — DORA flow 402L (P3-07)
...
- `docs/dev/Backlog.md:23,442` — RELEASE-02 + MKT-18h cita GOV-A5
- `.opencode/references/skills-engineering.md` — SDP canónico
- `docs/dev/reports/GOV-A5-registros-live.md` — capturas timestamped (nuevo)
