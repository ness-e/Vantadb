...
## Metadata
- **Plan file:** `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` (Wave3 — 20260902-alta-prioridad-paralelo, Wave3 ~42/86 ya ✅)
- **Fuente:** `docs/dev/research/archive/FND-01-core-bindings-boundaries.md` F2/F3 (rss_threshold backpressure 0.80 + memory budget compute/storage separation) + `src/config.rs:22` + `benches/memory_budget.rs`
...
|-----------|---------|-------------|
| **Blast radius RES-07** | `src/config.rs:22` (DEFAULT_RSS_THRESHOLD 0.80), `src/config.rs:285` (rss_threshold field), `src/config.rs:646` (Default), `src/config.rs:930` (with_rss_threshold clamp), `benches/memory_budget.rs` (161L, DIM 1536, scales 10k/25k/50k/100k, lite 5k/10k/20k, guard_effective pressure_ratio), `Cargo.toml:265` ([[bench]] memory_budget), `docs/user/operations/CONFIGURATION.md:28` (rss_threshold 0.80), `src/storage/engine/stats.rs:98` (check_memory_pressure guard_effective) | Memory budget es guard de backpressure: rss_threshold 0.80 (0.0 disable) + clamp 0.0..1.0. Bench mide RSS real vs logical_estimate (trend, no absoluto) bajo write+read mix (10k r/s). 0 líneas nuevas si landed — verify only. |
| **Disjoint Wave3** | MEM-14 toca `vanta-memory/src/core/scene/scene_extractor.rs` (strategy UPDATE>MERGE>CREATE + heat + soft-delete), GOV-C7 toca `docs/dev/Backlog.md` + `docs/dev/strategy/ROADMAP.md` | 0 archivos en común — parallel seguro MAX 3 (RES-07 src/config + benches, MEM-14 vanta-memory, GOV-C7 docs). Regla: no tocar vanta-memory. |
| **No tocar** | `vanta-memory/**`, `src/wal.rs`, `src/vector/`, `src/storage/` (Arch/Engine) | Guard disjoint MEM-14 — RES-07 solo verifica src/config.rs + benches/memory_budget.rs + docs/user/operations/CONFIGURATION.md. |
...
> AND `Select-String -Path "docs/user/operations/CONFIGURATION.md" -Pattern "rss_threshold" | Measure-Object Count` >=1
...
## Spec (doc-driven)
N/A — config + bench ya landed. Docs-first ya existe: `docs/user/operations/CONFIGURATION.md:28` row `rss_threshold | f64 | 0.80` + `benches/memory_budget.rs` header FND-01 contract doc. No crear doc nuevo.
...
- **Comandos de verificación:** `cargo check -p vantadb` Finished; `Select-String -Path "src/config.rs" -Pattern "rss_threshold" | Measure-Object Count` >=1; `Select-String -Path "docs/user/operations/CONFIGURATION.md" -Pattern "rss_threshold"` >=1; `Test-Path benches/memory_budget.rs`; `cargo check -p vantadb --bench memory_budget` Finished
...
- Read src/config.rs + benches/memory_budget.rs + docs/user/operations/CONFIGURATION.md + Cargo.toml
...
- writing-plans (plan docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md §RES-07)
...
- **docs/user/operations/CONFIGURATION.md:28:** `| rss_threshold | f64 | 0.80 | — | RSS pressure threshold for backpressure eviction (0.0-1.0) |` + Envelope outside VantaConfig (VANTA_EMBEDDING_PROVIDER etc).
- **Disjoint Wave3:** RES-07 toca src/config.rs + benches/memory_budget.rs + docs/user/operations/CONFIGURATION.md (config/bench); MEM-14 toca vanta-memory/src/core/scene/scene_extractor.rs (604L, strategy heat/soft-delete); GOV-C7 toca docs/dev/Backlog.md (130 activas) + docs/dev/strategy/ROADMAP.md — 0 archivos en común → parallel 3 seguro (MAX 3). No tocar vanta-memory — worker es multi-platform bindings pero este task es config core (disjoint garantizado).
...
- **Archivos:** `src/config.rs`, `benches/memory_budget.rs`, `Cargo.toml`, `docs/user/operations/CONFIGURATION.md`, `SKILLS-MANIFEST.md`, `.opencode/task-system/prompts/pipeline-full.md`
...
- **Archivos:** `src/config.rs:22`, `benches/memory_budget.rs`, `docs/user/operations/CONFIGURATION.md:28`, `Cargo.toml`
- **Acción:** (ponytail: reuse existente, 0 líneas nuevas si landed)
  ...
  3. Verificar `docs/user/operations/CONFIGURATION.md:28` row 0.80 — ya landed
  ...
  11. Run `Select-String -Path "docs/user/operations/CONFIGURATION.md" -Pattern "rss_threshold"` → Count 1
...
- **Archivos:** `.opencode/skills/campaign-executor/tasks/RES-07.md` (este file), `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` §RES-07
...
- **Verify:** `Test-Path .opencode/skills/campaign-executor/tasks/RES-07.md` == true AND `Select-String -Path "docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md" -Pattern "RES-07.*COMPLETED" | Measure-Object Count` >=1
...
> antes de editar `docs/user/operations/BENCHMARKS.md`. Steps 1-3 (2026-09-02) quedan ✅ intactos.

- **Archivos leídos (completos):** `docs/user/operations/BENCHMARKS.md` (362L, 0 hits rss_threshold), `src/config.rs` (:22 `DEFAULT_RSS_THRESHOLD 0.80`, :285 field, :646 Default, :930 clamp, tests :1413+), `benches/memory_budget.rs` (161L, DIM 1536, full [10k,25k,50k,100k] / lite [5k,10k,20k]), `docs/dev/research/archive/FND-01-memory-budget.md` (124L, F1 ✅ aplicado, F2/F3 ⬜ pendientes), `.opencode/rules/memory-budget.md` (34L, reglas 1-4), `docs/dev/plans/2026-09-03-quality-gtm-wave.md` (Task 1 contrato + pre-mortem + stop conditions)
...
- **Archivos:** `src/config.rs:22`, `benches/memory_budget.rs`, `docs/user/operations/BENCHMARKS.md`, `docs/dev/research/archive/FND-01-memory-budget.md`
...
- **Archivos:** `docs/user/operations/BENCHMARKS.md` (append §12 al final, tras §11; nada más)
...
- **Verify:** `rg -n "rss_threshold" docs/user/operations/BENCHMARKS.md | Measure Count` ≥1 + línea decisión ≥1 + `cargo fmt --all -- --check` exit 0
...
- **Archivos:** `docs/dev/plans/2026-09-03-quality-gtm-wave.md` (Task 1 Estado → ✅ + recitation), este task file (Steps 4-6 → ✅ + Context Save Point)
- **Acción:** sync plan + recitation canónica; commit convencional `fix(memory-budget): calibrar rss_threshold con bench F2 (RES-07)` solo con archivos del blast radius (`git add docs/user/operations/BENCHMARKS.md` + task file + plan file)
...
- `docs/user/operations/CONFIGURATION.md:28` — rss_threshold 0.80 row
- `src/storage/engine/stats.rs:98` — check_memory_pressure guard_effective
- `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` — Wave3 RES-07 fila
