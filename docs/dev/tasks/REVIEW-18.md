...
## Metadata
- **Plan file:** docs/dev/plans/2026-08-23-backlog-triage.md (Task 13 — NO editar; estado trackea este task file)
...
- **Referencias hacia dentro:** ninguna en código fuente — `next.config.ts` se carga por convención de Next.js (build/dev/start). `rg -l "next.config"` = solo docs/dev/reviews/plans (documentales).
