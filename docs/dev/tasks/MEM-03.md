...
Wave2 F1 paralelo MAX 3: MEM-03 (parser/query/planner) || MEM-02 (vantadb-mcp) || GOV-B3 (docs/user/tutorials). Archivos clave disjuntos: `src/parser/mod.rs`, `src/query.rs`, `src/planner/optimize_and_compile`.
...
- **Disjoint verificado:** MEM-02 toca `vantadb-mcp/src/handlers/tools.rs` + validation; GOV-B3 toca `docs/user/tutorials/03-migrating-from-chromadb.md` + `docs/user/glosario/hnsw.md`; MEM-03 toca `src/parser`, `src/query`, `src/planner` — 0 overlap.
...
- Plan: `docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` Wave2 F1 MEM-01→02→34 → MEM-03→04→05
