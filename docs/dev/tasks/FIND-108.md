...
## Metadata
- **Plan file:** docs/dev/plans/2026-09-17-mvp-memoria-agentes.md
...
`scripts/install.*`, `README.md`, `docs/user/QUICKSTART.md` (FIND-105 en paralelo — NO tocar), `reparacion.bat`, `.opencode` vivo (edits — las rules van a `.opencodereview/`, nunca a `.opencode/`), `Justfile`, `completions/*`, `desktop/src-tauri/Cargo.lock`, `stash@{0}` GOV-C4, `docs/dev/Backlog.md`, plan file (solo append recitation al cierre, nunca re-spec), `src/`, `examples/`, `vanta-memory/`, `vantadb-mcp/`, `skills/`, `setup-embeddings.ps1`, `vanta-mcp-local.ps1`. WIP ajeno visible en `git status` (`completions/*`, `.opencode`, `docs/dev/Backlog.md`, plan) → staging SELECTIVO (`git add` solo paths de esta tarea).
...
- **Archivos:** `docs/dev/tasks/FIND-108.md` (sync), plan file (append recitation)
...
- **Cierre 2026-09-17 (reanudación tras abort):**
  ...
  - Intel orquestador corregida: `.github/workflows/ocr-delegate.yml` está TRACKED e intacto (no `??`); WIP ajeno real: `completions/*`, `.opencode`, `docs/dev/Backlog.md`, plan file, `reparacion.bat` — ninguno stageado.
