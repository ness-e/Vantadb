...
## Metadata
- **Plan file:** docs/dev/plans/2026-08-28-master-pipeline-optimization.md
...
| Dirección | Módulos |
|-----------|---------|
| Callers | `.opencode/task-system/prompts/plan.md` (template, línea 246), `docs/dev/plans/2026-08-28-master-pipeline-optimization.md` (Task 8 + Task 16), `.opencode/task-system/prompts/question-gates.md` (§Anti-abuso, Routing), `.opencode/task-system/mcp/campaign-server.mjs` (campaign_get_next_task autonomous) |
...
- **Deuda pendiente:** ninguna — idempotente completo, sin edición; próxima tarea HIGH-009 continúa secuencial (Session Cleanup). Plan file header `docs/dev/plans/2026-08-28-master-pipeline-optimization.md` no tiene `> **Autonomous:**` porque fue creado antes de HIGH-008 — el campo vive en el template (canónico plan.md Formato del plan file), no requiere backfill en planes históricos ya COMPLETED; futuros planes usarán template con default false
...
- Futuros planes: template canónico `docs/dev/plans/<fecha>-<nombre>.md` ahora siempre incluye `> **Autonomous:** false` por defecto (plan.md:246) — planes históricos (como 2026-08-28) no requieren backfill; campaign_get_next_task ya lee autonomous via extractAutonomous y lo expone para orquestador (question-gates §Anti-abuso routing)
...
- `docs/dev/plans/2026-08-28-master-pipeline-optimization.md:206-220` — Task 8 definición + contrato grep -n Autonomous + pre-mortem default false
- `docs/dev/plans/2026-08-28-master-pipeline-optimization.md:536-545` — Verificación por task (HIGH-008: grep Autonomous plan.md → 1 ✅)
