...
- **ID:** FND-10
- **Fuente:** docs/dev/Backlog.md:498 (P20b, prio 🔴)
- **Plan:** docs/dev/plans/2026-08-16-wave-p20-tsys.md (Wave 3, Task FND-10)
...
- `docs/user/operations/BENCHMARKS.md` — métricas certificadas (p50/p95/p99 de SDK Python), sin benchmark canónico Rust insert 100k×1536d + search.
- `docs/user/operations/PERFORMANCE_TUNING.md` §7 — comandos de benchmark (`cargo bench`), perfil con RUSTFLAGS target-cpu=native.
...
- Sección en `docs/user/operations/BENCHMARKS.md` (inglés, fuente de verdad) con el baseline registrado: entorno (CPU/RAM/OS), comando, resultados P99 insert+search, fecha.
- Este task file registra el baseline.

## Impacto mapeado (Regla 0)

- **Archivos leídos completos:** `benches/hnsw_pure.rs`, `benches/high_density.rs`, `benches/common/mod.rs`, `Cargo.toml` (§benches), `docs/user/operations/BENCHMARKS.md`, `docs/user/operations/PERFORMANCE_TUNING.md` (§7), `.opencode/AGENTS.md` (Reglas 1-10), plan file, task file PERF-02.md.
...
- `docs/dev/Backlog.md`, `.opencode/skills/campaign-executor/tasks/AUD-024.md`, `.opencode/task-system/enforcement/verify-log.jsonl`, `completions/_vanta-cli.ps1`, `docs/dev/plans/2026-08-16-wave-p20-tsys.md`, `.opencode/agents/*`.

## Baseline registrado (Step 4)

> Ver `docs/user/operations/BENCHMARKS.md` § "Canonical P99 Baseline (FND-10)" — se agrega en Step 4.
...
- Baseline en `docs/user/operations/BENCHMARKS.md` (P99 insert + search con entorno)
...
- **Step 4:** ✅ COMPLETO — §8 "Canonical P99 Baseline (FND-10)" agregada a `docs/user/operations/BENCHMARKS.md` con entorno, comando, resultados y nota de uso
