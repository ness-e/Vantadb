...
## Metadata
- **Plan file:** `docs/dev/plans/2026-08-25-research-desktop-quickwins.md`
...
## Blast Radius
- `docs/dev/Backlog.md` — tabla P37 (9 filas DAUD-01..09) + Exec Summary línea P37 (conteo 9→0) + `last_reviewed`. 9 filas todas ✅ Hecho con commits `3c53d8b2`,`480935a7`,`b865c625` + DAUD-02 via `ad0f34b1` (QW4). Edición es DELETE de filas stale, sin lógica. No afecta código Rust/TS/desktop TSX.
- `docs/dev/avance/activo/desktop.md` — registro de dominio donde vive el completado de Phase DESKTOP (Trigger 1 progreso). DAUD pertenece a desktop post-fix. Si no se registra → gap potencial en check-avance-coverage si DAUD se considera fuente.
- `docs/dev/avance/historial/backlog-history.md` — historial de items removidos del Backlog (Trigger 1 para items sin completar, o registro de limpieza masiva). DAUD removido debe quedar trazado aquí si no va a activo/desktop.md.
...
- `docs/dev/plans/2026-08-25-research-desktop-quickwins.md` — plan file Wave1 Task5. Tras cierre, agregar recitation DESKTOP-QW5.
...
- **Archivos leídos completos (antes de editar):**
  - `docs/dev/Backlog.md` (completo, ~800 líneas; P37 líneas 517-532, Exec Summary línea 47, last_reviewed header línea 6)
  - `docs/dev/plans/2026-08-25-research-desktop-quickwins.md` (78 líneas + 4 recitations QW1-4)
  - `docs/dev/avance/activo/desktop.md` (294 líneas, `last_reviewed: 2026-08-07`, §DESKTOP-38 + UX-01, sin DAUD aún)
  - `docs/dev/avance/historial/backlog-history.md` (216 líneas, §§ limpieza masiva 2026-08-07/25, sin DAUD aún)
...
- **Referencias hacia dentro (qué importa este archivo):**
  - `docs/dev/Backlog.md` → importado por `skill progreso` (lee backlog, chequea WIP), por `docs/dev/avance/*` (cross-ref), por CI docs. No importado por código Rust/TS. Es catálogo de tareas, no código.
  - `docs/dev/avance/activo/desktop.md` → referenciado desde Backlog Exec Summary P12/P37 como registro de completado (ver sección P12). Fuente de verdad de dominio desktop para `check-avance-coverage` si ID está en fuentes.
...
3. `Select-String -Path docs/dev/Backlog.md -Pattern "DAUD-"` — 0 hits (Backlog sin DAUD stale)
...
- **Archivos:** `docs/dev/Backlog.md:517-532`, `docs/dev/avance/activo/desktop.md`, `git log` (3c53d8b2,480935a7,b865c625,ad0f34b1,a7ed0d22), `git stash list`
...
- **Verify:** `git log --oneline b865c625^..b865c625` presente ✅ (2026-08-25 fix recover DAUD-11+06); `git log 3c53d8b2` presente ✅ (8 files DAUD-LIMPI); `git log 480935a7` presente ✅ (11 files E2E-VISUAL); `Select-String DAUD docs/dev/Backlog.md` 10 hits pre-edit (1 P37 header + 9 filas) todos con `Hecho/Cerrada` ✅; `git stash list | Select-String "06aa1a86"` 0 hits ✅ — stash@{0} actual es `2fc26b26`, no `06aa1a86` (consumido)
...
- **Archivos:** `docs/dev/Backlog.md` (líneas 47 Exec Summary + 517-532 P37)
...
- **Archivos:** `docs/dev/avance/activo/desktop.md` (append §DAUD), `docs/dev/avance/historial/backlog-history.md` (append limpieza DAUD)
- **Acción:** 
...
- **Verify:** `Select-String DAUD docs/dev/avance/activo/desktop.md` → 11 hits (P37 header + 9 DAUD IDs + refs) ✅; `Select-String DAUD docs/dev/avance/historial/backlog-history.md` → 12 hits (limpieza + 9 rows) ✅; `last_reviewed: 2026-08-26` en desktop.md ✅; `pwsh scripts/check-avance-coverage.ps1 -Detail` → sin huérfanos (ver Step4)
...
- **Archivos:** `docs/dev/plans/2026-08-25-research-desktop-quickwins.md`, `docs/dev/Backlog.md`, `docs/dev/avance/*`, `.opencode/skills/campaign-executor/tasks/DESKTOP-QW5.md`
- **Acción:** Verify mecánico del contrato:
  ...
  4. `Select-String -Path docs/dev/Backlog.md -Pattern "^\| \`DAUD-"` → 0 hits ✅ (Backlog sin filas stale; solo header P37 + historial refs)
  ...
  - Si todo pasa: `git add docs/dev/Backlog.md docs/dev/avance/activo/desktop.md docs/dev/avance/historial/backlog-history.md docs/dev/plans/2026-08-25-research-desktop-quickwins.md .opencode/skills/campaign-executor/tasks/DESKTOP-QW5.md` + commit `docs(backlog): DESKTOP-QW5 — limpiar filas DAUD-01..09 stale (H-13) + historial desktop` (conventional `docs:`)
...
- check-avance-coverage escanea fuentes en docs/dev/avance/historial/fuentes + campanas, no Backlog → DAUD no afecta cobertura pre/post, pero registrar en activo/desktop.md asegura trazabilidad de dominio (progreso Trigger 1).
