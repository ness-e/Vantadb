...
- **Recitation:** D4b-diseño entrega `docs/dev/tasks/D4b.md` completo (tabla §A con evidencia, compat §B, slices+semver §C, ADR-datos §D); PROXIMO_STEP = D4b-impl (worker, slices §3); sin commit (diseño no commitea por despacho); Gate D = GO (blast radius diseño <10 archivos tocados — 1 archivo creado, 0 código modificado).
