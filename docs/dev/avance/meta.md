...
Los archivos de `docs/dev/avance/activo/` **se actualizan al cierre de cada campaña** (no daily); los dominios del mirror = **crates activos** del workspace. Un crate nuevo ⇒ archivo de dominio nuevo en el mismo cierre. Verificación: muestreo cruzado `git log --grep MEM-` ↔ archivos de dominio (GOV-D1, 2026-08-22).
...
- Fuente: `docs/dev/research/VantaDB_ANALISIS_COMPLETO.md`.
...
- Planes archivados: `docs/dev/plans/archive/2026-08-23-task-system-hardening{,-2}.md`; retrospectivas en `docs/progreso/campanas/planes-archivados-punteros.md`.
...
### 2026-08-07 — Migración de `docs/progreso/` → `docs/dev/avance/`
...
- Reclasificación de archivos: `vectara-competitive-research` y `meta-001-root-cause-analysis` → `docs/dev/research/`; `backlog-validation`, `progreso-readme-part1/2/3`, `progreso-sistema` → `docs/dev/reviews/archive/`.
...
- Referencia: `docs/dev/reviews/FINAL-REVIEW.md` (Core 50).
...
- **Regla dura:** derivar un hallazgo = crear la fila en `docs/dev/Backlog.md` EN EL MISMO commit donde se registra la derivación. Prohibido dejar la derivación solo en prosa de reports/informes.
- **Chequeo mecánico:** Trigger 4 de `progreso` incluye grep de IDs citados en `docs/dev/reviews/*.md` (apéndices H-NN, MOD-\*, derivaciones) vs filas reales en Backlog/historial; IDs huérfanos = hallazgo REC-\* inmediato.
- **Origen:** decisión HITL `/research synthesis` Q8 (sesión 2026-08-26); ref: `docs/dev/reviews/research-bindings-synthesis-20260825.md` §3.
...
Plan docs/dev/plans/archive/2026-09-02-error-observability-excellence.md archivado con retrospectiva: 9/9 ✅ (4 waves, MAX_CONCURRENT=3, 89% first-try — único retry por fallo de infra uv_spawn, no de diseño). Contrato unificado VANTADB_* (10 códigos) core→WASM/Node/TS→Python/providers→MCP(-320xx)→Desktop→Web. Follow-ups: FIND-52/53/54 + sanitización 500.
...
Plan `docs/dev/plans/archive/2026-09-09-backlog.md` (+budget) archivado: 6/6 ✅ en 2 waves secuenciales, 0 failed. Backlog: -5 filas (STABLE-09, TS-12, MEM-68, PRX-05, PRX-12) + PRX-09 re-scopeada a slice 2 (semántico/TTL/LRU). P47 cerrada (STABLE-09 subset `[., python, memory, server, mcp]` 546dabd1). Avance: ci-cd (STABLE-09, TS-12), vanta-memory (MEM-68), vanta-proxy.md nuevo (PRX-05/12), operaciones (PRX-09 slice 1). check-avance-coverage 1038/1038 (100%). 2 aborts de lanzamiento recuperados con RETRY fresco sin pérdida (TS-12, PRX-12); PRX-12 pedida como RESUME pero sin task_id → fresco (lección: abort pre-creación no deja sesión).
...
- **Plan:** docs/dev/plans/archive/2026-09-10-anti-stutter.md (+ .budget.json) - 7/7 COMPLETED (AST-001...007).
...
- **Plan:** docs/dev/plans/archive/2026-09-11-anti-stutter-cierre.md (+ .budget.json) - 4/4 COMPLETED (AST-008/009/010/011, Wave 1 + Wave 2).
...
- **Plan:** docs/dev/plans/archive/2026-09-13-cleanCA-fase2.md - 18/18 COMPLETED (M1,C1,D0,A1,S1,S7,M2,S2,S5,S3,T1,T2,T3,S3b,C2,A2,M3,S6).
...
- **Plan:** docs/dev/plans/archive/2026-09-15-find-correcciones.md - 31/31 COMPLETED (Wave0-9: 90/91/64, 63/65/88, 79/78/70, 87/73/71, 77/82/83, 67/68/81, 66/75/69, 84/85/80, 74/86/72, 92/93/94/95; +plan-adjust Wave9 2026-09-16; SKIP FIND-76 → backlog-history).
...
- **Plan:** docs/dev/plans/archive/2026-09-16-embeddings-auto.md - 11/11 COMPLETED (Wave0: EMB-10; Wave1: EMB-11/12; Wave2: EMB-13/16/18; Wave3: EMB-14; Wave4: EMB-15/17; Wave5: EMB-19/20; FIND-99 épica cerrada por EMB-13+14; HALLAZGOS → FIND-100/101/102).
...
- **Plan:** docs/dev/plans/archive/2026-09-18-smoke-e2e.md - 7/7 fases verdes (MCP 79/87, loop memoria + fallback:false, hooks 50/50, wizard/demo/roundtrip/salvage, superficie honesta, fmt/clippy + full nextest 3331 passed) + triage (0 bugs nuevos, GO a Publicacion). Sin budget.json (run manual asistido, sin campana MCP).
...
- **Plan:** docs/dev/plans/archive/2026-09-19-ci-green.md - 5/5 COMPLETED (Wave0: FIND-124 desktop + FIND-123 vs-table; Wave1: FIND-125 TS + FIND-126 docs; Wave2: FIND-127 infra; +P2-01 2 reviews approve + follow-ups d755aeec). Deuda: FIND-128/129/130/131/132 siguiente plan.
...
### Pipeline compliance — PASO 0 en MODO TAREA + pre-spawn checklist + review P2-01 (2026-09-24)
- **Contexto:** en EST-03/EST-10/EST-05 (primera pasada) la ejecución saltó pasos canónicos del harness: no se leyó `prompts/task.md`; el campaign MCP (detect/discover/verify) no se usó; task files en formato reducido (sin Impacto Regla 0/Spec/Invariantes/Deuda/DoD/Review); gate Review P2-01 omitido; sin fallback explícito inline cuando no hay subagentes.
- **Cambios (harness `.opencode`, repo configOpencode):** `commands/pipeline.md` MODO TAREA gana **PASO 0 — GATE de proceso** (lectura completa de `task.md` + campaign tools obligatorias + formato canónico + gate Review + fallback inline sin subagentes); `prompts/pipeline-run.md` gana **CHECKLIST PRE-SPAWN** (10 bloques poblados, WIP ajeno, rutas exactas, contrato, SDP, fallback); `prompts/pipeline-full.md` §Cierre gana el paso **Review P2-01** como requisito de COMPLETED.
- **Lección:** "subagentes caídos → todo directo" es un fallback de DELEGACIÓN, nunca una exención de GATES; el flujo canónico vive en los prompts referenciados — hay que leer la cadena completa (`pipeline.md → task.md / pipeline-full.md / pipeline-run.md`) ANTES de actuar.

### Harness OpenCode V2 — auditoría de migración (v2.0.16, 2026-09-25)
- **Alcance:** docs V2 (`opencode.ai/v2/docs`: migrate-v1, agents, compaction, warming) vs `.opencode/` completo (agents/commands/configs/mcp-profiles/opencode-loop/plugins/references/rules/skills/task-system/AGENTS.md/MANUAL) + `opencode.jsonc`.
