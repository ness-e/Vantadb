...
- **Fuente:** `docs/dev/Backlog.md` → P21
- **Fecha:** 2026-08-17
- **Objetivo:** dejar la skill `skills/vantadb/` sincronizada con el código real.
- **Resultado:** ✅ versión 0.5.0 / Rust 1.94.1 / Py ≥3.11; dep corregida; installer real (6B "test" → 2109B); claims falsos corregidos (IQL SÍ soportado, stemming SÍ soportado); paths vivos (docs/user/operations/BENCHMARKS.md, integrations/langchain/); benchmarks con fuente; features reales (ttl_ms, purge_expired, compact_wal, AsyncVantaDB, :memory:, hardware_profile); firmas API alineadas con pyi. Dirs vacíos references/ assets/ eliminados (ponytail).
- **Ids:** `SKL-01`
