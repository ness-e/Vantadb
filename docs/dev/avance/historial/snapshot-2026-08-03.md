...
> **Activate backlog:** [`docs/dev/Backlog.md`]([[Backlog.md]]) — prioritized tasks
...
- complete `docs/dev/graphrag/README.md`: comparison, getting started, Python examples
...
- `docs/user/operations/DURABILITY_GUARANTEES.md`, 9 sections, 10 guarantees, 7 failure scenarios
...
- [x] Modify `docs/dev/operations/CI_POLICY.md`
...
- **ROAD-06:** `docs/user/operations/grafana-dashboard.json` (6 panels: RSS, Memory Pressure, Vector Ops, Latency P50/P95/P99, Disk Usage, Index Memory) + `docs/user/operations/GRAFANA_SETUP.md`.
...
- **Resultado:** ✅ Doc: `docs/dev/research/INV-016-motion-duration-tokens.md`. **NO existen tokens de duración/easing.** globals.css solo colores+fuentes; easing `cubic-bezier(0.2,0.8,0.2,1)` hardcodeado en 4 lugares. Duraciones hardcodeadas: framer-motion (page-transition .28s, latency .4/.5s), Reveal (NOTE: NO framer-motion — CSS transition vía IntersectionObserver; default 600ms + delays 40-240), animejs (marks), Tailwind duration-* (75-1000). Propuesta: CSS vars `--duration-fast:150ms; --duration-normal:300ms; --duration-slow:500ms` + `--ease-default`; mapa JS `web/src/lib/motion.ts` `MOTION={duration:{fast:.15,normal:.3,slow:.5},ease:[.2,.8,.2,1]}` (framer-motion/animejs no leen CSS vars en `duration`); Reveal consume CSS vars vía transitionDuration. Cero cambios de código.
- **Ids:** `INV-016`
...
- **Checklist:**
  ...
  - [x] `docs/user/operations/BENCHMARKS.md` sección 3 reescrita — path standalone (pip install) antes que maturin
...
- **Fecha:** 2026-08-02
- **Objetivo:** Learning path estructurado en `docs/user/tutorials/` (5-7 ejemplos). Backlog decía "4/7, algunos draft". Corrige API inventada (`vantadb.connect`, `db.space`) en tutoriales draft y completa el path a 6 tutoriales.
- **Checklist:**
  ...
  - [x] `docs/user/tutorials/index.md` — learning path estructurado (core track 01→02→04→05, migration track 03/lancedb)
  - [x] `03-migrating-from-chromadb.md` — draft → active
  - [x] mdBook sync — index con `{{#include}}`, stubs 04/05, SUMMARY lista 6 tutoriales
  - [x] `docs/dev/master-index.md`, `docs/README.md` — links de tutoriales actualizados
...
- **Checklist:**
  ...
  - [x] `docs/user/operations/EXPERIMENTAL_FEATURES.md:73` actualizado al nuevo nombre
...
**Plan:** docs/dev/plans/2026-08-28-embeddings-local.md
...
| EMB-07 | Bench benchmarks/embed_bench.py 533L 1k EN+ES QPS/recall/RSS + docs/user/operations/BENCHMARKS.md §8 + report 9 filas | 67fda296 | ✅ |
