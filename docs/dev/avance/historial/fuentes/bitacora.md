...
- **AUD-07 — FAQ created:** `docs/user/FAQ.md` with common questions.
...
**Source:** `docs/dev/reviews/FULL_CODEBASE_AUDIT_2026-07-11.md` (755L, ~80 hallazgos)
...
**Qué se hizo:** Carga completa de `docs/dev/research/` (9 archivos) y `docs/dev/reviews/` (12 archivos). Extracción de ~150 items. Verificación cruzada contra el codebase real usando `codegraph_explore`, `grep`, y lectura directa para determinar estado (resuelto vs pendiente). Skills usadas: `code-review-and-quality`, `doubt-driven-development`, `writing-plans`, `ponytail (full)`.
...
Todos los items extraídos de `docs/dev/research/` (9 archivos) y `docs/dev/reviews/` (12 archivos) que implican modificar el proyecto, verificados contra código real. Organizados por dominio. Cada item incluye: qué es, por qué importa, qué investigar antes de implementar, archivos fuente de referencia, y estado.
...
- **Archivo recomendado:** `docs/dev/research/ACID_TRANSACTIONS.md` (análisis completo con 3 enfoques evaluados)
...
- **Investigar:** (1) Leer `docs/dev/research/AUDITORIA_COMPLETA_VantaDB_WEB.md` líneas 424-430; (2) Verificar números actuales en `web/src/routes/product/benchmarks.tsx`; (3) Decidir: corregir números, o remover la sección completa si no hay benchmarks reproducibles.
...
- **Qué:** WASM bundle único sin code-splitting. Plan existe en `docs/dev/plans/2026-07-10-wasm-code-splitting.md`.
...
- **Investigar:** (1) Leer `docs/dev/research/COGNEE_EVALUATION.md` (análisis completo); (2) Implementar `SessionCache` trait con backend Redis opcional y fallback HashMap in-memory; (3) Integrar con MCP tools.
...
- **Investigar:** (1) Leer `docs/dev/research/DOCS_TOOLS_RESEARCH.md` análisis completo; (2) Convertir Obsidian wikilinks `[[Link]]` a markdown estándar `[Link](path.md)`; (3) Crear `book.toml` y `SUMMARY.md`; (4) Integrar `cargo doc`.
...
- **Investigar:** (1) Revisar `docs/dev/architecture/adr/` para decisiones existentes; (2) Escribir ADRs faltantes.
- **Fuente:** AP-36
- **Esfuerzo:** 1 día

#### D6: Docs split (archivos >500L)
- **Qué:** `docs/progreso/README.md` (1529L), `docs/DESIGN_RULES.md` (709L), `docs/dev/Backlog.md` (674L) sin fragmentar.
...
- **Fuente:** `docs/dev/research/SIGNED_RELEASES.md`
...
- **Leer:** `docs/dev/research/ACID_TRANSACTIONS.md` para análisis completo de 14 páginas.
...
- **Leer:** `docs/dev/research/COGNEE_EVALUATION.md` para análisis completo.
...
- **Leer:** `docs/dev/research/DOCS_TOOLS_RESEARCH.md`

#### R6: Backlog items para eliminar
- **Fuente:** `docs/dev/research/VantaDB_ANALISIS_COMPLETO.md` Sección 3.1 — 15 items para eliminar del backlog:
...
- **Referencia:** `docs/dev/reviews/FINAL-REVIEW.md` (Core 50 lista recomendada).
...
**Qué se hizo:** Ejecución secuencial del plan `docs/dev/plans/2026-07-13-verified-backlog-execution.md` usando backlog-executor loop. 20 tareas verificadas/ejectadas como ya existentes o fixes mínimos; 15 restantes marcadas como 🟡 DEFER (contenido humano o feature work >1d).
...
**Plan actualizado:** `docs/dev/plans/2026-07-13-verified-backlog-execution.md` — 26/35 ✅ · 9 🟡 DEFER.
...
| Dominio | Archivos |
|---------|----------|
| **Audit Técnico** | `docs/dev/reviews/FULL_CODEBASE_AUDIT_2026-07-11.md` (755L, ~80 findings) |
| **Full Review Jul 13** | `docs/dev/reviews/2026-07-13-full-review.md` |
| **Análisis de Proyecto** | `docs/dev/reviews/analisis_proyecto.md` (~50 findings) |
| **Backlog Activo** | `docs/dev/Backlog.md` (48 items verificados) |
| **ACID Transactions** | `docs/dev/research/ACID_TRANSACTIONS.md` |
| **Signed Releases** | `docs/dev/research/SIGNED_RELEASES.md` |
| **Research Unificado** | `docs/dev/research/VantaDB_RESEARCH_UNIFIED.md` |
