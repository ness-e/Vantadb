...
> Registro vivo del trabajo completado en VantaDB, organizado por dominio en lugar de cronología plana. Este índice es el **único punto de entrada** del árbol `docs/dev/avance/`; cada archivo cubre un área y cada entrada conserva su ID original (`DRV-*`, `COMP-*`, `SEC-*`, etc.) para trazabilidad.
...
| Sección origen (`docs/progreso/README.md`) | Destino (`docs/dev/avance/`) |
|---|---|
...
| Fuente | Destino (`docs/dev/avance/historial/`) |
|---|---|
...
| `investigaciones.md` | `docs/dev/research/` catálogo + ARCHIVO_HISTORICO §INV | ✅ creado |
...
| `docs/dev/research/` | Research (destino de INV-*) | `investigaciones.md` (ya existente, 1:1) |

## Cruz de referencia con `docs/dev/Backlog.md`

- `docs/dev/Backlog.md` mantiene el **catálogo activo** (lo que queda por hacer) — ver `docs/dev/avance/README.md` → `docs/dev/Backlog.md`.
