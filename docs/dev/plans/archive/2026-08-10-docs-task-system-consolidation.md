...
> **Fuente:** Auditoría multi-agente (6 sub-agentes, solo-lectura) sobre `docs/dev/research/2026-08-10-agent-engineering/`, las 10 carpetas de `docs/` objetivo, y el flujo completo del task-system en `.opencode/`.
...
**Principio rector:** `docs/progreso/` es el registro canónico (single source of truth); `docs/dev/avance/` es su mirror curado navegable; `docs/dev/research/` es la carpeta canónica de investigación. Nada se borra sin grep previo (Regla 0). No se reescriben snapshots/historial (estados pasados congelados).
...
### Task 3: Unificar carpetas de investigación → `docs/dev/research/` (4→1)
- **Esfuerzo:** 🟡 | **Prioridad:** P0 | **Ruta:** vanta-docs
- **Archivos clave:**
  - `docs/investigacion/investigacion-equipo-2026-08-09.md` → MOVE a `docs/dev/research/` (basename conservado o `INV-021-`)
...
- **Archivos clave:** `.opencode/task-system/workflows/research.json:80` (dice "Guardar output final en docs/dev/research/")
- **Verificación real:** Regla en `.opencode/AGENTS.md:855` y manual L775 prohíben `docs/dev/research/`; el propio task `DESKTOP-01.md:39` documenta el override manual.
- **Gate Result:** 🔵 DO
- **Contrato:** workflows/research.json apunta a `docs/dev/research/`; `rg "docs/research"` en `.opencode/task-system/` = 0.
- **Estado:** ✅ COMPLETED (2026-08-10) — `workflows/research.json:80` → `docs/dev/research/` (commit `d4b3fe12`).

### Task 5: Restaurar ACID_TRANSACTIONS.md desde git
- **Esfuerzo:** 🟢 | **Prioridad:** P1 | **Ruta:** vanta-docs
- **Archivos clave:** `git show 8b1c52cd^:docs/dev/research/ACID_TRANSACTIONS.md` → restaurar a `docs/dev/research/ACID_TRANSACTIONS.md`
- **Verificación real:** Recomendado explícitamente por `docs/dev/research/ACID_ROLLBACK_DESIGN.md:57,61,455,495`; el archivo fue borrado en `8b1c52cd`.
- **Gate Result:** 🔵 DO
...
- **Archivos clave:**
  ...
  - `docs/dev/reviews/` es el hogar de la skill `unified-review` (`review-<mode>-<ts>.md`)
  - `docs/audit-reports/archive/` (14) vs `docs/dev/reviews/archive/` (1)
- **Verificación real:** El mismo pipeline (`/audit` ≡ `/review full`, alias legacy según `unified-review/SKILL.md:77`) genera ambos tipos → mismo artefacto partido en 2 por modo. `docs/dev/reviews/logs/` vacía (feature `keep_raw_logs` nunca activado).
...
- **Estado:** ✅ COMPLETED (2026-08-10) — Decisión (A): unificación en `reviews/`. Escritores (`audit-all.ps1:20`, `commands/audit.md:141`, `prompts/audit-full.md:194`) redirigidos a `docs/dev/reviews/` (commit `1732d6b7`); lectores (`status.md:24`, `progreso/SKILL.md` Trigger 4, `check-avance-coverage.ps1:16`, AGENTS/manual/fuentes-vivas/INDEX) apuntan a `docs/dev/reviews/` (commit `aad261e4`); `docs/audit-reports/` eliminado; solo quedan refs históricas (plan/registro/blog/extracción).
...
- **Archivos clave (refs rotas verificadas):**
  - `.opencode/AGENTS.md:783` → `docs/dev/plans/2026-08-06-oc-vantadb-pro.md` (movido a `archive/`)
...
- **Esfuerzo:** 🟢 | **Prioridad:** P2 | **Ruta:** vanta-lead
- **Archivos clave:** `docs/dev/plans/archive/2026-08-10-p3-remaining-fallas.md` — header `✅ COMPLETADO` con commits, pero 6 tasks `⬜ PENDING`
...
- **Archivos clave:** `docs/dev/research/2026-08-10-agent-engineering/REPORTE-FINAL.md` §3.3/§3.5/gap-02
...
- **Estado:** ✅ COMPLETED (2026-08-10) — items §3.3/§3.5/gap-02 llevados a `docs/dev/Backlog.md` §P17 (TSYS-01..08) + fallas persistidas en `docs/progreso/p3-remaining-fallas.md` (commit `d4bf5b2c`).
...
- **Archivos clave (duplicados byte-a-byte confirmados):**
  - `docs/progreso/README.backup-2026-08-03.md` ≡ `docs/dev/avance/historial/snapshot-2026-08-03.md` (3320 líneas, 0 diff)
  - `docs/dev/avance/historial/backlog-history.md` ≡ `docs/progreso/BACKLOG_HISTORY.md` (SHA-256 idénticos)
  - `docs/dev/avance/historial/archivo-historico.md` ≡ `docs/progreso/ARCHIVO_HISTORICO.md` (486 líneas, 0 diff)
  - `docs/dev/avance/historial/sdk-gap-audit-2026-07-28.md` ≡ `docs/progreso/2026-07-28-sdk-gap-audit.md` (SHA-256 idénticos)
...
- **Gate Result:** 🔵 DO
- **Contrato:** los mirrors vigentes pasan a link (`> Ver: docs/progreso/<archivo>`) o a checker de hash en `check-avance-coverage.ps1`; eliminar `README.backup-2026-08-03.md` junto con actualización de `scripts/check-avance-coverage.ps1:24` y `docs/dev/avance/COBERTURA.md:19`; snapshots congelados se conservan (historial).
...
- **Notas:** `docs/dev/Backlog.md:54,376` cita `snapshot-2026-08-07.md` como evidencia — NO borrar snapshots.
