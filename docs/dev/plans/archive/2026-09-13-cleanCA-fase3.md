...
> `docs/dev/architecture/BOUNDARIES.md` (BND-01…08) + §10 Adaptador Fase 1/2 (sin campaign MCP para
...
- Appetite 4h · 🟢 · 🔴 Alta · Archivos clave: `docs/dev/reviews/dsm-baseline-slim.json`, `docs/dev/architecture/BOUNDARIES.md` (BND-03), `docs/dev/architecture/ARCHITECTURE.md`, `src/accumulator.rs` (solo lectura)
...
- Contrato: (1) re-medición same-tool M1 post-Fase-2 archivada en `docs/dev/reviews/` con tabla D antes/después de `storage/engine` y `sdk` (D debe bajar vs baseline: 26→24→20 campos); (2) excepción `GraphAccumulator↔new` firmada en BOUNDARIES/BND-03 como artefacto del analizador (decisión humana Q2); (3) revisión vanta-arch de A1 firmada en el doc (cierra el "pendiente del lead" de C2A1). Todo verificable por archivo.
- Task file `docs/dev/tasks/F3G.md` · ✅ COMPLETED · Ruta vanta-worker (con revisión vanta-arch para la firma A1).
...
- Dependencias: ninguna (primera). Impacto: docs + `docs/dev/reviews/`; cero `src/` (salvo lectura). Habilita la cotización de la mudanza.
...
- Task file `docs/dev/tasks/F3X.md` · ✅ COMPLETED · Ruta vanta-arch (diseño + ADR-datos) → vanta-worker (slices).
...
- Task file `docs/dev/tasks/F3C.md` · ✅ COMPLETED · Ruta vanta-arch (diseño + ADR-datos) → vanta-worker (slices por dominio).
...
- Appetite 4h · 🟢 · 🟡 · Archivos clave: `benches/canonical_p99.rs`, `docs/user/operations/BENCHMARKS.md`, `.github/workflows/` (nuevo job), `Cargo.toml` (features del bench)
...
- Task file `docs/dev/tasks/F3B.md` · ✅ COMPLETED · Ruta vanta-lead (CI) + vanta-worker (bench).
