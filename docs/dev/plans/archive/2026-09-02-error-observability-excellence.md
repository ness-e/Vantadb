...
> **Inicio:** 2026-09-02
> **Estado:** ⏳ EN PROGRESO
> **Fuente:** `docs/dev/Backlog.md` (121 activas) + investigación multi-agente 2026-09-02 (4 audits paralelos read-only) + research internet 10 fuentes (Rust thiserror/anyhow, Python Real Python/docs.python.org, TS Convex, Vanta standardized errors)
> **Autonomous:** false
...
- **Progreso:** docs/dev/avance/activo/{core-engine,bindings,web-frontend}.md OK
...
- **DoD multi-nivel:** Task: `Backtrace` + `tracing::error!` + `metrics` + `catch_unwind` · Commit: `feat(observability): Backtrace + tracing + metrics + catch_unwind (ERR-OBS-01)` · Release: `docs/user/operations/OBSERVABILITY.md` + `CONSTRAINTS.md` quality bar
- **Validación Appetite vs Effort:** max 1d ≥ 🟡 1d ✅
- **Estado:** ✅ COMPLETED (2026-09-02, vanta-worker)
...
- **Desarrollar, investigar, analizar y aplicar en cada tarea:** investigar = codegraph + internet (fuentes citadas), analizar = blast radius + causa raíz (systematic-debugging), desarrollar = codificar, aplicar = commit verde + `skill progreso` (Trigger 1: elimina fila Backlog, migra a `docs/dev/avance/<dominio>`, `campaign_memory_write`).
