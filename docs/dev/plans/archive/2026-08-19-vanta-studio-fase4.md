...
> **Fuente:** auditoría multi-agente 2026-08-19 (4 sub-agentes read-only: research original `docs/dev/research/human-facing-db-ui/` + Fases 0/1 + Fases 2/3 + cross-check git/registro) → gaps consolidados en el digest del lead; decisiones del usuario 2026-08-19 (ver Decisiones).
...
- `docs/dev/Backlog.md` — migración/estados la hace el lead (tarea DOC-01 = lead, no sub-agente)
- `web/` (Next.js marketing site) — NO es la consola; no tocar
- `docs/dev/plans/2026-08-19-web-design-audit.md` + `SKILLS-MANIFEST.md` — workstreams ajenos, no commitear
...
> **Rutas:** `docs/dev/Backlog.md`, `.opencode/skills/campaign-executor/tasks/*.md`, `docs/progreso/README.md`, `docs/dev/avance/activo/*.md`, `docs/dev/plans/archive/2026-08-18-vanta-studio-fase{0,1}.md`. Sin código. Ejecutada por el **lead** (Backlog/plan/progreso son del lead) — no se delega a sub-agente.

### Task 1: DOC-01 — Reconciliar Backlog P26 (18/19 filas obsoletas) + línea resumen
- **Archivos clave:** `docs/dev/Backlog.md` (L53 línea resumen "🔴 Alta (Fase 2: grafo R3F)" → actualizar; L660-678 tabla P26: filas VS-01..11 + VS-CORE-01..07)
...
- **Verificación:** `rg -n "Pendiente" docs/dev/Backlog.md | rg "VS-|WEB-|GRAFO-|ESPACIO-|OP-"` → solo las verdaderamente pendientes (VS-* sin implementar: revisar VS-01 Tailwind); tabla coherente con progreso.
- **Estado:** ⏳ PENDING
...
- **Verificación:** `rg -l "IN PROGRESS|PENDING" .opencode/skills/campaign-executor/tasks/` → solo tareas verdaderamente activas; `rg "WEB-001" docs/dev/Backlog.md` coherente.
- **Estado:** ⏳ PENDING

### Task 3: DOC-03 — Fase 0 en registro canónico + plan F0 archivado + hash F1 corregido
- **Archivos clave:** `docs/progreso/README.md` (sección Detalle), `docs/dev/avance/activo/desktop.md`, `docs/dev/plans/2026-08-18-vanta-studio-fase0.md` → mover a `docs/dev/plans/archive/`, `docs/dev/plans/archive/2026-08-18-vanta-studio-fase1.md` (L5)
...
- **Archivos clave:** `docs/progreso/README.md` (cabecera L5/L11/L21 fecha ~2026-08-02/~95% vs contenido 2026-08-19; L630 comentario muerto "movido a ARCHIVO_HISTORICO.md"), `docs/dev/plans/archive/2026-08-18-vanta-studio-fase3.md` (L102 WEB-06 ⏳ PENDING vs header 7/7; L5 "7 commits" lista 8 hashes), `docs/dev/avance/activo/desktop.md` (header "Fase 1" agrupa Fase 2)
...
- **Próximo:** ejecutar `/pipeline run docs/dev/plans/2026-08-19-vanta-studio-fase4.md` — Wave 0 (reconciliación documental, lead) antes de delegar Wave 1.