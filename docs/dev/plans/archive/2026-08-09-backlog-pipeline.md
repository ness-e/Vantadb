...
> **Inicio:** 2026-08-09
> **Estado: completed
> **Fuente:** `docs/dev/Backlog.md` (backlog activo completo)
...
- **Esfuerzo:** 🔴 · **Prig:** 🟡 · **Archivos:** `src/pitr.rs`, `src/storage/wal.rs`, `src/lib.rs` (`pitr` feat), ADR en `docs/dev/architecture/adr/`
...
- **Estado:** ✅ COMPLETED
- **Ejecutado:** `418bc5bb` — Documented per-feature status (29 features, EXPERIMENTAL flags + dead/no-op `wasm` marker) in docs/dev/architecture/FEATURES.md

### Task 33: FEAT-06 — Config hot-reload JSON + config.toml

- **Esfuerzo:** 🟡 · **Prig:** 🟢 · **Archivos:** `src/config.rs:1313`, `docs/user/operations/CONFIGURATION.md` | **DO** (doc formato real) | **Contrato:** CONFIGURATION.md describe builder/env y JSON hot-reload (si existe) · not invent config.toml
- **Estado:** ✅ DONE
...
- **Esfuerzo:** 🟢 · **Archivos:** `docs/user/QUICKSTART.md`, `README.md` | **DO** · **Contrato:** `rg "0.4" docs/user/QUICKSTART.md README.md` → no match (except history)
- **Estado:** ✅ DONE
...
- **Esfuerzo:** 🟢 · **Archivos:** `docs/api/PYTHON_SDK.md`, `docs/user/QUICKSTART.md` | **DO** · **Contrato:** sección "ID limits" existe
- **Estado:** ✅ DONE
- **Ejecutado:** 724e6d53 — sección "ID limits" añadida a `docs/api/PYTHON_SDK.md` y `docs/user/QUICKSTART.md`; límite real verificado en binding: u128 nativo (pyo3 0.29), IDs > u64::MAX soportados como ints Python sin truncación (ERR-023); OverflowError fuera de rango, string solo en `recover_archived_nodes()`, precaución JSON 2^53

### Task 47: DOC-07 — Documentar hot-reload JSON / config.toml

- **Esfuerzo:** 🟡 · **Archivos:** `docs/user/operations/CONFIGURATION.md` | **DO** (con FEAT-06) · **Contrato:** sección config-examples
- **Estado:** ✅ DONE
...
- **Resultado:** ✅
- **Próxima acción:** migrar `docs/dev/Backlog.md` → `docs/progreso/` (skill progreso Trigger 1) + reporte final al usuario
