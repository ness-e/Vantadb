...
> **Fuente:** docs/dev/Backlog.md (105 activas, verificado contra código el 2026-09-04)
> **Autonomous:** false
...
- **Contrato:** `npm pack` + prepublish artifacts OK + `npm publish --dry-run` verde + checklist de release escrita (`docs/dev/plans/artifacts/bnd-08-publish-checklist.md`). PROHIBIDO publicar.
...
- **Archivos clave:** `src/wal.rs:305-389` (`append`, `batch_append` existentes), `src/config.rs` (SyncMode), nueva spec en `docs/dev/architecture/adr/` o `docs/dev/plans/artifacts/fut-12-spec.md`
...
- **Task file:** `tasks/BND-09.md` (`docs/dev/tasks/BND-09.md`)
...
- **Archivos clave:** `src/storage/engine/mod.rs:637-796` (`create_snapshot` + `snapshot_restore` + failpoint EXISTEN — verificado hoy), `docs/dev/research/res02-backup-restore.md` §3, `vantadb-mcp/src/handlers/tools.rs`
...
Contrato: verificacion: fmt/check/clippy/deny/docs-coverage(0 gaps)/package-list/test-mcp.py 4/4/nextest 86-86 + mcp_tests 91-91, todos exit 0; evidencia: claim 72 tests->91/91 pass | claim 37 checks->4/4 disco+pass | OOM-1455->retry -j2 verde | teardown-hang->harness stderr-drain fix + 4/4 exit 0; artefactos: d0bb4e91, docs/dev/tasks/STABLE-04.md; invariantes: 0 cambios producto, publish=false intacto, ajenos sin stagear; deuda: deny warnings pre-existentes fuera de blast radius; queda_pendiente: orquestador push + Wave 3
