...
> **Fuente:** docs/dev/plans/2026-09-10-anti-stutter.md (auto-fuente, investigación S0-S6) + verificación `rg`/`codegraph` 2026-09-10
> **Autonomous:** false
...
- **Prioridad:** 🔴
- **Ruta:** vanta-lead
- **Archivos clave:** `scripts/anti_stutter_map.json`, `docs/dev/architecture/adr/NNN_anti_stutter.md`, `src/lib.rs`, `vantadb-ts/src/types.ts`, `vantadb-python/vantadb_py/__init__.py`
...
- **Gate Result:** ✅ DO
- **Contrato:** `test -f scripts/anti_stutter_map.json && test -f docs/dev/architecture/adr/*anti_stutter*.md`
- **Task file:** `docs/dev/tasks/AST-001.md`
- **Estado:** ✅ COMPLETED
- **Branch:**
...
- **Contrato:** `cargo check -p vantadb && cargo clippy -p vantadb --all-targets --all-features -- -D warnings && cargo fmt --check -p vantadb`
 - **Task file:** `docs/dev/tasks/AST-002.md`
 - **Estado:** ✅ COMPLETED
 - **Branch:**
...
- **Task file:** `docs/dev/tasks/AST-003.md`
- **Estado:** ✅ COMPLETED
- **Branch:**
- **Commit:**
...
- **Contrato:** `npx tsc --noEmit -p vantadb-ts/ && npm test --prefix vantadb-ts`
 - **Task file:** `docs/dev/tasks/AST-004.md`
 - **Estado:** ✅ COMPLETED
 - **Branch:**
...
- **Task file:** `docs/dev/tasks/AST-005.md`
- **Estado:** ✅ COMPLETED
- **Branch:**
...
- **Contrato:** `rg -n "VantaMemoryRecord|VantaConfig|VantaSearchHit|search_memory|get_memory" docs/ README.md llms.txt | wc -l` → `0` (salvo `VantaHeader`/wire comentados)
 - **Task file:** `docs/dev/tasks/AST-006.md`
 - **Estado:** ✅ COMPLETED
 - **Branch:**
...
- **Task file:** `docs/dev/tasks/AST-007.md`
- **Estado:** ✅ COMPLETED
- **Branch:**
- **Commit:** chore: AST-007 release gate major + cero-remanentes (semver 15/0 + deny + verify 3143✅)
  ...
  **Notas:**
  ...
  - **Cynefin:** 🟨 complicado.
  - **Top 3 riesgos:** ver register.
  - **Uphill/Downhill:** ⬆️ 0 / ⬇️ release + retrospectiva + archivar plan en `docs/dev/plans/archive/`.
...
Contrato: Contrato: npx tsc --noEmit -p vantadb-ts/ (0 err, corrido desde pkg dir por resolucion npx) + tsc d.ts standalone 0 err + npm test 280/280 ✅. Evidencia: commits 07406d9f + 7d18cbb4. Artefactos: vantadb-ts/src/{types,errors,vantadb,guards,metadata,native}.ts, vantadb-wasm d.ts, docs/dev/tasks/AST-004.md. Invariantes: wire shapes intactos; VANTADB_* codes; this.name=VantaError; nada fuera de vantadb-ts/wasm. Deuda: ninguna. Queda_pendiente: AST-005 (metodos Rust, vanta-worker).
