...
> **Fuente:** `docs/dev/Backlog.md` (130 activas post-sync 2026-09-01) + `docs/dev/reviews/archive/auditoria-documentacion-2026-08-21.md` (D1-D14) + `docs/dev/research/tdam/SYNTHESIS.md` (F1-F7) + `docs/dev/research/archive/res02-backup-restore.md` (RES-01/02)
...
| **Wave4** | P27 F5-F7 MEM-22..38 (17) + GOV-D1..D6+E1+F1+F2 (10) + RES-11..15+DEC-02 (6) | P27 F5-F7 + GOV D/E/F + P38 baja/proceso | `vanta-memory/src/offload/**`, `vanta-proxy/**`, `docs/dev/research/wiki/**`, `src/storage/engine/mod.rs` snapshot quiesce, `docs/dev/avance/**` | Resto + transversales + decisiones producto; SOLO para MEM-16 (pipeline 1218L) y MEM-22 (assemble 1194L) dentro de wave4a/b |
...
- **Archivos clave:** `src/wal.rs`, `src/storage/engine/mod.rs:507,540`, `docs/dev/research/archive/res02-backup-restore.md` §3 S1-S2, `docs/dev/research/ACID_ROLLBACK_DESIGN.md`
...
- **Archivos clave:** `dev-tools/validate_doc_snippets.py`, `docs/user/tutorials/*.md`, `docs/api/EMBEDDED_SDK.md`
...
Contrato: `python dev-tools/validate_doc_snippets.py` → Summary: 27 PASS, 0 FAIL, 27 SKIP ✅ + `Select-String -Path docs/user/QUICKSTART.md -Pattern hit\.key` ≥1 ✅ + `Select-String vanta-skip` en migrators ≥1 ✅
Invariantes: No tocar GOV-A3 (vanta-cli) ni RES-02 (wal/engine) — disjoint preservado, docs-only, 0 archivos Rust
Comandos de verificación: `python dev-tools/validate_doc_snippets.py` → 0 FAIL + `Select-String -Path docs/user/QUICKSTART.md -Pattern "hit\.key"` Count 1 + `Select-String -Path docs/user/tutorials/03-migrating-from-chromadb.md -Pattern vanta-skip` Count 1
...
- **Archivos clave:** `src/wal.rs` (WAL v2 Prepare), `src/storage/engine/mod.rs:514,543,609,656,752`, `src/storage/engine/maintenance.rs:36` (flush), `src/storage/vfile.rs`, `src/storage/vfile_mmap.rs`, `docs/dev/research/archive/res02-backup-restore.md` §3 S1
...
- **Archivos clave:** `benches/scores_semantics.rs`, `benches/common/mod.rs` (apply_fixed_profile), `Cargo.toml` [[bench]] scores_semantics, `src/api/scores.rs` (RES-04), `docs/user/operations/BENCHMARKS.md` §9, `benchmarks/*` (python harness disjoint)
...
- **Contrato:** `Test-Path benches/scores_semantics.rs` True AND `Select-String benches/scores_semantics.rs "rrf_contribution|cosine_distance" | Measure Count` >=3 AND `Select-String Cargo.toml 'scores_semantics' | Measure Count` >=1 AND `Select-String docs/user/operations/BENCHMARKS.md "scores_semantics|Score Semantics" | Measure Count` >=1 AND `cargo bench -p vantadb --bench scores_semantics --no-run` Finished (Executable) AND `cargo check -p vantadb` Finished
- **Task file:** `.opencode/skills/campaign-executor/tasks/RES-05.md`
- **Estado:** ✅ COMPLETED
- **last-synced:** 2026-09-02T23:50

=== RECITATION ===
Objetivo activo: RES-05 — Benchmark semántica scores (benches/scores_semantics, docs/user/operations/BENCHMARKS.md §9)
Estado: completed (desde: pending)
Última acción: DISCOVERY Read benchmarks/* + BENCHMARKS.md 308L + src/api/scores.rs 109L + grep SKILLS-MANIFEST benchmark/scores/bench/canonical_p99 (1 hit vantadb) + Cargo.toml benches 22 entries + canonical_p99.rs 133L reuse pattern → EJECUCIÓN crear task file RES-05.md + implementar bench semántica minimal benches/scores_semantics.rs (6 micro-benches batch 10k reuse common::apply_fixed_profile, ponytail pure f32 O(1) xorshift determinístico) + Cargo.toml [[bench]] scores_semantics + docs/user/operations/BENCHMARKS.md §9 table expected ~ns/op — verify cargo bench --no-run 45.7s Executable + cargo check Finished
Resultado: ✅
State: COMPLETED (desde: PENDING)
...
Comandos de verificación: `cargo check -p vantadb` → Finished + `cargo check --bench scores_semantics -p vantadb` → Finished + `cargo bench -p vantadb --bench scores_semantics --no-run` → Executable + `Select-String benches/scores_semantics.rs rrf_contribution|cosine_distance` Count 12 + `Select-String docs/user/operations/BENCHMARKS.md scores_semantics` Count 5
...
Invariantes: No tocar docs/user/tutorials/* docs/user/glosario/* (GOV-B3/B4 disjoint) ni src/planner.rs (MEM-01) — dominio vantadb-mcp/search only; Hyrum {}→None Hybrid default documentado; bounds protegen OOM
...
- **Archivos clave:** `docs/user/operations/DISASTER_RECOVERY_RUNBOOK.md:142,233,266`, `src/cli.rs:130-144`
- **Gate Justificación:** Addendum elevó a 🔴 addendum 3.4.2 — falla cuando se necesita; D4a docs ya
- **Contrato:** `Select-String -Path "docs/user/operations/DISASTER_RECOVERY_RUNBOOK.md" -Pattern "restore --dry-run|doctor --fix" | Measure-Object Count` ==0
...
Comandos de verificación: `Select-String -Path "docs/user/operations/DISASTER_RECOVERY_RUNBOOK.md" -Pattern "restore --dry-run|doctor --fix" | Measure-Object Count` (0) + `cargo check -p vantadb` (Finished 29s)
...
- **Archivos clave:** `docs/dev/Backlog.md:703-705`, `docs/dev/plans/archive/*p29*/*p30*`, `docs/dev/plans/2026-08-22-vanta-final-cierre.md`
- **Gate Justificación:** SYNC-03 single source of truth roto
- **Contrato:** `Select-String -Path "docs/dev/Backlog.md" -Pattern "MEM-43.*✅" | Measure-Object Count` >=1
...
Invariantes: No tocar .config/nextest.toml (GOV-C1 disjoint) ni docs/master-index.md (GOV-C4) ni docs/dev/Backlog.md purga (ya Nota 2026-08-22) — dominio docs/operations + dev-tools only; ponytail docs-only, full restore ya validado GOV-A3
Comandos de verificación: `Select-String -Path "docs/user/operations/DISASTER_RECOVERY_RUNBOOK.md" -Pattern "Daily Backup Verification" | Measure-Object Count` (4) + `Select-String -Path "dev-tools/verify.ps1" -Pattern "Daily Backup Verification" | Measure-Object Count` (2) + `cargo check -p vantadb` (Finished)
Deuda: ninguna — purga audit-reports ya cerrada como mención textual Nota GOV-C3 2026-08-22 docs/dev/Backlog.md:200; guard daily backup ceiling ponytail: fast gate docs-only, full restore+doctor si heavy gate (GOV-A3)
...
Invariantes: No tocar src/wal.rs, src/storage, src/vector, docs/user/operations/** (GOV-C6) — disjoint 100% preservado; RRF_K=60, cosine pass-through ERR-028 (no fallback silencioso), bench pure f32 O(1) reuse canonical_p99; ponytail TS bench dedicado skip (core bench cubre semántica, TS thin glue)
...
Resultado: ✅
State: COMPLETED (desde: COMPLETED)
Próxima acción: Wave3 continúa — MEM-11 (L1 dedup) + GOV-C6/RES-06 paralelos MAX 3, disjoint preservado (no docs/user/operations/CONFIGURATION.md ni docs/api)
...
- **Archivos clave:** `docs/user/operations/master-index.md`, `docs/master-index.md`, `docs/user/operations/**` (35 .md)
...
- **Contrato:** `Select-String -Path "docs/user/operations/master-index.md" -Pattern "hardening|UPGRADE" | Measure-Object Count` >=2 AND `Select-String -Path "docs/user/operations/master-index.md" -Pattern "last_reviewed.*2026-09-02" | Measure-Object Count` >=1 AND `Get-ChildItem docs/user/operations/*.md | Measure-Object Count` == md-leaves indexados (35==35) AND `cargo check -p vantadb` Finished AND `Select-String -Path "docs/master-index.md" -Pattern "audit-reports/" | Measure-Object Count` ==0
...
Estado: completed (desde: in-progress)
Última acción: DISCOVERY Read docs/master-index 370L + operations/master-index 57L + Get-ChildItem 35 vs indexed 32 → faltan hardening.md + UPGRADE.md + self + last_reviewed 2026-08-29 → EJECUCIÓN ponytail fix taxonomía (3 rows + header date) docs-only 5 líneas, docs/user/operations/master-index.md → VERIFY 35==35 + audit-reports/ 0 + cargo check Finished + task file + plan sync
Resultado: ✅
State: COMPLETED (desde: IN_PROGRESS)
Próxima acción: Wave3 continúa — MEM-08 (vanta-memory L0-L3) + RES-06 (scores drift) paralelos MAX 3, disjoint src/* preservado
Contrato: `Select-String docs/user/operations/master-index.md hardening|UPGRADE` 3 ≥2 ✅ + `Select-String last_reviewed 2026-09-02` 1 ≥1 ✅ + `Get-ChildItem 35 == indexed md 35` ✅ + `cargo check -p vantadb` Finished 0.43s ✅ + `Select-String docs/master-index.md audit-reports/` 0 ✅ + `cargo check --workspace` (no src tocado)
...
Comandos de verificación: `Get-ChildItem docs/user/operations/*.md | Measure Count` (35) + `Select-String -Path "docs/user/operations/master-index.md" -Pattern "hardening|UPGRADE"` (3) + `Select-String -Path "docs/user/operations/master-index.md" -Pattern "last_reviewed.*2026-09-02"` (1) + `Select-String -Path "docs/master-index.md" -Pattern "audit-reports/"` (0) + `cargo check -p vantadb` (Finished)
...
- **Archivos clave:** `docs/user/operations/master-index.md`, `docs/user/operations/*.md` (35 files), `docs/master-index.md`
...
- **Contrato:** `Get-ChildItem docs/user/operations/*.md | Measure Count` 35 == indexed md 35 (filtrado archive) AND `Select-String master-index.md last_reviewed.*2026-09-02` >=1 AND `Select-String Taxonomía|Deploy|Durability|Performance|Security|CI` >=3 AND `cargo check -p vantadb` Finished
...
Comandos de verificación: `Get-ChildItem docs/user/operations/*.md | Measure Count` (35) + `Select-String docs/user/operations/master-index.md "\[.*\.md\]" | Measure Count` (37 inc archive, 35 fs) + `Select-String last_reviewed 2026-09-02` (1) + `Select-String Taxonomía` (15) + `cargo check -p vantadb` (Finished 21.25s)
...
- **Archivos clave:** `docs/user/operations/CONFIGURATION.md`, `src/config.rs:299,659`, `src/llm.rs:40,132,147`
- **Gate Justificación:** drift operador dimensiona capacidad falso
- **Contrato:** `Select-String -Path "docs/user/operations/CONFIGURATION.md" -Pattern "rate_limit_rpm.*600" | Measure-Object Count` >=1 AND grep env vars 0 drift
...
Comandos de verificación: `Select-String -Path "docs/user/operations/CONFIGURATION.md" -Pattern "rate_limit_rpm.*600" | Measure-Object Count` (1) + `Select-String -Path "docs/user/operations/CONFIGURATION.md" -Pattern "last_reviewed.*2026-09-02" | Measure-Object Count` (1) + `cargo check -p vantadb` (Finished 0.75s)
...
Estado: completed (desde: in-progress)
Última acción: DISCOVERY Read Backlog header 16L (121 activas, previo 130 post-sync gap regex 0) + ROADMAP header 14L (~45 cifra drift) + ops master-index 35/35 ✅ + SKILLS-MANIFEST grep operations/taxonomia 0 hits → EJECUCIÓN ponytail fix docs-only 2 líneas (Backlog.md:16 "130 post-sync"→"130 activas post-sync" + ROADMAP.md:14 "~45 items abiertos (conteo real...)"→"ver docs/dev/Backlog.md para conteo actual (regla sync...)") → VERIFY Select-String Backlog 130 activas 1≥1 + ops hardening 3≥2 + last_reviewed 1≥1 + 35==35 + audit-reports 0 + cargo check Finished 1.70s
Resultado: ✅
State: COMPLETED (desde: IN_PROGRESS)
...
- **Archivos clave:** `docs/dev/Backlog.md` P24, `docs/dev/research/investigacion-equipo-2026-08-09.md` §roadmap
- **Gate Justificación:** 3 gaps sin filas, validados con archivo:línea
- **Contrato:** `Select-String -Path "docs/dev/Backlog.md" -Pattern "WAL async ingest|query planner|DiskANN.*disk" | Measure-Object Count` >=3
...
- **Archivos clave:** `docs/dev/architecture/adr/ADR-03*.md`, `docs/api/VANTA_MEMORY.md`
...
- **Archivos clave:** `docs/dev/avance/activo/*`, `docs/dev/avance/meta.md`
- **Gate Justificación:** mirror roto congelado 20/08; D9 avance domina; depende GOV-C2
- **Contrato:** `Test-Path docs/dev/avance/activo/vanta-memory.md` == true AND `Select-String -Path "docs/dev/avance/meta.md" -Pattern "vanta-memory" | Measure-Object Count` >=1
...
- **Archivos clave:** `docs/Investigaciones/**` → `docs/dev/research/`
...
- **Archivos clave:** `docs/dev/reviews/propuesta-limpieza-artefactos-2026-09-02.md`
- **Gate Justificación:** D12 proponer sin ejecutar; requiere aprobación separada
- **Contrato:** `Test-Path docs/dev/reviews/propuesta-limpieza-artefactos-2026-09-02.md` == true
...
- **Contrato:** `Test-Path docs/dev/reviews/auditoria-raiz-publica-2026-09-02.md` == true AND `Select-String -Path "docs/dev/reviews/auditoria-raiz-publica-2026-09-02.md" -Pattern "finding|Severidad" | Measure-Object Count` >=2
...
- **Contrato:** `Test-Path docs/dev/reviews/auditoria-zonas-intocadas-2026-09-02.md` == true
...
- **Archivos clave:** `docs/api/*.md`, `docs/user/tutorials/*.md`
...
- **Archivos clave:** `.opencode/rules/*.md`, `docs/dev/Backlog.md`, `meta-001-root-cause-analysis.md`
...
- **Archivos clave:** `docs/dev/architecture/adr/ADR-0XX-session-layer.md`, `docs/dev/research/res03-session-layer-gonogo.md`, `COGNEE_EVALUATION.md`
- **Gate Justificación:** DEC-01 huérfana ya research-resuelta 2026-08-25; solo doc, no código
- **Contrato:** `Test-Path docs/dev/architecture/adr/ADR-0*session*.md` == true AND `Select-String -Path "docs/dev/architecture/adr/ADR-0*session*.md" -Pattern "defer-as-scoped" | Measure-Object Count` >=1
...
- **Archivos clave:** `docs/dev/research/tdam/SYNTHESIS.md` §9, `tdam/09-deploy-usage.md`, `docs/dev/architecture/adr/ADR-0XX-billing.md`
- **Gate Justificación:** TDAM #9 diferido fuera F1-F7 nunca trackeado; decisión previa a MEM-27 proxy/Pro
- **Contrato:** `Test-Path docs/dev/architecture/adr/ADR-0*billing*.md` == true AND `Select-String -Path "docs/dev/architecture/adr/ADR-0*billing*.md" -Pattern "÷1000|÷10000|CreditCalculator" | Measure-Object Count` >=1
...
- **Reapertura (6):** RES-07/08/09/12/15 + DEC-02 marcadas ✅ sin recitación ni evidencia (estampilla masiva `T00:00`) — verificadas contra código/filesystem, reabiertas ⬛; sus filas de `docs/dev/Backlog.md` quedan activas. Ejecución futura: `/pipeline task RES-07|RES-08|RES-09|RES-12|RES-15|DEC-02`.
...
plan-adjust [2026-09-02]: creación inicial D — 86 DO / 0 DEFER / 0 SKIP / 0 BLOQUEADO
  fuente: docs/dev/Backlog.md 130 activas + GOV D1-D14 + tdam SYNTHESIS F1-F7 + res02 S1-S5 + EMB-01..09 ✅ + incidente MCP-35 2026-08-25
  ...
  nextAction: /pipeline run docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md
```
...
`skill progreso` → migrar filas completadas a `docs/dev/avance/` por dominio + historial + `campaign_memory_write` (lessons/decisions) → recitation → próxima wave. Ponytail: skipped over-engineering (4 servicios split, Redis, Mongo, @colbymchenry/codegraph, prompts Kenty chino, 3 imágenes Docker), add when benchmark/ADR lo exija.
...
NextAction: `/pipeline run docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md` — Wave0 (MCP-35 + GOV-T01..T03 + RES-01 SOLO) con 3 agentes vanta-arch/worker/docs en paralelo
Result: ⬜ PENDING
...
Contrato: verificacion: cargo check -p vanta-memory Finished + cargo test --test l1_extractor 9 passed + cargo test --lib extractor 3 passed + cargo test --lib 328 passed + cargo fmt --check 0 ✅ | evidencia: claim: L1 extractor reuse 0 lineas ya landed 260L + parsers + prompts 73/73 tests | evidencia: vanta-memory/src/core/record/l1_extractor.rs, vanta-memory/tests/l1_extractor.rs, vanta-memory/src/offload/local_llm/parsers/json_utils.rs, confianza: alta | artefactos: docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md (recitation MEM-10), .opencode/skills/campaign-executor/tasks/MEM-10.md | invariantes: L1 LLM-optional, prompts ingles, 1 call l1-extraction, split bg/new, parse tolerante, legacy type normalize, sin unwrap, records vacio MEM-11, disjoint GOV-C6/RES-06 | deuda: ninguna | queda_pendiente: MEM-11 dedup Wave3
...
Contrato: verificacion: Select-String CONFIGURATION.md rate_limit_rpm.*600 1≥1 ✅ + Select-String last_reviewed.*2026-09-02 1≥1 ✅ + Select-String Outside 5≥1 ✅ + cargo check -p vantadb Finished ✅ | evidencia: claim: rate_limit 600 en doc + sweep 44 vars 0 drift + 5 Outside VANTA_EMBEDDING_PROVIDER/OPENAI/REPORTED_VERSION/BACKUP_DIR evidencia: docs/user/operations/CONFIGURATION.md:45 + src/config.rs:659 + src/llm.rs:40 confianza: alta | artefactos: docs/user/operations/CONFIGURATION.md, docs/dev/plans/2026-09-02-alta-prioridad-paralelo.md | invariantes: No tocar src/ (disjoint RES-06 vantadb-ts/MEM-11 vanta-memory) — dominio docs/operations only | deuda: ninguna | queda_pendiente: MEM-11 L1 dedup Wave3 paralela
