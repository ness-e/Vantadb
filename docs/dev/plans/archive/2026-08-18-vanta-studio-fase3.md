...
- `docs/dev/Backlog.md` — migración la hace el lead
...
- **Archivos clave:** `desktop/scripts/selfcheck-web-e2e.ts` (nuevo, Playwright), `docs/dev/architecture/` (ADR), `docs/dev/Backlog.md` (estado P25/DESKTOP)
...
- **Contrato:** e2e: arranca server con DB temp + dist-web, Playwright navega `http://127.0.0.1:8080/dashboard/`, verifica HOME con datos, ingesta un registro, aparece en grid, se edita, se borra (con undo), search híbrida devuelve hits; ADR en `docs/dev/architecture/` (o `docs/dev/plans/` si no existe dir) documentando REST completo + sin-auth-loopback + WASM diferido a Fase 4; Backlog: P25 nota actualizada, tasks WEB-00..06 registradas como completadas.
