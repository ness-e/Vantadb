...
5. Guardar en `docs/dev/plans/` y pedir aprobación (este archivo). Al completar → `docs/dev/plans/archive/`.
...
1. **Lookup:** verificar ID y estado en `docs/dev/tasks/` (+ `complete/`, `closed/`).
...
4. **Task file:** crear `docs/dev/tasks/<ID>.md` con las 4 fases de `prompts/task.md` (ver §4).
...
## 4. Pasos para la CREACIÓN del task file (`docs/dev/tasks/<ID>.md`)
...
Estado: task file docs/dev/tasks/B1.md fase 1; baseline: /cleanCA src/server/handlers.rs (A2 🔴).
...
Estado: docs/dev/tasks/B2a.md fase 1.
...
Contexto/Estado: tabla de B2a en docs/dev/tasks/B2a.md; empieza por wal.rs + engine.rs.
...
`/cleanCA` (sin parámetro) con 0 🔴, deuda 🟡 restante registrada con dueño, docs/api al día, ADRs escritos, plan archivado en `docs/dev/plans/archive/`.
...
> campaign, (2) hace `Read` de `docs/dev/tasks/<ID>.md` inexistente → error de herramienta,
...
2. **Existencia del task file SOLO con `glob`:** jamás `Read` de `docs/dev/tasks/<ID>.md`
   para ver si existe (Read de inexistente = error fatal). `glob docs/dev/tasks/<ID>.md`:
