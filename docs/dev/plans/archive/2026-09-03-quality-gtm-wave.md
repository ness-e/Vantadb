...
> **Fuente:** `docs/dev/Backlog.md` (123 activas post-paso0; 126 triadas + 3 purgadas con evidencia) + 4 audits de sesión 2026-09-02/03 + verificación código hoy
> **Autonomous:** false
...
```
Wave0 (paralelo 3, disjuntos):   RES-07 (config.rs+BENCHMARKS §nuevo) | GOV-TK1 (src/cli.rs+cli_handlers) | GOV-TK9 (docs/user/operations/checklist)
...
Wave2 (paralelo 3, disjuntos):   RES-12 (web/src/components/vanta/*.tsx) | MKT-18f (integrations/**+workflow nuevo) | RES-09 (docs/dev/Backlog.md filas P24)
...
- **Archivos clave (TODOS):** `src/config.rs:22` (`DEFAULT_RSS_THRESHOLD: f64 = 0.80`), `benches/memory_budget.rs` (default sizes `[10k,25k,50k,100k]` + delta RSS−logical), `docs/user/operations/BENCHMARKS.md` (nueva subsección en §memory-budget), `docs/dev/research/FND-01*` (origen F2)
...
- **Archivos clave (TODOS):** `src/cli.rs:154,380` (Doctor, dry_run), handler doctor en `src/cli_handlers/*`, `docs/user/operations/DISASTER_RECOVERY_RUNBOOK.md` §3.1 (insumo GOV-A3/C3), tests CLI en `tests/`
...
- **Contrato:** `rg -n -e "fix" src/cli.rs | rg -c "doctor|Fix|--fix"` ≥1 AND `vanta-cli doctor --fix` sobre DB temporal exit 0 con salida que liste reparaciones (o "nothing to fix") AND `rg -n "doctor --fix" docs/user/operations/DISASTER_RECOVERY_RUNBOOK.md` sigue ≥1 y es VERDAD ahora AND `cargo clippy --workspace --all-targets -- -D warnings` 0
...
- **Verificación real (cierre):** `webfetch github.com/vantadb/vantadb-examples` → 404; `webfetch github.com/ness-e/vantadb-examples` → 404; `rg "vantadb/vantadb-examples" docs/user/operations/ docs/api/` = 0; checklist:51 ahora TODO-humano explícito; fila Backlog eliminada (progreso)
...
- **Archivos clave (TODOS):** `Dockerfile` (existe, verificar user root), `docker-compose.yml` (servicio base; NO tocar compose multi-servicio — eso es MKT-18i), `.github/workflows/release-binaries-63.yml` (hook de build/push de imagen o decisión explícita de no publicar), `docs/user/operations/*deployment*` (documentar run unprivileged)
...
- **Archivos clave (TODOS):** `docker-compose.yml` (hoy: 1 servicio + volume), `docs/user/tutorials/*` (migración LanceDB ya existe — enlazar), README (bloque compose), `docker-compose.dev.yml` (no romperlo)
...
- **Estado:** ✅ COMPLETED (re-escalado por stop condition 2026-09-03: AnythingLLM sin soporte VantaDB — evidencia en `docs/dev/avance/activo/operaciones.md`; demo shipped VantaDB+Ollama, commit `abb6594c`; fila Backlog re-escalada a 🔴 upstream) | **Task file:** `.../tasks/MKT-18i.md` | **Ruta:** vanta-worker | **Branch:** develop
...
- **Archivos clave (TODOS):** `docs/dev/Backlog.md` sección P24, `docs/dev/research/investigacion-equipo-2026-08-09.md` §roadmap (fuente con archivo:línea)
...
- **Contrato:** `rg -n "FUT-1[234]" docs/dev/Backlog.md | Measure Count` ≥3 con descripciones re-escaladas: FUT-12 WAL fsync-batching, FUT-13 query planner optimizaciones, FUT-14 DiskANN disk-I/O real
- **Pre-mortem:** 1 los IDs FUT-12..14 chocan con filas existentes → verificar primero (`rg FUT-1 docs/dev/Backlog.md`)
...
- **Estado:** ✅ COMPLETADA 2026-09-03 (split hecho: `docs/dev/Backlog-negocio.md` 15 filas; técnico 123→107; BLOG-CTA queda técnico con criterio documentado; regla en `docs/dev/avance/meta.md`; contrato rg verde) | **Task file:** `.../tasks/RES-15.md` | **Ruta:** vanta-lead (docs/process) | **Branch:** develop
...
rg -n "FIND-22|PY-02|FIND-51" docs/dev/Backlog.md            # = 0 (purga efectiva)
```
...
Resultado: ✅
Próxima acción: Commit + fila Backlog SRV-07 eliminada + registro docs/dev/avance/activo/operaciones.md
