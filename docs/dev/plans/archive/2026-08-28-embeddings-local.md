...
|-----------|---------------|-----------------|
| Filosofía | BYO-vector — VantaDB no embebe (`docs/user/tutorials/05-embedding-integrations.md:11`) | BYO-vector **+** `embed-local` opt-in (no breaking, `EmbeddingProvider` nuevo) |
...
- `docs/user/tutorials/05-embedding-integrations.md:11` — "does not embed text for you"
- `docs/user/QUICKSTART.md:182` — "It is not yet exposed through the Python or TypeScript SDKs"
- `README.md:232` + `docs/user/operations/EXPERIMENTAL_FEATURES.md:60` — LLM/Ollama es "External optional integration (`llm.rs`, feature-gated)"
...
| **Hybrid** `src/search/` | Sí `query_vector`+`text_query` | BYO-vector RRF | query dim debe = docs dim (`docs/user/tutorials/05:126` "One model per namespace") |
...
**Regla de oro:** `docs/user/tutorials/05-embedding-integrations.md:126` — **un modelo por namespace** (misma dim para writes y query). Cross-model search rompe HNSW.

---

## 5. Backlog — 9 Tareas EMB (insertar en `docs/dev/Backlog.md` P38, tras `RES-15`)
...
**Descripción:** Nuevo `benchmarks/embed_bench.py` (ingest 1k docs EN+ES, QPS, recall@10, RSS, p50 embed) para 9 modelos (Qwen3 solo si `--include-exception` y GPU). Output `benchmarks/embed_bench_report.json` + sección `docs/user/operations/BENCHMARKS.md` EMB (comando reproducible `python benchmarks/embed_bench.py --models multilingual-e5-small,bge-m3 --dataset tiny-en-es-1k`).

**Archivos:** `benchmarks/embed_bench.py`, `benchmarks/embed_bench_report.json` (gitignored), `docs/user/operations/BENCHMARKS.md`
...
**Descripción:** Actualizar `docs/user/QUICKSTART.md:182` (quitar "not yet exposed"), `docs/user/tutorials/05-embedding-integrations.md:11,126` añadir sección `embed-local` (tabla 9 modelos, one-model-per-namespace), nuevo `docs/api/EMBEDDINGS.md` (API `EmbeddingProvider`, `LocalOnnxProvider`, env vars, `embeddings/` manifest). `README.md:232` mover `embed-local` de Experimental a Optional.

**Archivos:** `docs/user/QUICKSTART.md:182`, `docs/user/tutorials/05-embedding-integrations.md`, `docs/api/EMBEDDINGS.md` (nuevo), `docs/user/operations/EXPERIMENTAL_FEATURES.md:60`, `README.md:232`
...
| Dim mismatch por namespace | Alta | Alto | `docs/user/tutorials/05:126` + bench `namespace="bench-{model}"` + test `mismatched dim → error` |
...
- [ ] `docs/api/EMBEDDINGS.md` existe y `docs/user/QUICKSTART.md:182` actualizado
...
1. **Este plan** → `docs/dev/plans/2026-08-28-embeddings-local.md` (este archivo)
2. **Backlog** → insertar EMB-01..09 en `docs/dev/Backlog.md` P38 (sección dedicada)
...
**Refs:** `src/llm.rs:26,39,66`, `Cargo.toml:97,107`, `vanta-memory/Cargo.toml:38`, `src/physical_plan/vector.rs:51`, `docs/user/tutorials/05-embedding-integrations.md:11`, `docs/user/operations/EXPERIMENTAL_FEATURES.md:60`, `lessons.md:47,137`, `FUT-02`, `.gitignore:72`.
...
Estado: completed
Última acción: benchmarks/embed_bench.py (ingest 1k EN+ES, QPS, recall@10, RSS, p50 embed, 9 modelos 3/3/3, Qwen3 flag --include-exception, dummy deterministico fallback, numpy matmul fast path, ingest 1k~70 QPS, p50 embed 0.77-8.19ms, multi_cos EN<0.05 vs ES/combined 1.0) + docs/user/operations/BENCHMARKS.md seccion 8 con tabla 9 filas + comando reproducible --models all --skip-exception / --include-exception + .gitignore delta benchmarks/embed_bench_report*.json + py_compile ok + --help ok + reporte JSON 9 modelos generado
Resultado: ✅
Próxima acción: EMB-08 Docs + Quickstart multi (QUICKSTART.md, EMBEDDINGS.md)
Contrato: benchmarks/embed_bench.py existe y es ejecutable con --help; docs/user/operations/BENCHMARKS.md tiene seccion EMB con tabla o referencia a embed_bench; py_compile ok
...
Estado: completed
Última acción: docs/api/EMBEDDINGS.md creado (trait EmbeddingProvider + LocalOnnxProvider ort+tokenizers dim auto-detect, factory VANTA_EMBEDDING_PROVIDER/VANTA_LOCAL_MODEL, manifest 9 modelos 3/3/3, one-model-per-namespace, SQL/MCP/memory wiring) + docs/user/QUICKSTART.md §7 embed-local (download default, cargo --features embed-local, verify --check, referencia EMBEDDINGS/tutorial) + docs/user/tutorials/05-embedding-integrations.md creado (BYO-vector + embed-local, tabla 9 modelos completa, elección modelo, wiring consumidores, verificación) + docs/user/operations/EXPERIMENTAL_FEATURES.md mover embed-local de Experimental a Optional (tabla Optional con Local ONNX, texto not Experimental) + README.md mover embed-local a Optional wrapper, Experimental rename remote LLM/Ollama alternative to embed-local
Resultado: ✅
Próxima acción: EMB-09 Qwen3 excepción >3GB — wiring + doc (embeddings/README.md sección excepción, download --include-exception)
Contrato: grep -r embed-local docs/ >=3 hits (82 hits); docs/api/EMBEDDINGS.md existe (9242 bytes); docs/user/QUICKSTART.md ya no dice 'not yet exposed through the Python or TypeScript SDKs' (False → ya no dice, menciona embed-local en §7)
