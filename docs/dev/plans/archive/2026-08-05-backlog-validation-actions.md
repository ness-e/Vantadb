...
> **Inicio:** 2026-08-05
> **Estado: completed
> **Fuente:** `docs/dev/Backlog.md` (85 tareas abiertas validadas)
...
> **Justificación:** los ejecutores leen `docs/dev/Backlog.md`. Si las descripciones quedan stale, cada implementación partirá de una premisa falsa. Esta fase solo edita el backlog — no toca código.

### Task 1: Backlog-EDIT — Corregir 12 premisas stale + limpiar referencia muerta
- **Archivos clave:** `docs/dev/Backlog.md`
...
- **Acción:** fusionar en una tarea única (absorbe AUD-010):
  1. ADR breve en `docs/dev/architecture/adr/` (próximo número libre): decisión = `VANTA_DB` sigue siendo flag CLI (clap), `VANTADB_STORAGE_PATH` el env de config; el child `vantadb-server` setea ambos.
...
- **Archivos clave:** `docs/dev/architecture/EXPERIMENTAL_GOVERNANCE_DESIGN.md:14,172,138`, `src/gds.rs` (403 L, GraphDataScience page_rank :38, degree_centrality :145)
...
- **Nota:** backlog ubica el doc en docs/dev/research/ pero está en docs/dev/architecture/ (corregir path en F0).
- **Estado:** ✅ COMPLETED
...
- **Archivos clave:** `docs/dev/architecture/ARCHITECTURE.md:296-298,327`, `src/index/graph.rs:255-260,318`, `src/wal_sharded.rs:9`
...
- **Archivos clave:** `docs/dev/architecture/STORAGE_VERSIONING.md:54-55,315,168,328`, `src/index/graph.rs:142` (VECTOR_INDEX_VERSION=8), `src/storage/vfile.rs:26` (VFILE_VERSION=2), `src/wal.rs:17,31` (postcard)
...
- **Archivos clave:** `docs/dev/research/DESKTOP-01b...:1090`, `web/package.json` (next ^16.1.1), `web/next.config.ts`
...
- **Archivos clave:** `docs/progreso/README.md:1793`, `docs/progreso/bitacora.md`, `docs/user/operations/BENCHMARKS.md` (path file:/// máquina-específico), `docs/user/glosario/`
...
- **Archivos clave:** `examples/rust/graphrag.rs`, `docs/user/glosario/graphrag.md`, `docs/user/blog/`
...
Próxima acción: Revisar el lead antes de commit; reporte en docs/dev/research/AUDIT-02-2026-08-06.md