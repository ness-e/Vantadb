...
- **Fecha:** 2026-08-04
- **Fuente:** Extensión de `docs/dev/research/DESKTOP-01-tauri-plataforma-desktop.md` + 5 sub-agentes (4 de investigación de módulos + 1 definidor de arquitectura)
...
- **Estado:** ✅ Investigación + arquitectura definida. Tareas en `docs/dev/Backlog.md` → Phase 12 DESKTOP (DESKTOP-02..27).
...
26 tareas (DESKTOP-02..27) — detalle completo en `docs/dev/Backlog.md` → **Phase 12**. Regla: 1 tarea = 1 concepto; ninguna mezcla 2 integraciones; `desktop/` no toca el workspace raíz.
...
- Concurrencia interna: semaphore (max 32) + `spawn_blocking` + timeout 60s por request — lib.rs:517-537 (mismo patrón documentado en docs/dev/research/INV-003-tokio-blocking-audit.md:37).
...
- **Opción A (recomendada): crate `vantadb` directa en src-tauri/.**
  ...
  - Esta es exactamente la conclusión del informe existente: docs/dev/research/DESKTOP-01-tauri-plataforma-desktop.md:104-140 y su recomendación final :178-184 ("SÍ — Tauri v2 … vantadb como dependency directa en src-tauri/, VantaEmbedded en managed state, commands async delgados. Sin bridge WASM ni OPFS"). Esfuerzo MVP estimado: ~8-13 días (:157-172).
...
3. Docker: imagen `vantadb/server:latest` con args `server --http --port 8080 -d /data` (docs/user/operations/DEPLOYMENT_GUIDE.md:246-256).
...
**Autor:** vanta-arch · **Estado:** Propuesto · **Relacionado:** docs/dev/research/DESKTOP-01-tauri-plataforma-desktop.md · **Gama de tareas:** DESKTOP-02..DESKTOP-27
...
**ADR (inline — persistir en DESKTOP-27 como docs/dev/architecture/adr/ADR-0XX.md)**
- Estado: Aceptado (propuesta) · Fecha: 2026-08-04 · Área: desktop
