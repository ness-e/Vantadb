...
# RES-01: WAL v2 with `WalRecord::Prepare` — Analysis and Implementation Plan
...
- **`WalRecord` enum** — `src/wal.rs:40-73`: `Insert(UnifiedNode)`, `Update{id,node}`,
  `Delete{id}`, `Checkpoint{node_count,index_checksum,timestamp}`, `Begin(u64)`,
  `Commit(u64)`, `Abort(u64)`. No Prepare variant.
- **Format version** — `WAL_FORMAT_VERSION = 1` (`src/wal.rs:12`);
...
- **Version compatibility** — range-based: header accepts any `format_version <= current`
  (`src/wal.rs:140-152`). Downgrade (new format read by old binary) is NOT supported.
- **Commit point today** — `StorageEngine::commit_transaction`
  ...
  3. `ShardedWal::batch_append(wal_records)` — one `write_all` + at most one
  ...
  **The commit point is the durability of step 3**: if the process crashes during the
  ...
  incomplete prefix.
- **Crash-atomic replay (MOD-02, resolved)** — `src/storage/engine/init.rs:505-551`:
...
- But the txn buffer was already dropped (`txn.rs:133-136`) and the WAL holds
  `Commit` — on restart, replay re-applies ALL ops (`init.rs:559-599`).
...
- Keep the MOD-02 path for v1 records — mixed WALs (v1 tail + v2 head) must work
...
physically present — the prerequisite for stable snapshots under concurrent apply and
...
| Scope creep toward full 2PC/coordinator | real risk | this design has NO lock phase, NO participant set — it is a redo-log prepare, nothing more |
...
longer contiguous — but the invariant MOD-02 established (an uncommitted txn's ops are
...
- **S1** — Add `Prepare { txn_id, op_count }` variant; bump `WAL_FORMAT_VERSION` to 2;
  roundtrip + mixed-version unit tests. Verify: `cargo nextest run -p vantadb wal`.
- **S2** — Two-phase `commit_transaction` behind `config.wal_prepare: bool`
  (default `false`): flow per section 4.2; failpoint test mid-apply asserting
  Abort + Err + no resurrection on replay.
- **S3** — Recovery v2 outcome-map (two-pass), coexisting with MOD-02 for v1 records;
  chaos tests via existing `failpoints` feature (`wal_append_fail`, `wal.rs:272-277`).
- **S4** — Persist `max_committed_txn` at checkpoint; wire watermark into
  snapshot visibility; update `docs/api` durability docs (Doc-Driven Development).
- **S5** — Bench before/after with flag on/off against `benches/canonical_p99.rs`
  (Regla 9); record in `docs/user/operations/BENCHMARKS.md`. Go/no-go data for owner.
- **S6** — After owner ADR: flip default, close backlog rows, changelog via release-plz.
