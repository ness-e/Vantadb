...
|---|----------|----------------|--------------------------|
| 1 | **Saturación <20% como stop**: "una fase termina cuando una ronda nueva de búsqueda añade <20% de fuentes nuevas o ninguna fuente sobre umbral de citas/relevancia"; evaluar por fuente | `docs/dev/research/2026-08-10-agent-engineering/agent-02-task-execution.md:275-279` (§7.6); también `:240` (§7.2), `:476` (§14 Fase A), `:483` (§14 Fase B) | ❌ ausente. Único 20% en prompts = Context Budget (`iter-loop-tools.md:372`) — es otra cosa, no saturación de research |
