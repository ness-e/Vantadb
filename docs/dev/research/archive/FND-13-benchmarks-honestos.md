...
### 2.4 `docs/user/operations/BENCHMARKS.md`
...
### 2.5 `docs/user/operations/PERFORMANCE_TUNING.md`
...
| `docs/user/operations/BENCHMARKS.md` §4/§6/§7 | Citar comandos exactos de `prefetch_comparison.py`, `batch_vs_sequential_bench.py`, `competitive_bench.py` | Futura tarea tuner |
| `docs/user/operations/PERFORMANCE_TUNING.md` T2-T9, T11, T12 | Claims sin fuente (recall SQ8, SIMD, build time, sync modes, storage, tabla de escala) — reformular con números o quitar | Futura tarea tuner |
| `docs/user/operations/BENCHMARKS.md` §2 | Regenerar tabla con `update_markdown.py` para sincronizar con el JSON | CI / release |

---

## 6. Verificación del contrato

1. `grep AGENTS.md "Regla 11"` → ✅ presente, referenciando benchmark reproducible (`canonical_p99`, `docs/user/operations/BENCHMARKS.md`, archivos `benches/*.rs`/`benchmarks/*.py` + comando)
2. Análisis en `docs/dev/research/FND-13-benchmarks-honestos.md` → ✅ existe con inventario clasificado (citado/medible/sin fuente)
