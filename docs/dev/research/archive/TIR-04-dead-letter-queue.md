...
| **Plan file** | Fila de la tarea en `❌ FAILED` + recitation (activeGoal, lastAction, result, nextAction, contract/evidencia/artefactos) | `docs/dev/plans/<plan>.md` vía `campaign_update_task_state` |
...
3. **Documentar el índice de fallos** en la operación del lead: `rg -l "❌ FAILED" docs/dev/plans/*.md` + `tasks/closed/` como vistas de inspección.
