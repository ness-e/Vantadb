...
| **Action on threshold not met** | Default-on **confirmed**; graph engine stays compiled by default. Close this ADR's reopen signal; document the measured evidence in `docs/user/operations/BENCHMARKS.md` or the follow-up ADR. |
...
- FND-03 (`docs/dev/Backlog.md:485`) — feature isolation for vector-only consumer.