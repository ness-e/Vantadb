...
> **Source:** `docs/dev/research/2026-08-10-agent-engineering/REPORTE-FINAL.md` §3.3-24 (FALTA #24) · plan `docs/dev/plans/archive/2026-08-11-residuo-consolidado.md` Task 19.
> **Backlog:** `docs/dev/Backlog.md` §P17 → `TSYS-06`.
...
|------|-----------|---------|---------------------------|
| `docs/dev/plans/<plan>.md` | `campaign_update_task_state`, `getOrCreateCampaignId` | every read tool (`parseTasks`) | tasks silently dropped, states misdetected, wrong task updated |
| `docs/dev/plans/<plan>.budget.json` | `writeBudget` (init/consume/reset, `getOrCreateTraceId`) | `readBudget`, `budgetStatus`, `campaign_stalled_tasks` | `readBudget` catch → `{ tasks: {} }` → counters/traceId reset mid-run |
...
| C11 | **Plan file missing from `docs/dev/plans/` while running** | `findPlanFile` returns null → tools return `{ error: "No plan file found" }` instead of crashing (current behavior); campaign cannot silently claim a task | Fail loudly, not silently |
