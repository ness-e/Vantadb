...
- The certified stress protocol reports **Recall@10 = 0.9560** at 10K×128d
  ...
  (`docs/user/operations/BENCHMARKS.md:27-30`,
  `docs/user/operations/PERFORMANCE_TUNING.md:454-458`).
...
- **Why 100 is the default**: at `ef_search=100`, `top_k=10`, the certified
  ...
  (`docs/user/operations/PERFORMANCE_TUNING.md:454-457`). Lower values (32–64)
...
- **Cost**: edges dominate index memory. Each directed edge is a `u128` id
  ...
  `docs/user/operations/PERFORMANCE_TUNING.md:87-93`: for 128d at defaults,
...
- **Rule of thumb**: never ship `ef_construction < ef_search` — build
  quality caps search quality (`docs/user/operations/PERFORMANCE_TUNING.md:73`).
...
1. **Latency target**: sub-ms to low-ms on 10K–1M vectors. Certified p50
   ...
   50K vs 5× linear) — `docs/user/operations/PERFORMANCE_TUNING.md:456-470`.
   Rust-core p99 ≈ 441 µs @ 100K (`docs/user/operations/PERFORMANCE_GUIDE.md:104`).
...
| **Mature quantization (PQ/OPQ)** | VantaDB ships scalar (SQ8/3-bit/1-bit) and mmap tiering (`docs/user/operations/PERFORMANCE_TUNING.md:150-162`); PQ is a documented future path (`docs/dev/architecture/PQ_FEASIBILITY.md`) |
...
`docs/user/operations/BENCHMARKS.md:27-30` — Stress Protocol
...
(`docs/user/operations/PERFORMANCE_TUNING.md:462-467`).
