...
- Contrato: `rust-dsm -f json` archivado en `docs/reviews/` + `cargo modules dependencies --lib --acyclic` verde + job CI informativo (no bloqueante aún) + tabla baseline en task file. Pins: `cargo-modules 0.27 + cargo-coupling <versión fijada en DISCOVERY> + rust-dsm@<commit fijado>`; validar JSON con `jq`; documentar alcance por target (`--lib/--bin/-p`, `--features`, `--cfg-test`) y workspace.
- Task file `docs/tasks/C2M1.md` · ✅ COMPLETED · Ruta vanta-worker.
...
- MCP/tools: `codebase-memory-mcp_get_architecture`, `check_index_coverage`; bash: `cargo install cargo-modules rust-dsm`, `cargo modules dependencies --lib --acyclic`, `rust-dsm -f json -o docs/reviews/dsm-baseline.json`.
...
- Task file `docs/tasks/C2C1.md` · ✅ COMPLETED · Ruta vanta-worker.
...
- Contrato: `docs/tasks/C2D0.md` con tabla cambio-conjunto por sección + recomendación (partir por dominios storage/server/llm/evicción/pool/rbac vs mantener) + `question` al usuario.
- Task file `docs/tasks/C2D0.md` · ✅ COMPLETED · Ruta vanta-arch.
...
- Appetite 3h · 🟢 · 🟡 · nuevo `docs/architecture/BOUNDARIES.md` + diagrama
...
- Contrato: doc creado + enlazado desde `docs/architecture/` + regla citable por `/cleanCA` futuro (A3) + sección "Fase 3: gate de reactivación de reorg física". Añadir convención `pub use` re-export (conservar API al mover) + regla "ciclos entre siblings = error" para que A3 tenga qué citar.
- Task file `docs/tasks/C2A1.md` · ✅ COMPLETED · Ruta vanta-docs (con revisión vanta-arch).
...
- Task file `docs/tasks/C2S1.md` · ✅ COMPLETED · Ruta vanta-worker. Depende de: M1 (números antes/después).
...
- Task file `docs/tasks/C2S7.md` · ✅ COMPLETED · Ruta vanta-worker.
...
- Task file `docs/tasks/C2M2.md` · ✅ COMPLETED · Ruta vanta-worker. Depende de: M1.
...
- Task file `docs/tasks/C2S2.md` · ✅ COMPLETED · Ruta vanta-worker (diseño previo vanta-arch si DISCOVERY lo pide). Depende de: S1.
...
- Task file `docs/tasks/C2S3.md` · ✅ COMPLETED · Ruta vanta-worker. Depende de: S1, M1. Slices obligatorios (txn primero, cache después).
...
- Task file `docs/tasks/C2S5.md` · ✅ COMPLETED · Ruta vanta-worker.
...
- Task file `docs/tasks/C2T1.md` · ✅ COMPLETED · Ruta vanta-worker.
...
- Task file `docs/tasks/C2T2.md` · ✅ COMPLETED · Ruta vanta-worker (mem0: con cuidado integrations).
...
- Task file `docs/tasks/C2T3.md` · ✅ COMPLETED · Ruta vanta-worker.
...
- Task file `docs/tasks/C2S3b.md` · ✅ COMPLETED · Ruta vanta-worker. Depende de: S3-txn (`cdb15b3e`), Wave 3 cerrada (suite rápida+determinista como red). Bloquea: nada de Wave 4 (disjunta), pero debe cerrar antes de Wave 5 (M3/M1-full exigen la D final).
...
- Task file `docs/tasks/C2C2.md` · ✅ COMPLETED · Ruta vanta-docs (con vanta-worker para firmas). Slices por binding.
...
- Task file `docs/tasks/C2A2.md` · ✅ COMPLETED · Ruta vanta-worker (frontend) o docs.
...
- Task file `docs/tasks/C2M3.md` · ✅ COMPLETED · Ruta vanta-worker. Depende de: M1 (medir ciclo), A1 (boundaries). BLOQUEADA hasta re-medición.
...
- Task file `docs/tasks/C2S6.md` · ✅ COMPLETED · Ruta vanta-arch (diseño) → vanta-worker.
...
> flipadas a ✅ en este commit. `skill progreso`: campagne registrada en `docs/avance/`
> + nota en `meta.md`; plan archivado a `docs/plans/archive/`.
...
- **S6 cadena LogicalOperator (Task 17):** nació DEFER-activo ("solo si hay apetito", costo 🔴
  ...
  `docs/tasks/C2S6.md` (vanta-arch) → implementación slices S6-1/S6-2/S6-3
