...
fn bench_tokenizer_text_sizes(c: &mut Criterion) {
    ...
    group.bench_with_input(BenchmarkId::new("advanced", "short"), &short_text, |b, text| {
        b.iter(|| tokenize_advanced_default(black_box(text)))
    });
    
    group.bench_with_input(BenchmarkId::new("advanced", "medium"), &medium_text, |b, text| {
        b.iter(|| tokenize_advanced_default(black_box(text)))
    });
    
    group.bench_with_input(BenchmarkId::new("advanced", "long"), &long_text, |b, text| {
        b.iter(|| tokenize_advanced_default(black_box(text)))
    });
...
fn bench_multilingual(c: &mut Criterion) {
    ...
    for (lang, text) in [("english", english), ("spanish", spanish), ("french", french), ("german", german)] {
