...
complete -c vanta-cli -n "__fish_vanta_cli_needs_command" -f -a "tui" -d 'Launch the interactive TUI (requires `tui` feature)'
...
complete -c vanta-cli -n "__fish_vanta_cli_using_subcommand tui" -s d -l db -d 'Path to the database directory. Defaults to the value of the VANTADB_STORAGE_PATH environment variable, or \'./db\' if neither is set' -r
complete -c vanta-cli -n "__fish_vanta_cli_using_subcommand tui" -l memory-limit -d 'Optional memory limit for the database engine, in bytes. Accepts suffixes: KB, MB, GB (also KiB, MiB, GiB), e.g. `500MB` or `2GB`. Defaults to the value of the VANTADB_MEMORY_LIMIT environment variable' -r
complete -c vanta-cli -n "__fish_vanta_cli_using_subcommand tui" -s v -l verbose -d 'Enable verbose output'
complete -c vanta-cli -n "__fish_vanta_cli_using_subcommand tui" -s h -l help -d 'Print help'
...
complete -c vanta-cli -n "__fish_vanta_cli_using_subcommand wal; and not __fish_seen_subcommand_from compact vacuum help" -s d -l db -d 'Path to the database directory. Defaults to the value of the VANTADB_STORAGE_PATH environment variable, or \'./db\' if neither is set' -r
...
complete -c vanta-cli -n "__fish_vanta_cli_using_subcommand help; and not __fish_seen_subcommand_from put get list rebuild-index audit-index repair-text-index export import query status backup restore doctor inspect stats tui completions search delete delete-by-filter count similar-to-key migrate namespace snapshot wal search-multi search-all server help" -f -a "put" -d 'Save a key-value pair to persistent memory'
...
complete -c vanta-cli -n "__fish_vanta_cli_using_subcommand help; and not __fish_seen_subcommand_from put get list rebuild-index audit-index repair-text-index export import query status backup restore doctor inspect stats tui completions search delete delete-by-filter count similar-to-key migrate namespace snapshot wal search-multi search-all server help" -f -a "tui" -d 'Launch the interactive TUI (requires `tui` feature)'
