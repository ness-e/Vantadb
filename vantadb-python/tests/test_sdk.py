"""
NexusDB Python SDK — Integration Test Suite
...
class TestNexusDBLifecycle:
    ...
    def test_open_and_repr(self):
        """NexusDB instance should open and display hardware profile."""
        db = nexus.NexusDB(_unique_path(), memory_limit_bytes=128 * 1024 * 1024)
        r = repr(db)
        assert "NexusDB(" in r
        assert "profile=" in r
    ...
    def test_insert_and_get(self):
        ...
        db = nexus.NexusDB(_unique_path(), memory_limit_bytes=128 * 1024 * 1024)
        db.insert(42, "Hello NexusDB", [0.1] * 384)
        
        node = db.get(42)
        assert node is not None
        assert node["id"] == 42
        assert node["fields"]["content"] == "Hello NexusDB"
    ...
    def test_insert_with_extra_fields(self):
        ...
        db = nexus.NexusDB(_unique_path(), memory_limit_bytes=128 * 1024 * 1024)
        db.insert(1, "Test node", [0.5] * 128, fields={
            "category": "test",
            "score": 42,
            "active": True,
        })
...
class TestMemoryBoundary:
    ...
    def test_explicit_memory_limit(self):
        ...
        db = nexus.NexusDB(_unique_path(), memory_limit_bytes=64 * 1024 * 1024)
