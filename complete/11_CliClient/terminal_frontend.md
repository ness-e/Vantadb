...
## 1. REPL Interface
Developers working with Local AI need a direct method to inspect the Unified Nodes stored in IADBMS. 
The `iadbms-cli` provides an interactive shell (REPL) out of the box, connecting implicitly to localhost:8080 or bypassing network for an embedded memory state.
