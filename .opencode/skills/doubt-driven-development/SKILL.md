...
This is not `/audit review`. `/audit review` is a verdict on a finished artifact. This is an in-flight posture: non-trivial decisions get cross-examined while course-correction is still cheap.
...
| "I'll do doubt at the end with `/audit review`" | `/audit review` is a final gate. Doubt-driven catches wrong directions early when course-correction is cheap. By PR time it's too late. |
...
- Doubting only after committing — that's `/audit review`, not doubt-driven development
...
- **`code-review-and-quality` / `/audit review`**: complementary. `/audit review` is post-hoc PR verdict; doubt-driven is in-flight per-decision. Use both.
