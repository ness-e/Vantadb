#!/usr/bin/env node
/**
 * VantaDB Model Proxy — Entry point for the campaign executor.
 *
 * Usage:
 *   node start-proxy.js                              # standalone, multi-backend
 *   node start-proxy.js --mode deepseek               # start with DeepSeek
 *   node start-proxy.js --port 3200 --mode openrouter # custom port + OpenRouter
 *
 * Called by vantadb-proxy.ps1:
 *   $port = node .agents\proxy\start-proxy.js <url> <key>
 *
 * Control endpoints (live toggle):
 *   POST /_proxy/mode  -d backend=deepseek
 *   GET  /_proxy/status
 *   GET  /_proxy/cost
 */
...
// Legacy mode: start-proxy.js <targetUrl> <apiKey> (used by vantadb-proxy.ps1)
