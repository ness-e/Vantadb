...
export function startModelProxy({ targetUrl, apiKey, startPort = 3200, backends, defaultMode }) {
    return new Promise((resolve, reject) => {
        ...
        const server = createServer((clientReq, clientRes) => {
            ...
            if (urlPath.startsWith('/_proxy/')) {
                ...
                if (urlPath === '/_proxy/mode' && clientReq.method === 'POST') {
                    ...
                    clientReq.on('end', () => {
                        ...
                        console.log(`[VANTADB-PROXY] Mode switched: ${result.previous} → ${result.mode}`);
                        clientRes.writeHead(200, { 'content-type': 'application/json' });
                        clientRes.end(JSON.stringify(result));
                    });
                    return;
            ...
            if (isModelCall) {
                console.log(`[VANTADB-PROXY] #${reqId} → ${dest.hostname}${fullPath}`);
            }
            ...
            clientReq.on('end', () => {
                ...
                if (isModelCall && MODEL_REMAP[state.mode]) {
                    try {
                        ...
                        if (mapped) {
                            console.log(`[VANTADB-PROXY] #${reqId} model remap: ${parsed.model} → ${mapped}`);
                            parsed.model = mapped;
                ...
                const proxyReq = httpsRequest(opts, (proxyRes) => {
                    if (isModelCall) {
                        ...
                        console.log(`[VANTADB-PROXY] #${reqId} TTFB ${ttfb}ms (status ${proxyRes.statusCode})`);
                    }
                    ...
                    if (isModelCall && isSSE) {
                        ...
                        proxyRes.on('end', () => {
                            console.log(`[VANTADB-PROXY] #${reqId} done in ${((Date.now() - t0) / 1000).toFixed(1)}s (${norm._inputTokens}in/${norm._outputTokens}out)`);
                        });
                        ...
                        proxyRes.on('end', () => {
                            ...
                            clientRes.end(fixed);
                            console.log(`[VANTADB-PROXY] #${reqId} done in ${((Date.now() - t0) / 1000).toFixed(1)}s (json, ${fixed.length}b)`);
                        });
                    } else {
                        ...
                        if (isModelCall) {
                            proxyRes.on('end', () => {
                                console.log(`[VANTADB-PROXY] #${reqId} done in ${((Date.now() - t0) / 1000).toFixed(1)}s`);
                            });
                        }
                    }
                });
                ...
                proxyReq.on('timeout', () => {
                    console.error(`[VANTADB-PROXY] #${reqId} TIMEOUT after ${REQUEST_TIMEOUT_MS / 1000}s`);
                    proxyReq.destroy(new Error('Request timeout'));
                });

                proxyReq.on('error', (err) => {
                    const elapsed = ((Date.now() - t0) / 1000).toFixed(1);
                    console.error(`[VANTADB-PROXY] #${reqId} ERROR after ${elapsed}s: ${err.message}`);
        ...
        function tryListen(port) {
            ...
            server.listen(port, '127.0.0.1', () => {
                ...
                console.log(`[VANTADB-PROXY] Listening on 127.0.0.1:${actualPort} → ${targetUrl} (mode: ${state.mode})`);
