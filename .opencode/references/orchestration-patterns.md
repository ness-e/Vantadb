...
```
/audit review → five-axis code review → report
```
...
Plugin subagents go in `agents/` at the plugin root. This repo uses OpenCode's `.opencode/agents/` — see `vanta-audit.md`, `vanta-chaos.md`, `vanta-tuner.md`, etc. for the VantaDB specialist personas. The legacy `code-reviewer`, `security-auditor`, and `test-engineer` have been replaced by the vanta-* family.
...
Don't redefine these. Layer your specialist personas (vanta-audit, vanta-chaos, vanta-tuner) on top of them.
...
- The user already knew they wanted a review; they could have called `/audit review` directly
