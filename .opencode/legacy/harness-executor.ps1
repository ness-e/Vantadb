...
.PARAMETER OpenCodePrompt
  ...
  Default: .opencode/skills/backlog-executor/hybrid-prompt.md
...
while ($true) {
  ...
  # Construir prompt para el agente — usa el prompt file completo
  $promptPath = if ($OpenCodePrompt) {
    $OpenCodePrompt
  } else {
    Join-Path $projectRoot ".opencode" "skills" "backlog-executor" "hybrid-prompt.md"
  }
  ...
  if ($DryRun) {
    ...
    try {
      ...
      # Mostrar y validar recitation
      $recitation = Get-Recitation $planFile
      if (-not $recitation) {
        Write-Host "  ⚠️ Recitation no encontrada o corrupta" -ForegroundColor Yellow
        Write-Host "     ¿Continuar de todas formas? (s/N)" -ForegroundColor Yellow
        $answer = Read-Host
        if ($answer -ne 's') {
          Write-Host "❌ Campaña abortada por recitation faltante." -ForegroundColor Red
          exit 3
        }
      } elseif ($recitation -notmatch '[✅❌]') {
        Write-Host "  ⚠️ Recitation no contiene ✅ ni ❌ — posible incompleta" -ForegroundColor Yellow
        Write-Host "     Contenido: $recitation" -ForegroundColor DarkGray
        Write-Host "     ¿Continuar de todas formas? (s/N)" -ForegroundColor Yellow
        $answer = Read-Host
        if ($answer -ne 's') {
          Write-Host "❌ Campaña abortada por recitation inválida." -ForegroundColor Red
          exit 3
        }
      } else {
